...
export function validateBrokerConfiguration(config) {
  if (!config || config.clusterArn !== STAGE_B.clusterArn || config.approvalSecretArn !== STAGE_B.approvalSecretArn
      ...
      || config.privateSubnetIds.length !== 2 || config.privateSubnetIds.some((id) => !/^subnet-[a-f0-9]+$/.test(id))
...
export function createHandler({ config, readApproval, verifySignature, claimApproval, runTask, writeReceipt = async () => {}, now = () => new Date() }) {
  ...
  return async (event, context = {}) => {
    ...
    const response = await runTask({ cluster: STAGE_B.clusterArn, taskDefinition, launchType: "FARGATE", count: 1, networkConfiguration });
    if ((response.failures || []).length || response.tasks?.length !== 1) throw new Error("Stage B broker did not start exactly one fixed task.");
...
export async function handler(event, context) {
  ...
  if (!/^[A-Za-z0-9._-]{3,255}$/.test(config.replayTable || "") || !/^mscqr-production-[a-z0-9-]+-artifacts-368992683803-eu-west-2-an$/.test(config.receiptBucket || "")) throw new Error("Stage B broker storage is outside the reviewed contract.");
  const [{ ECSClient, RunTaskCommand }, { SecretsManagerClient, GetSecretValueCommand }, { KMSClient, VerifyCommand }, { DynamoDBClient, PutItemCommand }, { S3Client, PutObjectCommand }] = await Promise.all([
  ...
  return createHandler({
    ...
    claimApproval: async ({ approvalId, nonce, mode, expiresAt }) => dynamo.send(new PutItemCommand({
      TableName: config.replayTable, Item: { approvalMode: { S: `${approvalId}#${mode}` }, approvalNonce: { S: nonce }, expiresAt: { N: String(Math.floor(Date.parse(expiresAt) / 1000)) } }, ConditionExpression: "attribute_not_exists(approvalMode)",
    })),
