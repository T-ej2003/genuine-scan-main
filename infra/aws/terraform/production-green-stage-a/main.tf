locals {
  tags = merge(
    { Environment = "production", ManagedBy = "Terraform", Component = "full-rls-green-stage-a" },
    var.tags,
  ...
  initial_activation_policy_reconciliation_non_reservation_object_arns = concat(
    local.activation_lifecycle_object_arns,
    local.rebaseline_evidence_object_arns,
    local.stage_a_production_artifacts_reconciliation_object_arns,
    local.provider_readonly_policy_reconciliation_object_arns,
  )
...
resource "aws_s3_bucket_policy" "production_artifacts" {
  ...
  policy = jsonencode({ Version = "2012-10-17", Statement = [
    ...
    { Sid = "DenyNonTargetInitialActivationPolicyReconciliationReservationReplacements", Effect = "Deny", Principal = "*", Action = "s3:PutObject", NotResource = concat(local.initial_activation_policy_reconciliation_non_reservation_object_arns, local.initial_activation_policy_reconciliation_legacy_transition_reservation_object_arns), Condition = { StringEquals = { "aws:PrincipalArn" = "arn:aws:iam::368992683803:root" }, StringNotEquals = { "s3:if-none-match" = "*" } } },
    ...
    { Sid = "AllowInitialActivationReconcilerListProviderReadonlyReconciliation", Effect = "Allow", Principal = { AWS = "arn:aws:iam::368992683803:role/mscqr-production-initial-activation-policy-reconciler" }, Action = "s3:ListBucket", Resource = var.receipt_bucket_arn, Condition = { StringLike = { "s3:prefix" = ["production-provider-readonly-policy-reconciliation/*"] } } },
