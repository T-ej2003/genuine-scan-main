...
resource "aws_s3_bucket_policy" "production_artifacts" {
  ...
  policy = jsonencode({ Version = "2012-10-17", Statement = [
    ...
    { Sid = "DenyNonTargetInitialActivationPolicyReconciliationReservationReplacements", Effect = "Deny", Principal = { AWS = "arn:aws:iam::368992683803:root" }, Action = "s3:PutObject", NotResource = concat(local.initial_activation_policy_reconciliation_non_reservation_object_arns, local.initial_activation_policy_reconciliation_legacy_transition_reservation_object_arns), Condition = { StringNotEquals = { "s3:if-none-match" = "*" } } },
