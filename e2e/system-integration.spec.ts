...
test.describe("MSCQR disposable system integration", () => {
  test("public verification uses the real backend for valid and invalid QR results", async ({ page, context }) => {
    ...
    await invalidPage.goto(`/verify/${encodeURIComponent(invalidCode)}`, { waitUntil: "domcontentloaded" });
    const invalidResult = await visibleResultContainer(invalidPage, /could not match this QR label|could not check this garment/i);
    await expect(invalidResult.getByText(/could not match this QR label|could not check this garment/i).first()).toBeVisible();
    await expect(invalidPage.locator("body")).not.toContainText(safePublicText);
