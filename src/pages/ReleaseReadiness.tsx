...
export default function ReleaseReadiness() {
  ...
  const [routeTelemetry, setRouteTelemetry] = useState<any | null>(null);
  ...
  const loadAll = useCallback(
    async (showToast = false) => {
      ...
      if (complianceJobsRes.status === "fulfilled" && complianceJobsRes.value.success) {
        const jobs = (complianceJobsRes.value.data as any)?.jobs;
  ...
  const actionItems = useMemo(() => {
    ...
    if (!releaseMetadata) {
      items.push({
        ...
        body: "The internal release endpoint is not returning build identity. Platform operators should treat release promotion as blocked until metadata is visible again.",
      });
    ...
    if ((compliance?.controlSummary?.ATTENTION || 0) > 0) {
      items.push({
        tone: "warning",
        title: "Governance controls need attention",
        body: `${compliance.controlSummary.ATTENTION} control area(s) are marked Attention in the current governance summary.`,
      });
    }

    if ((routeTelemetry?.verifyFunnel?.dropped || 0) > 0) {
      items.push({
        tone: "warning",
        title: "Verify funnel drops detected",
        body: `${routeTelemetry.verifyFunnel.dropped} route-transition drop(s) were recorded in the current telemetry window.`,
      });
    }
            ...
            <CardContent className="space-y-3 pt-4 text-sm text-slate-700">
              ...
              <div className="rounded-lg border border-slate-200 bg-white p-3">
                ...
                <div className="mt-1 font-mono text-xs text-slate-900">
                  {rateLimitAnalytics?.topLimitedRoutes?.[0]?.route || "No recent limiter events"}
                </div>
