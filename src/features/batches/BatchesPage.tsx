...
import { useAssignableManufacturers, useBatches } from "@/features/batches/hooks";
import { useManufacturerPrinterRuntime, usePrintJobs, type ManufacturerPrinterRuntime } from "@/features/printing/hooks";
...
export default function Batches() {
  ...
  const manufacturersQuery = useAssignableManufacturers(user?.licenseeId || undefined, false);
  ...
  const applyPrinterRuntimeSnapshot = (
    ...
    if (nextPreferredPrinterId) {
      setSelectedPrinterId((prev) => prev || nextPreferredPrinterId);
    }
  ...
  const fetchBatches = async () => {
    ...
    setError(null);
    try {
      const res = await batchesQuery.refetch();
      if (!res.data) {
        setRows([]);
        setError(res.error instanceof Error ? res.error.message : "Failed to load batches");
        return;
      }
  ...
  const fetchManufacturersForAssign = async () => {
    ...
    const res = await manufacturersQuery.refetch();
    if (res.data) {
      setManufacturers((Array.isArray(res.data) ? res.data : []) as ManufacturerRow[]);
    } else {
      setManufacturers([]);
    }
  };
  ...
  const loadRegisteredPrinters = async (preferredLocalPrinterId?: string | null) => {
    ...
    applyRegisteredPrintersSnapshot(
      response.data.registeredPrinters as RegisteredPrinterRow[],
      preferredLocalPrinterId || response.data.preferredPrinterId
    );
  };

  const loadRecentPrintJobs = async (_batchId?: string) => {
    if (!isManufacturer) return;
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [canAssignManufacturer, user?.licenseeId]);
  ...
  return (
    <DashboardLayout>
      <DataTablePagePattern
        ...
        description={
          isManufacturer
            ? "Your assigned QR batches for controlled print runs, job recovery, and post-print confirmation."
      ...
      >
        ...
        >
          <DialogContent data-testid="create-print-job-dialog" className="sm:max-w-[640px] max-h-[85vh] overflow-y-auto">
            <DialogHeader>
