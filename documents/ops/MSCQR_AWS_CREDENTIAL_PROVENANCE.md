...
| GitHub scheduled CloudFront proxy drift verification | workflow OIDC session | `github-oidc-proxy-drift-readonly` is bound to the exact protected-main workflow and permits only the AWS reads needed for its comparison; it does not enter the approval-gated mutation environment. |
...
The credential-source contract is implemented in `scripts/aws/production-credential-source-contract.mjs`. The release-gate workflow must pass `--credential-source github-oidc-release-deployer` to every MSCQR AWS CLI that runs after its OIDC configuration step. The scheduled proxy drift verifier selects `github-oidc-proxy-drift-readonly`. `publish-ecs-images.yml` maps its authenticated `auth-mode` output explicitly: `oidc` selects `github-oidc-release-deployer`; `keys` selects `github-access-keys`. Local production operator commands must not use either GitHub source: they use the explicit named local release profile instead.
