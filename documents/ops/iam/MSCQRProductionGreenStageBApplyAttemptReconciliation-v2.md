...
that mutation may have occurred. `APPLIED`, `FAILED`, and `UNKNOWN` are
terminal for automatic successor preparation.
...
time, unchanged state, or a missing local marker. A separately reviewed
incident procedure is required before any future action.
