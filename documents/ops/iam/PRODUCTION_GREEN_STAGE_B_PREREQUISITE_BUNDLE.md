...
The canonical `produce-production-green-stage-b-release-preflight.yml` workflow publishes the read-only report from the existing Stage-B preflight implementation. It consumes only a provenance-checked Release Gate image-authorization artifact and exact tfvars/binding hashes; it does not accept report contents or filesystem paths. The preparation workflow transports that report as a private GitHub artifact, never as a `workflow_dispatch` payload, and accepts only the fixed producer path, exact run and attempt, successful conclusion, non-expired artifact ID/name, and GitHub artifact digest; the downloaded archive must contain exactly `release-preflight.json`.
