...
Execution verifies the same source, authorization, state object identity, live IAM topology, and attachment before applying the refresh-only plan. It then requires an exact state successor and a second normal Terraform plan with zero resource drift and zero actionable changes. If a prior execution already persisted that exact successor but failed before publishing completion evidence, the same still-fresh authorization may authenticate it and complete with zero Terraform applies. Any other drift, attachment, inline policy, policy document, boundary representation, or plan action fails closed. The following no-op installer transaction remains a separate fresh authorization.
