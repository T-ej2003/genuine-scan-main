...
The purpose-bound `INITIAL_ACTIVATION_RECONCILER` GitHub Actions OIDC path is installed and prepared by #451, but it is not yet the current InitialActivationLifecycle mutation path on protected main. PR #448 is the reviewed migration that will move production target-policy mutation to that reconciler, using authenticated authorization and `production-deploy` concurrency controls. Until that migration is merged and verified, do not treat the reconciler path as live or the reservation statements as retired; afterward, the exact reverse Stage-A transition may retire the reservation boundary. Historical evidence from either phase remains distinguishable by its source and transition binding.
