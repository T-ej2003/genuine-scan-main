...
It accepts only the authenticated `v2` predecessor with the exact version inventory `[v1,v2]` and SHA-256 `7e9eef0b5dd5c089f4734a43cbc40ed963078dc500828c2e592cc07f04c6d564`, or the exact `v3` successor with `[v1,v2,v3]`. A pre-existing nondefault `v3` therefore fails before mutation instead of producing an unreviewed `v4`. The desired document is always `documents/ops/iam/MSCQRProductionInitialActivationLifecycle-v1.json` with SHA-256 `c6f917d8e246bc5b4bcdf3ea189f21128e23639aa1d9643b795d4f0cedf42134`. The sole new capability permits `secretsmanager:UpdateSecretVersionStage` on the seven immutable mixed-topology recovery ARNs; it grants no other Secrets Manager mutation.
