...
- Hardening apply is manual-only and protected by `dr-hardening-apply`. CloudWatch alarms, ALB access logs, and WAF COUNT mode are staged before any ASG apply. ASG apply is the highest-risk hardening step and must not be run unless `documents/ops/aws-asg-multi-instance-readiness.md` says `ASG_STATUS=CONDITIONALLY_READY` or `ASG_STATUS=READY`.
- Current app scaling gate: ASG_STATUS=CONDITIONALLY_READY. Before using `I_APPROVE_REGIONAL_ASG_CREATE_AND_ATTACH`, review `documents/ops/aws-asg-multi-instance-readiness.md`, `documents/ops/aws-asg-multi-instance-readiness.checklist.json`, `documents/ops/aws-asg-rolling-deploy-policy.md`, and a fresh `node scripts/dr/check-asg-multi-instance-readiness.mjs` result. The first ASG rollout must remain a no-production-DNS validation drill.
- Bucket deletion, DB deletion, DB failover, recursive object deletion, Docker prune, and automatic MinIO data deletion remain out of scope.
