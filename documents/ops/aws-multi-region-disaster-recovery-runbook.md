...
Latest ASG stabilization evidence: fresh ASG nodes got through Docker image builds, backend startup, backend container health, frontend/Nginx startup, loopback `/healthz`, host-primary-IP `/healthz`, and at least one public `/healthz` probe. ASG nodes still churned because cold bootstrap reached frontend `/healthz` around 295 seconds while the previous ASG health grace/default warmup were 180 seconds. The regional target group also still included legacy source instance `i-04ae3b689ab72a68a`, so total healthy target count could be misleading. The repo fix raises health grace/default warmup to 900 seconds, keeps `/healthz` independent of backend dependencies, hard-fails unless both loopback and host-primary-IP edge liveness pass, counts only current ASG instance IDs for readiness, prints legacy/source healthy targets separately, and hardens evidence collection to split IDs safely, curl each public IP, and loop console/SSH collection per instance.
