...
Latest ASG health evidence: fresh ASG nodes got through Docker image builds and backend startup. Backend listened on port 4000 internally and logged QR signing profile ready with `keyVersion: v1`, but `/health/ready` returned 503 repeatedly. Frontend stayed `Created` because Compose waited for backend `service_healthy`; therefore no process listened on host port 80 and ALB target health failed. The repo fix changes backend container health to `/health/live`, keeps frontend startup behind backend liveness, and adds no-secret bootstrap diagnostics to reveal database/Redis/object-storage readiness state when `/api/health/ready` blocks success.
