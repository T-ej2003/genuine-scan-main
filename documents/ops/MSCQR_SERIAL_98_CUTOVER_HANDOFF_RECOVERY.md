...
- Rotation supersession classifies every slot as `OLD_AUTHENTICATED`, `NEW_AUTHENTICATED`, `UNKNOWN`, or `INVALID`. It reads each captured predecessor by its immutable VersionId, verifies the returned VersionId, then rereads all seven `AWSCURRENT` labels immediately before its first write. Only an authenticated old-only state writes; an authenticated old/new prefix resumes missing slots; all-new is a zero-write replay. Every write uses a deterministic source/rotation/slot request token.
