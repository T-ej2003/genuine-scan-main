...
- Root-drop evidence: `produce-production-root-drop-evidence.mjs` requires an explicitly selected root operator profile, verifies the exact root caller with STS, and signs the canonical root payload with the dedicated root-only KMS key. The key policy denies non-root `kms:Sign`/`kms:Verify`; the release role may only read the public key for verification. SHA-256 is integrity/domain binding only; a self-hashed JSON object is never authorization evidence. Provisioning the new Stage-A root-drop key is a separate reviewed infrastructure prerequisite; this PR does not apply it.
