...
- [ ] General role-management actions are scoped to exactly the four Terraform-managed roles: `mscqr-staging-ecs-execution-role`, `mscqr-staging-ecs-task-role`, `mscqr-staging-database-role-admin-task`, and `mscqr-staging-database-role-executor-broker-role`.
- [ ] Inline-policy actions (`iam:GetRolePolicy`, `iam:PutRolePolicy`, and `iam:DeleteRolePolicy`) are scoped to exactly those same four Terraform-managed roles.
- [ ] `iam:AttachRolePolicy` and `iam:DetachRolePolicy` remain restricted to `mscqr-staging-ecs-execution-role` and the reviewed `AmazonECSTaskExecutionRolePolicy`; neither task role nor the broker role is an attachment target.
- [ ] `mscqr-staging-database-role-admin-task` and `mscqr-staging-database-role-executor-broker-role` cannot receive arbitrary or AWS-managed policy attachments; their Terraform-managed permissions remain inline only.
- [ ] The apply role policy and permissions boundary contain the same exact four-role lifecycle and inline-policy scope; any missing or additional role blocks approval.
