...
- ASG bootstrap renders a temporary root/backend union env file and runs `docker compose --env-file "$compose_env_path" -f docker-compose.asg-web.yml ...` so Compose-required secrets such as `QR_SIGN_PRIVATE_KEY` are available at interpolation time. Values are never printed.
