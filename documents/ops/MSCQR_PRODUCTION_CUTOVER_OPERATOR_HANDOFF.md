...
Use the existing authenticated local AWS and GitHub operator session; do not run `aws configure` or place credentials in shell startup files. The launcher passes only the runtime environment needed for the governed child plus the five prompted inputs. It does not pre-collect any MFA code. The prepare-overlap child does not establish a release-deployer or verifier session. After the independently authorized overlap deployment, the verifier continuation consumes the already-established inherited MFA-backed ECS Exec verifier session and rejects release-deployer or unrelated callers; it does not prompt for a second verifier session. Onboarding or tenant-canary MFA remains separate: if live onboarding returns `MFA_BOOTSTRAP`, the existing just-in-time controlling-TTY provider prompts at that point and immediately submits the code.
