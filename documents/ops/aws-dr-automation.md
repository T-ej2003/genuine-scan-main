...
- Latest ASG health evidence: the following retry built backend/frontend images, created containers, started the backend, marked backend container health `Healthy`, started frontend/Nginx, and reached waits for `http://127.0.0.1/healthz` plus `http://127.0.0.1/api/health/ready`; console evidence showed local `frontend_healthz_ready: ok http_status=200`. Apply briefly reached `HEALTHY_TARGET_COUNT=2`, but later ASG and ALB evidence disagreed or flapped with ASG targets in `Target.FailedHealthChecks` on port 80. The previous evidence collection malformed multiple instance IDs into one AWS argument, so the collector now splits IDs with POSIX shell args, describes all current instances with separate `--instance-ids` args, loops console output per ID, curls every public IP `/healthz`, and optionally SSHes each public IP only with `ALLOW_SSH_DEEP_INSPECTION=true`. Bootstrap now hard-gates host-primary-IP `/healthz` after loopback `/healthz`, and also prints Docker frontend port mappings, localhost and host-primary-IP `/healthz`, and iptables/nft summaries.
