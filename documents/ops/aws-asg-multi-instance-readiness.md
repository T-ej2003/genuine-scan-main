...
- Root production Compose pins frontend nginx to `172.30.10.2` on its dedicated `172.30.10.0/28` bridge with explicit gateway `172.30.10.1`, reserves `172.30.10.8/29` for dynamic services, configures backend `CLIENT_IP_TRUST_MODE=nginx`, and trusts only the frontend `/32`. ASG supplies the equivalent reviewed subnet, gateway, dynamic range, and frontend address through its SSM contract. Both reject network, gateway, broadcast, dynamic-range, and out-of-subnet frontend addresses. Its restart-idempotent root-only entrypoint adapter replaces inbound `X-Forwarded-For` with the direct client address before the canonical nginx entrypoint runs, so another container or caller-supplied chain cannot select `req.ip`. ASG does not mount that adapter and retains the shared templates' `$proxy_add_x_forwarded_for` chain.
...
- ASG proxy identity is explicitly configured as CloudFront -> ALB -> fixed frontend nginx -> backend. The SSM manifest requires reviewed ALB and CloudFront CIDRs, an ASG application-network subnet, explicit gateway and dynamic allocation range that exclude the pinned frontend address, and the fixed frontend address; bootstrap validates containment/capacity and requires `CLIENT_IP_TRUSTED_NGINX_CIDRS` to be that frontend address as a `/32`. Backend `/health/live` accepts direct probes only from loopback or the configured ALB CIDRs; all other traffic must satisfy the viewer proxy chain.
...
- Backend container healthcheck calls backend `/health/live` so Nginx can start once the backend process is live.
- Frontend healthcheck calls Nginx `/healthz`.
- Nginx proxies `/api/health/*` to backend `/health/*`.
- Bootstrap treats loopback `/healthz`, host-primary-IP `/healthz`, and host port 80 as the hard edge-liveness gate. It records `/api/health/ready` as dependency-readiness evidence; if readiness stays degraded while both edge health probes are healthy, bootstrap logs `CONDITIONALLY_READY` diagnostics instead of killing the node whose ALB health path is live.
