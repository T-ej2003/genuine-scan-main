...
- Root production Compose pins frontend nginx to `172.30.10.2` on its dedicated `172.30.10.0/28` bridge, reserves `172.30.10.8/29` for dynamic services, configures backend `CLIENT_IP_TRUST_MODE=nginx`, and trusts only the frontend `/32`. Its restart-idempotent root-only entrypoint adapter replaces inbound `X-Forwarded-For` with the direct client address before the canonical nginx entrypoint runs, so another container or caller-supplied chain cannot select `req.ip`. ASG does not mount that adapter and retains the shared templates' `$proxy_add_x_forwarded_for` chain.
