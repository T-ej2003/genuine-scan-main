...
and read-only verification actions.
...
bootstrap role and its inline policy from
`infra/aws/terraform/production-green-stage-b-publisher-bootstrap/`. The saved plan must
have exactly two additions, no changes, destroys, or replacements. Record the root
...
bootstrap role and inline policy. It must not alter images, databases, secrets, ECS,
