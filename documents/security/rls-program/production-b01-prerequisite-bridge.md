...
This bridge preserves the immutable RLS origin `0f7ae1a70eec588ef4fdcb2b53e9f42e831c4414`, bridge merge `e1c16977e9fdce7a19dc267ba3c816ef4ff14597`, live-predecessor correction `69e71ab21c847d27f8a76795a6de6f623313c8ad`, and read-only runtime-evidence merge `a2bed229ddfb893d4dcc53232ea5466d23d4e80a`. The deployment source is the exact protected-main merge SHA of the narrow expired-task recovery correction that immediately follows them. Executable diff attestations cover every range and reject application, authentication, RLS, schema, MFA, session, invitation, business, or unclassified changes.
...
Before any future mutation launch, the executor entry point requires Amazon ECS native event capture for the production cluster, with at least 30 days retention and exactly one enabled rule targeting the wildcard-free CloudWatch Logs group ARN for `/aws/events/ecs/containerinsights/mscqr-prod-euw2-main/performance`. The rule must either capture all `aws.ecs` events or explicitly capture `ECS Task State Change`; deployment-only or additionally constrained patterns fail closed. It refuses to launch without that durable capture and refuses to issue a receipt until the exact task's authenticated `STOPPED` event is present. Event capture must be enabled as a separately authorized AWS control-plane prerequisite; this repository does not create a broker, database, Lambda, or state machine for it.
