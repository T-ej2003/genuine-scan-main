...
Before any future mutation launch, the executor entry point requires Amazon ECS native event capture for the production cluster, with at least 30 days retention and exactly one enabled `aws.ecs` rule target bound to `/aws/events/ecs/containerinsights/mscqr-prod-euw2-main/performance`. It refuses to launch without that durable capture and refuses to issue a receipt until the exact task's authenticated `STOPPED` event is present. Event capture must be enabled as a separately authorized AWS control-plane prerequisite; this repository does not create a broker, database, Lambda, or state machine for it.
