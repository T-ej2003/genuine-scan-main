...
`decision-policy-command-semantics` is resolved by `command-semantics.json`. `backend/src/lib/canonicalDbContext.ts` is the promoted application primitive: it validates the context shape and assurance vocabulary, rejects explicit blank values and unknown keys, parameterizes one `set_config(..., true)` statement, and runs the protected callback on that same interactive Prisma transaction client. Audit CSV export, fraud-report query and audit-log listing now reuse this primitive. Audit-log listing binds tenant administrators to their licensee, manufacturers to their actor plus one linked licensee, and platform administrators to fresh MFA plus one explicit licensee and purpose; count, list, name enrichment and read attribution share the installed transaction. Later slices reuse this primitive rather than adding another context implementation. PostgreSQL policy certification remains separate and pending.
...
The first automated pass records three reviewed HTTP workflows in two implemented families, 38 workflows in 18 contract-only families, and 387 workflows in 296 blocked families. No remaining ordinary family is auto-implementable yet: generated/unreviewed tenant scope is still present across the remaining inventory, and internal access sites do not prove a complete registered-root-to-repository transaction chain. Mutations additionally require lifecycle, replay and database concurrency evidence. These are explicit blocker IDs with remediation in the family manifest, not permission to infer authority from current Prisma filters.

Contract-only families remain behind their approved exact pre-auth function, worker/scheduled, operator or migration boundary. They must never be rewritten as authenticated HTTP context. Ordinary families may move to `auto-implementable` only after one registered root, human-reviewed scope, full protected-query trace, feasible same-client transaction, resolved lifecycle/concurrency semantics and focused allow/deny tests are recorded. A material difference splits the family; it never broadens a helper or permission.

Each family retains the change ceiling of 15 production files, 10 test files and 2,500 net changed lines. Implementation proceeds in dependency-safe groups: simple reads, bounded administration, append-only writes, simple mutations, lifecycle mutations, batch/QR, printing, then incident/governance. Every implemented workflow remains `context-boundary-implemented-postgresql-certification-pending` until disposable PostgreSQL proves the real policies, roles, context and denial cases. `CONTEXT_BOUNDARY_MIGRATION_REPORT.md` is the human review summary; the JSON family manifest remains authoritative.
