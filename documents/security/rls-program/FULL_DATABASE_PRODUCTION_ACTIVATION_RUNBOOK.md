...
5. Prepare the unsigned input from the authenticated current-source evidence
context. The producer accepts only the change-ticket ID from the operator;
issued time defaults to the current UTC time, expiry defaults to no more than
two hours after issue, and the nonce is generated by the producer. It derives
all fixed, image, source, task-definition, broker, and identity fields from the
authenticated evidence context and refuses stale, mismatched, incomplete, or
unknown provenance. It writes only the 0600 private artifact below; it does not
sign, approve, publish, or call AWS mutation APIs:

```sh
node scripts/aws/prepare-production-green-stage-b-approval-input.mjs \
  --evidence /secure/operator/production-rls-stage-b-authenticated-evidence.json \
  --evidence-sha256 <authenticated-evidence-sha256> \
...
The evidence context must already have been produced and authenticated by the
protected Stage-B evidence producers. Its provenance map is exact; the producer
does not treat a caller-supplied hash as proof of an artifact's authenticity.
The review text is deterministic for the completed input and contains no
