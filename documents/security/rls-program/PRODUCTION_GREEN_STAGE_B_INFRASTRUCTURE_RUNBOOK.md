...
4. Run the single reviewed refresh boundary with `npm run stage-b:refresh-only -- --closure-mode production ...`. It validates the canonical tfvars filename/content contract before Terraform and never writes a deployable plan. Do not assemble raw `terraform plan -refresh-only` commands.
