...
2. Confirm Stage A owns and has applied the executor/database security groups, exact database/interface-endpoint/S3/DNS rules, executor/broker log groups, executor/broker roles, approval resources, runtime-secret ARNs, and the canonical root-drop key and alias. Stage B release readiness regenerates this handoff with the `POST_APPLY` Stage A contract for every planning mode; absent or partial root-drop state fails closed. Write one explicit reviewed prerequisite file matching `infra/aws/terraform/production-green-stage-b/stage-a-prerequisites.schema.json`; do not copy values into tfvars by hand.
