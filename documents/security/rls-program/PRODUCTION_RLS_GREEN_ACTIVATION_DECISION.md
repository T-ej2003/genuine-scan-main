...
Terraform ownership is split. Stage A (`infra/aws/terraform/production-green-stage-a`) owns only new green RDS/KMS/security-group/empty-secret/role/log resources and receives VPC, subnet, and runtime security-group references as explicit inputs. It never owns or imports blue ECS, ECR, ALB, DNS, RDS, or existing secrets. Its S3 backend coordinates are external and fail closed when missing; the state bucket must be dedicated, encrypted, versioned, ownership-enforced, private, and distinct from staging and the artifact bucket. AWS root must not plan or apply. Stage B is a release-only contract requiring the externally created `mscqr-production-release-deployer` role, a distinct independent checker, immutable images, and the Stage A outputs before any broker/task/canary action. Checker MFA is enforced at the exact checker IAM user to `mscqr-production-independent-checker` trust boundary; that role alone may assume the exact `mscqr-production-rls-independent-checker` signer role. The role-to-role trust does not require a second MFA condition because AWS role chaining does not create a fresh MFA request. The signer session ARN remains bound into the signed approval.

The immediate Stage A operator path is `arn:aws:iam::368992683803:user/mscqr-production-bootstrap-operator` with MFA into `arn:aws:iam::368992683803:role/mscqr-production-release-deployer`. The same bootstrap user may separately use MFA to assume `arn:aws:iam::368992683803:role/mscqr-production-ecs-exec-verifier` for independent ECS Exec verification, and the pre-existing `arn:aws:iam::368992683803:role/mscqr-production-stage-b-publisher-bootstrap` for its separate publisher-bootstrap lifecycle; it receives none of either role's operational permissions directly. These are its only three exact MFA-gated `sts:AssumeRole` grants. The bootstrap operator has no console password or standing access key; a temporary key may exist solely to obtain that MFA-backed STS session and is deleted before Terraform. The release role has a one-hour maximum session and cannot be assumed by root, either checker role, `github-actions-mscqr-deploy`, or application/runtime roles. This change adds no GitHub OIDC trust; a protected GitHub operator path requires separate review.

Stage A explicitly revokes executor egress. It retains only security-group-to-security-group TCP/5432 database ingress from the future executor and approved runtime security group. Before any executor task exists, Stage B must separately review either NAT TCP/443 plus green-DB TCP/5432 or approved VPC-endpoint TCP/443 plus green-DB TCP/5432. `manage_master_user_password = true` means RDS creates the KMS-encrypted administrator secret at database creation; it is distinct from the 14 empty application/runtime handles. Terraform outputs only its ARN, application runtime roles cannot read it, and secret values never enter plans, logs, Git, or receipts.
