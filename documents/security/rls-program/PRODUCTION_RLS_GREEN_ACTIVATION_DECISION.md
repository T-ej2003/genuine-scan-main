...
The immediate Stage A operator path is `arn:aws:iam::368992683803:user/mscqr-production-bootstrap-operator` with MFA into `arn:aws:iam::368992683803:role/mscqr-production-release-deployer`. The bootstrap operator has no console password, no standing access key, and only MFA-gated `sts:AssumeRole`; a temporary key may exist solely to obtain that MFA-backed STS session and is deleted before Terraform. The release role has a one-hour maximum session and cannot be assumed by root, either checker role, `github-actions-mscqr-deploy`, or application/runtime roles. This change adds no GitHub OIDC trust; a protected GitHub operator path requires separate review.
