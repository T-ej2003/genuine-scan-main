...
const bulkDeleteQRCodesSchema = zod_1.z
    ...
    codes: zod_1.z.array(zod_1.z.string().min(1)).optional(),
})
...
const buildPublicQrUrl = (code, baseUrl) => {
    const base = String(baseUrl || "").trim().replace(/\/+$/, "");
    if (base)
        return `${base}/verify/${encodeURIComponent(code)}`;
    return (0, qrService_1.buildVerifyUrl)(code);
};
const escapeCsv = (v) => {
...
const allocateQRRangeForLicensee = async (req, res) => {
    try {
        const auth = ensureAuth(req);
        if (!auth)
            return res.status(401).json({ success: false, error: "Not authenticated" });
...
/* ===================== QR CODES DELETE ===================== */
const bulkDeleteQRCodes = async (req, res) => {
    try {
        const auth = ensureAuth(req);
        if (!auth)
            return res.status(401).json({ success: false, error: "Not authenticated" });
        ...
        const deleted = await database_1.default.qRCode.deleteMany({ where });
        await (0, auditService_1.createAuditLog)({
            userId: auth.userId,
...
exports.bulkDeleteQRCodes = bulkDeleteQRCodes;
...
const createBatch = async (req, res) => {
    try {
        ...
        let mfgId = null;
        if (manufacturerId) {
            const m = await database_1.default.user.findFirst({
                where: {
                    id: manufacturerId,
        ...
        const batch = await database_1.default.$transaction(async (tx) => {
            const pool = await tx.qRCode.findMany({
                where: { licenseeId, batchId: null, status: client_1.QRStatus.DORMANT },
                orderBy: { code: "asc" },
                take: quantity,
...
/* ===================== BULK DELETE BATCHES ===================== */
const bulkDeleteBatches = async (req, res) => {
    try {
        const auth = ensureAuth(req);
        if (!auth)
            return res.status(401).json({ success: false, error: "Not authenticated" });
        ...
        if (auth.role === client_1.UserRole.LICENSEE_ADMIN || auth.role === client_1.UserRole.ORG_ADMIN) {
            ...
            if (!licId)
                return res.status(403).json({ success: false, error: "No licensee association" });
            const bad = await database_1.default.batch.findFirst({
                where: { id: { in: batchIds }, licenseeId: { not: licId } },
                select: { id: true },
            });
            if (bad)
                return res.status(403).json({ success: false, error: "Some batches are outside your tenant" });
        }
        // disallow deleting printed batches
        const printed = await database_1.default.batch.findFirst({
...
exports.bulkDeleteBatches = bulkDeleteBatches;
/* ===================== MANUFACTURER ASSIGN ===================== */
const assignManufacturer = async (req, res) => {
    try {
        const auth = ensureAuth(req);
        if (!auth)
            return res.status(401).json({ success: false, error: "Not authenticated" });
        ...
        const result = await database_1.default.$transaction(async (tx) => {
            ...
            if (eligible.length < quantity) {
                throw new Error(`Not enough available codes in this batch. Available: ${eligible.length}, requested: ${quantity}.`);
            }
...
const markPrinted = async (req, res) => {
    try {
        ...
        if (!qr?.batch)
            return res.status(404).json({ success: false, error: "QR or batch not found" });
        const count = await (0, qrService_1.markBatchAsPrinted)(qr.batch.id, req.user.userId);
...
const confirmBatchPrint = async (req, res) => {
    try {
        const batchId = req.params.id;
        ...
        if (!batch)
            return res.status(404).json({ success: false, error: "Batch not found" });
        const count = await (0, qrService_1.markBatchAsPrinted)(batch.id, req.user.userId);
        return res.json({ success: true, data: { count } });
    }
    catch (e) {
        return res.status(400).json({ success: false, error: e.message || "Bad request" });
...
exports.confirmBatchPrint = confirmBatchPrint;
/* ===================== MANUFACTURER PRINT PACK (BATCH) ===================== */
const createBatchPrintToken = async (req, res) => {
    try {
        const userId = req.user?.userId;
        if (!userId)
            return res.status(401).json({ success: false, error: "Not authenticated" });
...
exports.createBatchPrintToken = createBatchPrintToken;
const downloadBatchPrintPack = async (req, res) => {
    try {
        const userId = req.user?.userId;
        if (!userId)
            return res.status(401).json({ success: false, error: "Not authenticated" });
        ...
        const now = new Date();
        const tokenRow = await database_1.default.batchPrintPackToken.findUnique({
            where: { tokenHash },
            include: {
        ...
        const markUsed = await database_1.default.batchPrintPackToken.updateMany({
...
const getQRCodes = async (req, res) => {
    try {
        ...
        const licenseeId = role === client_1.UserRole.SUPER_ADMIN || role === client_1.UserRole.PLATFORM_SUPER_ADMIN
...
const exportQRCodesCsv = async (req, res) => {
    try {
        const role = req.user?.role;
        const userId = req.user?.userId;
        if (!role || !userId)
...
exports.exportQRCodesCsv = exportQRCodesCsv;
