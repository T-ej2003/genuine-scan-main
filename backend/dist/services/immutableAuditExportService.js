...
exports.buildImmutableBatchAuditPackage = void 0;
...
const sha256Hex = (v) => (0, crypto_1.createHash)("sha256").update(v).digest("hex");
const normalizePem = (value) => value.replace(/\\n/g, "\n").trim();
const signIntegrityPayload = (payload) => {
    const payloadHash = (0, crypto_1.createHash)("sha256").update(payload).digest();
    const privateKeyPem = process.env.QR_SIGN_PRIVATE_KEY;
    if (privateKeyPem) {
        const key = (0, crypto_1.createPrivateKey)(normalizePem(privateKeyPem));
        const signature = (0, crypto_1.sign)(null, payloadHash, key);
    ...
    const signature = (0, crypto_1.createHmac)("sha256", secret || "genuine-scan-fallback-signing-key")
        .update(payloadHash)
        .digest();
...
const buildImmutableBatchAuditPackage = async (batchId) => {
    ...
    const traceEventsJson = JSON.stringify(traceEvents.map((e) => ({
        id: e.id,
    ...
    const policyAlertsJson = JSON.stringify(policyAlerts.map((a) => ({
        id: a.id,
    ...
    files["integrity.json"] = JSON.stringify({
        ...
        signature,
        verification: {
            note: "Recompute SHA-256 for each file and compare with fileHashes, then verify integrityHash and signature.",
        },
    }, null, 2);
    files["README.txt"] = [
...
exports.buildImmutableBatchAuditPackage = buildImmutableBatchAuditPackage;
//# sourceMappingURL=immutableAuditExportService.js.map