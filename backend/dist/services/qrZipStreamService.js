...
exports.streamQrZipToResponse = exports.resolveQrZipProfile = void 0;
...
const CPU_COUNT = Math.max(1, os_1.default.cpus().length);
...
const safeCodeFilePart = (code) => String(code || "")
    .trim()
    .replace(/[^\w.-]+/g, "_")
    .replace(/^_+|_+$/g, "")
    .slice(0, 180) || "qr";
const renderPng = (url, width) => qrcode_1.default.toBuffer(url, {
    width,
    margin: 2,
...
const resolveQrZipProfile = (totalCount) => {
...
exports.resolveQrZipProfile = resolveQrZipProfile;
const streamQrZipToResponse = async ({ res, fileName, totalCount, manifestHeader, entries, profile: explicitProfile, }) => {
    const profile = explicitProfile || (0, exports.resolveQrZipProfile)(totalCount);
    ...
    const manifestStream = new stream_1.PassThrough();
    archive.pipe(res);
    ...
    const waitForCompletion = new Promise((resolve, reject) => {
        ...
        const onWarning = (err) => {
            if (err?.code !== "ENOENT")
                complete(err instanceof Error ? err : new Error(String(err || "Archive warning")));
        };
        const onFinish = () => complete();
        const onClose = () => {
            if (!res.writableFinished)
                complete(new Error("Client disconnected during ZIP download"));
        };
    ...
    const flushBatch = async (batch) => {
        if (batch.length === 0)
            return;
        const rendered = await Promise.all(batch.map(async (entry) => {
    ...
    try {
        ...
        manifestStream.destroy(error);
        archive.abort();
        throw error;
    }
    return profile;
};
exports.streamQrZipToResponse = streamQrZipToResponse;
//# sourceMappingURL=qrZipStreamService.js.map