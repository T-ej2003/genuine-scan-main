...
import { createInvite } from "../src/services/auth/inviteService";
import { normalizeEmailAddress } from "../src/utils/email";
import { maskEmailForLog } from "../src/services/mailTransportService";
...
const safeUser = (user: { id: string; email: string; role: UserRole; status: UserStatus; passwordHash?: string | null } | null) =>
  user
    ? {
        id: user.id,
...
const ensureVictoriaPlatformAdminInvite = async (actorUserId: string) => {
...
const main = async () => {
  ...
  const victoriaResult = isApply && admin?.id
