...
import { assertSafeTestDatabaseUrl } from "../helpers/testDbSafetyGuard";

const seedDatabaseUrl = String(process.env.E2E_SEED_DATABASE_URL || "").trim();
assertSafeTestDatabaseUrl(seedDatabaseUrl);
...
const backupCodes = (envName: string) => String(process.env[envName] || "")
...
const seedMfaBackupCodes = async (userId: string, envName: string) => {
  const codes = backupCodes(envName);
  if (codes.length === 0) {
    throw new Error(`${envName} is required for enterprise MFA smoke.`);
  }
  for (const code of codes) {
    if (!backupCodeShapeOk(code)) {
      throw new Error(`${envName} contains an invalid backup-code shape.`);
    }
  }
...
const resetCodesForBatch = async (codes: string[], batchId: string, status: QRStatus) => {
  await prisma.qRCode.createMany({
    data: codes.map((code) => ({
      code,
      displayCode: code.replace(/^E2E/, "E2E-"),
  ...
  await Promise.all(codes.map((code) => prisma.qRCode.update({
    where: { code },
    data: { displayCode: code.replace(/^E2E/, "E2E-") },
  })));
...
const seed = async () => {
  ...
  await seedMfaBackupCodes(IDS.superAdmin, "E2E_SUPERADMIN_MFA_BACKUP_CODES");
  await seedMfaBackupCodes(IDS.licenseeAdmin, "E2E_LICENSEE_ADMIN_MFA_BACKUP_CODES");
  await seedMfaBackupCodes(IDS.manufacturer, "E2E_MANUFACTURER_MFA_BACKUP_CODES");
