...
const run = () => {
  ...
  assert(latest.latestVersion === "2026.5.19", "Latest connector version should be the signed 2026.5.19 build");
  ...
  assert(
    latest.release.platforms.windows.signedAt === latestManifestWindows.signedAt,
  ...
  assert(
    latest.release.platforms.windows.buildVersion === manifest.latestVersion,
  ...
  assert(windowsPackage.bytes === latestManifestWindows.bytes, "Windows package bytes should match the published signed installer");
