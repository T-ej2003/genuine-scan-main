...
async function main() {
  ...
  assert.equal(last(preauth, `BEGIN; SELECT id FROM app_auth.lookup_password_user('auth-a@example.invalid'); SELECT "recorded" FROM app_rls.record_auth_session_risk_signal(90,'CRITICAL',ARRAY['UNTRUSTED_NETWORK'],NULL,NULL,transaction_timestamp()::timestamp,NULL,NULL,NULL,NULL,NULL,true,'blocked-temporary-role-risk'); COMMIT`), "t");
  assert.equal(last(bootstrap, `SELECT count(*) FROM public."AuditLogOutbox" WHERE payload->>'action'='AUTH_LOGIN_BLOCKED_RISK'`), String(Number(blockedRiskCount) + 1));
