...
const toMs = (input?: Date | string | null) => {
  if (!input) return NaN;
...
const isLocationConsistent = (current: ScanIdentitySnapshot, latest?: ScanIdentitySnapshot | null) => {
  if (!latest) return true;

  const dist = distanceKm(current, latest);
  if (dist != null) return dist <= 250;

  const cCountry = safeLower(current.locationCountry);
  const lCountry = safeLower(latest.locationCountry);
  if (!cCountry || !lCountry) return true;
  return cCountry === lCountry;
};

const detectImpossibleTravel = (current: ScanIdentitySnapshot, history: ScanIdentitySnapshot[]) => {
...
const detectBurstyPattern = (current: ScanIdentitySnapshot, history: ScanIdentitySnapshot[]) => {
  const currentTs = toMs(current.scannedAt);
  if (!Number.isFinite(currentTs)) return false;
...
export const classifyScan = (
  ...
  const distinctVisitorIds = new Set(
    history
      .map((s) => String(s.anonVisitorId || "").trim())
      .filter(Boolean)
  );

  if (currentUserId) distinctCustomerIds.add(currentUserId);
  if (currentAnonId) distinctVisitorIds.add(currentAnonId);

  const verifiedByYouCount = history.filter((entry) => {
    if (currentUserId && entry.customerUserId === currentUserId) return true;
    if (!currentUserId && currentAnonId && entry.anonVisitorId === currentAnonId) return true;
    return false;
  }).length;
  ...
  if (ownerMatch) reasons.push("Matched claimed owner account");
  if (seenUserBefore && !ownerMatch) reasons.push("Matched previous account activity");
  if (seenAnonBefore && locationConsistent) reasons.push("Matched previous device pattern");

  const hasDifferentAccount = Boolean(
    currentUserId && history.some((entry) => entry.customerUserId && entry.customerUserId !== currentUserId)
  );

  if (ownerConflict) reasons.push("Different account than claimed owner");
  if (hasDifferentAccount) reasons.push("Different account seen on this code");
  if (distinctCustomerIds.size > 1) reasons.push("Multiple customer accounts detected");
  if (distinctVisitorIds.size > 2) reasons.push("Multiple devices detected");

  if (!locationConsistent && seenAnonBefore) reasons.push("Device location changed unusually");

  const impossibleTravel = detectImpossibleTravel(context, history);
  if (impossibleTravel) reasons.push("Impossible travel pattern");

  const bursty = detectBurstyPattern(context, history);
  if (bursty) reasons.push("High scan frequency in a short time");

  const legitRepeat = ownerMatch || seenUserBefore || (seenAnonBefore && locationConsistent);

  const hardSuspicious =
    ownerConflict ||
    hasDifferentAccount ||
  ...
  if (legitRepeat && !hardSuspicious) {
    return {
      classification: ScanRiskClassification.LEGIT_REPEAT,
  ...
  if (hardSuspicious || fallbackSuspicious) {
    return {
      classification: ScanRiskClassification.SUSPICIOUS_DUPLICATE,
