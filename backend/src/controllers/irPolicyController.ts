...
export const listIrPolicies = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (!z.string().uuid().safeParse(licenseeId).success) {
    ...
    const result = await withC03ActorTransaction(
      ...
      async (tx, context) => {
        const data = await listPolicyRulesInTransaction<{ rules: any[]; total: number }>(tx, {
      ...
      async (tx, context) => {
        ...
        if (!__c03Replay) {
          await createAuditLogInTransaction(tx, context, {
            ...
            entityType: "PolicyRule",
            entityId: result.id,
            details: { name: result.name, ruleType: result.ruleType },
            ipAddress: req.ip,
          });
        }
        return result;
