...
function assertTaskDefinitionAppendOnlyPlan(plan, terraformConfiguration, { strict = true } = {}) {
  ...
  for (const change of current) {
    ...
    if (change.address === STAGE_B_IMPORTED_BACKEND_CANDIDATE_ADDRESS && exactActions(change.change?.actions || [], ["update"])) {
      assertStageBImportedBackendMetadataNormalization(change, { terraformConfiguration });
      currentArnByAddress.set(change.address, change.change.after.arn);
      continue;
    }
  ...
  const currentNoOps = current.filter((change) => exactActions(change.change?.actions || [], ["no-op"]) || (change.address === STAGE_B_IMPORTED_BACKEND_CANDIDATE_ADDRESS && exactActions(change.change?.actions || [], ["update"]))).length;
  ...
  const currentEntries = current.map((change) => ({
      ...
      classification: exactActions(change.change?.actions || [], ["create"]) ? "create-only" : exactActions(change.change?.actions || [], ["no-op"]) || (change.address === STAGE_B_IMPORTED_BACKEND_CANDIDATE_ADDRESS && exactActions(change.change?.actions || [], ["update"])) ? "no-op" : "rollover",
      priorTaskDefinitionArn: (exactActions(change.change?.actions || [], ["no-op"]) || (change.address === STAGE_B_IMPORTED_BACKEND_CANDIDATE_ADDRESS && exactActions(change.change?.actions || [], ["update"]))) ? currentArnByAddress.get(change.address) : currentRotations.find((entry) => entry.address === change.address)?.oldArn || null,
    }));
...
function assertRecoveryOnlyReferenceAuditBinding(plan, audit, auditBytes, auditSha256, planBytes, planSha256, trustedCallerArn, now) {
  if (!audit || !auditBytes || !auditSha256 || !planBytes || !planSha256) throw new Error("Recovery-only plan requires an explicit plan-bound reference audit.");
  if (sha256(auditBytes) !== auditSha256 || sha256(planBytes) !== planSha256) throw new Error("Recovery-only reference audit or plan SHA-256 mismatch.");
  if (audit.schemaVersion !== STAGE_B_REFERENCE_AUDIT_SCHEMA_VERSION || audit.recoveryOnly !== true || audit.planJsonSha256 !== planSha256) throw new Error("Recovery-only reference audit is missing its exact plan binding.");
  try { assertStageBReleaseCallerArn(audit.callerArn); } catch { throw new Error("Recovery-only reference audit caller identity is unauthorized."); }
  if (trustedCallerArn !== undefined && audit.callerArn !== trustedCallerArn) throw new Error("Recovery-only reference audit caller identity is not trusted.");
  assertStageBReferenceAuditFreshness(audit.auditedAt, now);
  const alias = (plan.resource_changes || []).find((change) => change.address === STAGE_B_PARTIAL_APPLY_RECOVERY_ADDRESS);
  const mutations = (plan.resource_changes || []).filter((change) => !exactActions(change.change?.actions || [], ["no-op"]));
  if (!alias || mutations.length !== 1 || mutations[0] !== alias || !exactActions(alias.change?.actions || [], ["update"])) throw new Error("Recovery-only plan must contain exactly one reviewed-alias update.");
  if (audit.recoveryAlias?.address !== alias.address
    || audit.recoveryAlias.liveVersion !== alias.change?.before?.function_version
    || audit.recoveryAlias.targetVersion !== alias.change?.after?.function_version
    || audit.recoveryAlias.aliasArn !== STAGE_B.brokerAliasArn
    || audit.recoveryAlias.aliasName !== STAGE_B.brokerAliasQualifier) throw new Error("Recovery-only reference audit alias evidence does not match the plan.");
  const noOpAddresses = new Set((audit.recoveryNoOpResources || []).map((entry) => entry?.address));
  const expectedNoOps = (plan.resource_changes || []).filter((change) => change !== alias).map((change) => change.address);
  if (noOpAddresses.size !== expectedNoOps.length || expectedNoOps.some((address) => !noOpAddresses.has(address))) throw new Error("Recovery-only reference audit does not cover every non-alias resource as no-op.");
  const broker = assertStageBBrokerConfigurationIdentity({
    configuration: { FunctionArn: audit.broker?.configurationFunctionArn, Version: audit.broker?.configurationVersion },
    alias: { AliasArn: audit.broker?.aliasArn, Name: audit.broker?.aliasName, FunctionVersion: audit.broker?.aliasFunctionVersion },
  });
  if (audit.broker.resolvedVersionArn !== broker.resolvedVersionArn || audit.broker.aliasFunctionVersion !== alias.change?.before?.function_version) throw new Error("Recovery-only reference audit broker identity is not bound to the live alias.");
  return true;
}

export function assertStageBPlan(plan, options = {}) {
  const { referenceAudit, referenceAuditBytes, referenceAuditSha256, planJsonBytes, planJsonSha256, trustedCallerArn, now = new Date(), terraformConfiguration, imageEvidence, strictResourceContract = false, protectedMainCheckout, terraformWorkspace, requireReferenceAudit = true, captureMode = false, requireSemanticCompleteness = false, recoveryOnly = false, partialApplyRecovery = false, freshImagePartialApplyRecovery = false } = options;
  ...
  const resourceClassification = classifyStageBPlan(plan, { strict: strictResourceContract, terraformConfiguration, allowBrokerPolicyCreate: captureMode, partialApplyRecovery, freshImagePartialApplyRecovery });
  const planSemanticCensus = requireSemanticCompleteness ? assertStageBPlanSemanticCompleteness(plan, { terraformConfiguration, partialApplyRecovery, freshImagePartialApplyRecovery }) : undefined;
  ...
  if (recoveryOnly) {
    ...
    if (!captureMode && requireReferenceAudit) assertRecoveryOnlyReferenceAuditBinding(plan, referenceAudit, referenceAuditBytes, referenceAuditSha256, planJsonBytes, planJsonSha256, trustedCallerArn, now);
  }
  const brokerChange = (plan.resource_changes || []).find((change) => change.address === "aws_lambda_function.broker");
  ...
  if (brokerMutation && !recoveryOnly && !partialApplyRecovery && !freshImagePartialApplyRecovery) assertStageBBrokerTaskDefinitionMapping(plan, terraformConfiguration);
  const brokerActionShape = captureMode && !recoveryOnly && !partialApplyRecovery && !freshImagePartialApplyRecovery ? classifyStageBBrokerActionShape(plan) : undefined;
  let brokerCapture;
  if (captureMode) {
    if (recoveryOnly) {
    ...
    } else if (freshImagePartialApplyRecovery) {
      brokerCapture = { brokerOperation: "fresh-image-partial-apply-recovery", brokerUpdatePresent: true, brokerActions: ["update"], brokerResourceAddresses: ["aws_iam_policy.broker", "aws_lambda_alias.reviewed", "aws_lambda_function.broker"], brokerReferenceValidationPending: true };
  ...
  if (brokerChange) {
    ...
    else if (freshImagePartialApplyRecovery && exactActions(brokerActions, ["update"])) assertStageBFreshImagePartialApplyRecoveryPlan(plan, { terraformConfiguration });
    else if (partialApplyRecovery && exactActions(brokerActions, ["update"])) assertStageBPartialApplyRecoveryPlan(plan);
    else if (!recoveryOnly && !partialApplyRecovery && !freshImagePartialApplyRecovery && exactActions(brokerActions, ["update"]) && requireReferenceAudit) assertBrokerAuditBinding(plan, brokerChange, referenceAudit, referenceAuditBytes, referenceAuditSha256, planJsonBytes, planJsonSha256, now, terraformConfiguration);
  ...
  const taskDefinitionChanges = (plan.resource_changes || []).filter((change) => change.type === "aws_ecs_task_definition");
  const taskDefinitionClassification = taskDefinitionChanges.length && !recoveryOnly && !partialApplyRecovery && !freshImagePartialApplyRecovery
  ...
  for (const change of plan.resource_changes || []) {
    ...
    if (change.type === "aws_ecs_task_definition") {
      if (retainedTaskDefinitionDescriptor(change.address)) {
        ...
        assertTaskDefinitionScope(change);
        if (!exactActions(actions, ["create"]) && !exactActions(actions, ["no-op"]) && !((partialApplyRecovery || freshImagePartialApplyRecovery) && isStageBPartialApplyDeposedTaskDefinitionCleanup(change)) && !(change.address === STAGE_B_IMPORTED_BACKEND_CANDIDATE_ADDRESS && exactActions(actions, ["update"])) && !isStageBTaskDefinitionRotationActionsValue(actions)) throw new Error(`Stage B task-definition plan rejected: ${change.address}`);
      }
    } else if (actions.includes("delete") && !((partialApplyRecovery || freshImagePartialApplyRecovery) && isStageBPartialApplyDeposedTaskDefinitionCleanup(change))) {
  ...
  return { taskDefinitions: taskDefinitionClassification, deploymentIdentity, imageBindings, brokerCapture, planSemanticCensus, ...resourceClassification };
}
...
export function assertStageBPlanCapture(plan, options = {}) {
  ...
  if (options.partialApplyRecovery && !options.freshImagePartialApplyRecovery && (result.actionCounts?.destroy !== 11 || result.actionCounts?.update !== 2 || result.unclassifiedResources?.length !== 0)) throw new Error("Stage B partial-apply recovery plan capture census is not exact.");
  if (options.freshImagePartialApplyRecovery && (result.actionCounts?.replacement !== 12 || result.actionCounts?.destroy !== 11 || result.actionCounts?.update !== 3 || result.unclassifiedResources?.length !== 0)) throw new Error("Fresh-image partial-apply recovery plan capture census is not exact.");
  return result;
}
...
export function readPlanningInputs(tfvars, cliOptions, protectedMainCheckout, deps = {}) {
  ...
  const refreshBindingReportPath = readOption(cliOptions, "--refresh-binding-report");
  const refreshBindingReportSha256 = readOption(cliOptions, "--refresh-binding-report-sha256");
  ...
  const recoveryAttestationSha256 = readOption(cliOptions, "--recovery-attestation-sha256");
  ...
  const freshImagePartialApplyRecovery = cliOptions.includes("--fresh-image-partial-apply-recovery");
  const closureMode = readOption(cliOptions, "--closure-mode");
  if ([recoveryOnly, partialApplyRecovery, freshImagePartialApplyRecovery].filter(Boolean).length > 1 || (freshImagePartialApplyRecovery && !partialApplyRecovery)) throw new Error("Stage B recovery modes are mutually exclusive and fresh-image recovery requires partial-apply recovery.");
  ...
  if ((recoveryOnly || partialApplyRecovery || freshImagePartialApplyRecovery) && (!refreshBindingReportPath || !refreshBindingReportSha256)) throw new Error("Stage B recovery planning requires the original observation binding report and SHA256.");
  ...
  if (Boolean(bindingReport.partialApplyRecovery) !== partialApplyRecovery || Boolean(bindingReport.freshImagePartialApplyRecovery) !== freshImagePartialApplyRecovery || ((partialApplyRecovery || freshImagePartialApplyRecovery) && bindingReport.recoveryMode !== (freshImagePartialApplyRecovery ? "FRESH_IMAGE_PARTIAL_APPLY_RECOVERY" : "PARTIAL_APPLY_RECOVERY"))) throw new Error("Stage B recovery mode must exactly match the canonical tfvars binding report.");
  ...
  if (refreshReport.status === "RESOURCE_DRIFT") {
    ...
    if ((partialApplyRecovery || freshImagePartialApplyRecovery) && (recoveryClassificationPath || recoveryAttestationSha256)) throw new Error("Partial-apply recovery profiles do not accept RECOVERY_ALIAS_ONLY attestation inputs.");
    ...
    if (recoveryOnly) {
      ...
      assertStageBRecoveryProvenance({ refreshReport, refreshReportSha256, observationBindingReport: refreshBindingReport, observationBindingReportSha256: refreshBindingReportSha256, recoveryBindingReport: bindingReport, recoveryBindingReportSha256: bindingReportSha256, recoveryClassificationSha256: recoveryClassificationSha256, recoveryAttestationSha256 });
    } else if (partialApplyRecovery) {
      ...
      assertStageBRecoveryProvenance({ refreshReport, refreshReportSha256, observationBindingReport: refreshBindingReport, observationBindingReportSha256: refreshBindingReportSha256, recoveryBindingReport: bindingReport, recoveryBindingReportSha256: bindingReportSha256, recoveryMode: freshImagePartialApplyRecovery ? "FRESH_IMAGE_PARTIAL_APPLY_RECOVERY" : "PARTIAL_APPLY_RECOVERY" });
    }
  ...
  } else if (recoveryAttestationSha256 || recoveryClassificationPath || recoveryOnly || partialApplyRecovery || freshImagePartialApplyRecovery) throw new Error("Recovery planning requires the exact reviewed RESOURCE_DRIFT refresh evidence.");
  ...
  return { bindingReport, backendMetadata, bindingReportPath, bindingReportSha256, refreshBindingReportPath, refreshBindingReportSha256, toolingTreeSha256, refreshReportPath, refreshReportSha256, recoveryOnly, partialApplyRecovery, freshImagePartialApplyRecovery, recoveryAttestationSha256, recoveryAttestationReportPath, recoveryAttestationSignaturePath, recoveryAttestationSignatureSha256, recoveryClassificationPath, recoveryClassificationSha256, expectedImageReleaseSha };
}
...
export function captureStageBPlan({ tfvars, cliOptions, protectedMainCheckout = readStageBProtectedMainCheckout({ cwd: process.cwd(), fetchOriginMain: true }), plan = () => execFileSync("terraform", [`-chdir=${root}`, "plan", `-var-file=${tfvars}`, `-out=${readOption(cliOptions, "--saved-plan")}`], { stdio: "inherit" }), show = (savedPlanPath) => captureStageBTerraformJson({ args: [`-chdir=${root}`, "show", "-json", savedPlanPath], cwd: process.cwd() }), readInputs = readPlanningInputs, showWorkspace } = {}) {
  ...
  const { workspace } = runStageBTerraformPlanCommand({ argv: [tfvars, ...cliOptions], showWorkspace, plan: () => plan() });
  ...
  const classification = assertStageBPlanCapture(parsedPlan, { terraformConfiguration: fs.readFileSync(path.resolve(root, "main.tf"), "utf8"), strictResourceContract: true, requireSemanticCompleteness: true, recoveryOnly: inputs.recoveryOnly, partialApplyRecovery: inputs.partialApplyRecovery, freshImagePartialApplyRecovery: inputs.freshImagePartialApplyRecovery, protectedMainCheckout, terraformWorkspace: { envWorkspace: process.env.TF_WORKSPACE, observedWorkspace: workspace } });
...
export function recoverCapturedStageBPlan({ tfvars, cliOptions, protectedMainCheckout = readStageBProtectedMainCheckout({ cwd: process.cwd(), fetchOriginMain: true }), readInputs = readPlanningInputs } = {}) {
  ...
  if (!savedPlanPath || !planJsonPath || !canonicalPlanJsonPath || !captureReportPath || Object.entries(expected).some(([name, value]) => name !== "recoveryAttestationSha256" && !value) || ((!partialApplyRecovery && !freshImagePartialApplyRecovery) && !expected.recoveryAttestationSha256)) throw new Error("Stage B captured-plan recovery requires exact plan hashes and Stage B identity bindings.");
  ...
  if (inputs.bindingReport.stateLineage !== expected.stageBLineage || inputs.bindingReport.stateSerial !== expected.stageBSerial) throw new Error("Stage B preserved plan state identity does not match the authoritative binding.");
  const plan = JSON.parse(planJsonBytes);
  const classification = assertStageBPlanCapture(plan, { terraformConfiguration: fs.readFileSync(path.resolve(root, "main.tf"), "utf8"), strictResourceContract: true, requireSemanticCompleteness: true, recoveryOnly: inputs.recoveryOnly, partialApplyRecovery: inputs.partialApplyRecovery, freshImagePartialApplyRecovery: inputs.freshImagePartialApplyRecovery, protectedMainCheckout, terraformWorkspace: { envWorkspace: process.env.TF_WORKSPACE, observedWorkspace: process.env.TF_WORKSPACE } });
  ...
  if (inputs.recoveryAttestationSha256) { const recoveryReport = JSON.parse(fs.readFileSync(inputs.recoveryAttestationReportPath, "utf8")); if (inputs.recoveryOnly) assertRecoveryOnlyPlan(plan, recoveryReport); else assertRecoveryPlanDelta(plan, recoveryReport); }
  const report = createStageBPlanCaptureReport({ toolingSha: protectedMainCheckout.currentHead, toolingTreeSha256: inputs.toolingTreeSha256, refreshReportSha256: inputs.refreshReportSha256, refreshBindingReportSha256: inputs.refreshBindingReportSha256, recoveryAttestationSha256: inputs.recoveryAttestationSha256, hashes, capturedAt: new Date().toISOString(), stageBLineage: inputs.bindingReport.stateLineage, stageBSerial: inputs.bindingReport.stateSerial, terraformVersion: plan.terraform_version, terraformFormatVersion: plan.format_version, planExitCode: 2, showExitCode: 0, planProfile: inputs.recoveryOnly ? "RECOVERY_ALIAS_ONLY" : inputs.freshImagePartialApplyRecovery ? "FRESH_IMAGE_PARTIAL_APPLY_RECOVERY" : inputs.partialApplyRecovery ? "PARTIAL_APPLY_RECOVERY" : classification.planProfile, taskDefinitionRotations: classification.taskDefinitionRotations, classification: { noOp: classification.actionCounts["no-op"] || 0, create: classification.actionCounts.create || 0, update: classification.actionCounts.update || 0, destroy: classification.actionCounts.destroy || 0, replacement: classification.actionCounts.replacement || 0, unclassified: classification.unclassifiedResources.length }, brokerEvidence: classification.brokerCapture });
...
export function readStageBApprovalPlanArtifacts({ savedPlanPath, planJsonPath, canonicalPlanJsonPath }) {
  return {
    savedPlanBytes: fs.readFileSync(savedPlanPath),
    planJsonBytes: fs.readFileSync(planJsonPath),
    canonicalPlanJsonBytes: fs.readFileSync(canonicalPlanJsonPath),
  };
}

export function finalizeCapturedStageBPlanApproval({ approval, approvalReportPath, repositoryRoot, captureReport, captureReportBytes, referenceAudit, referenceAuditBytes, plan, hashes, logicalCanonicalPlanJsonSha256, referenceAuditSha256, trustedCallerArn, stageBLineage, stageBSerial, savedPlanPath, planJsonPath, canonicalPlanJsonPath, terraformConfiguration, publish = writeStageBPlanEvidence } = {}) {
  const approvalReportBytes = Buffer.from(`${JSON.stringify(approval, null, 2)}\n`);
  assertStageBPlanApprovalReport(approval, { approvalReportBytes, captureReport, captureReportBytes, referenceAudit, referenceAuditBytes, plan, hashes, logicalCanonicalPlanJsonSha256, referenceAuditSha256, trustedCallerArn, stageBLineage, stageBSerial, terraformConfiguration });
  const result = {
