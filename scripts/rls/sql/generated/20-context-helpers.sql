...
DO $$ BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM mscqr_rls_install.state WHERE singleton
    ...
    AND source_contract_sha256='6fd37a028f0f7ab1aa9f3f4f7a3f94f18f45018484acfea858c2969d97e57b50'
    AND package_role_marker='mscqr-full-rls-clean-room:certification:6fd37a028f0f7ab1aa9f3f4f7a3f94f18f45018484acfea858c2969d97e57b50'
  ...
  IF (SELECT count(*) FROM pg_roles WHERE rolname IN ('mscqr_rls_cert_owner', 'mscqr_rls_cert_auth_owner', 'mscqr_rls_cert_app', 'mscqr_rls_cert_read', 'mscqr_rls_cert_preauth', 'mscqr_rls_cert_worker', 'mscqr_rls_cert_scheduled', 'mscqr_rls_cert_operator', 'mscqr_rls_cert_migration'))<>9
    ...
    ('mscqr_rls_cert_migration', true)) spec(role_name,expected_login) ON spec.role_name=r.rolname WHERE r.rolcanlogin IS DISTINCT FROM spec.expected_login OR r.rolinherit OR r.rolsuper OR r.rolcreatedb OR r.rolcreaterole OR r.rolreplication OR r.rolbypassrls OR obj_description(r.oid,'pg_authid')<>'mscqr-full-rls-clean-room:certification:6fd37a028f0f7ab1aa9f3f4f7a3f94f18f45018484acfea858c2969d97e57b50')
  ...
  IF false THEN
    IF (SELECT count(*) FROM pg_auth_members m JOIN pg_roles parent ON parent.oid=m.roleid WHERE parent.rolname IN ('mscqr_rls_cert_owner', 'mscqr_rls_cert_auth_owner', 'mscqr_rls_cert_app', 'mscqr_rls_cert_read', 'mscqr_rls_cert_preauth', 'mscqr_rls_cert_worker', 'mscqr_rls_cert_scheduled', 'mscqr_rls_cert_operator', 'mscqr_rls_cert_migration'))<>9
       OR EXISTS (SELECT 1 FROM pg_auth_members m JOIN pg_roles parent ON parent.oid=m.roleid JOIN pg_roles member ON member.oid=m.member JOIN pg_roles grantor ON grantor.oid=m.grantor WHERE parent.rolname IN ('mscqr_rls_cert_owner', 'mscqr_rls_cert_auth_owner', 'mscqr_rls_cert_app', 'mscqr_rls_cert_read', 'mscqr_rls_cert_preauth', 'mscqr_rls_cert_worker', 'mscqr_rls_cert_scheduled', 'mscqr_rls_cert_operator', 'mscqr_rls_cert_migration') AND (member.rolname<>'certification-administrator' OR grantor.rolname<>'rdsadmin' OR m.admin_option OR m.inherit_option OR NOT m.set_option))
       OR EXISTS (SELECT 1 FROM pg_roles parent WHERE parent.rolname IN ('mscqr_rls_cert_owner', 'mscqr_rls_cert_auth_owner', 'mscqr_rls_cert_app', 'mscqr_rls_cert_read', 'mscqr_rls_cert_preauth', 'mscqr_rls_cert_worker', 'mscqr_rls_cert_scheduled', 'mscqr_rls_cert_operator', 'mscqr_rls_cert_migration') AND (SELECT count(*) FROM pg_auth_members m JOIN pg_roles member ON member.oid=m.member JOIN pg_roles grantor ON grantor.oid=m.grantor WHERE m.roleid=parent.oid AND member.rolname='certification-administrator' AND grantor.rolname='rdsadmin' AND NOT m.admin_option AND NOT m.inherit_option AND m.set_option)<>1)
  ...
  ELSIF (EXISTS (SELECT 1 FROM pg_roles WHERE oid=10 AND rolsuper)
      AND NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname IN ('rdsadmin','rds_superuser'))) THEN
    IF (SELECT count(*) FROM pg_auth_members m JOIN pg_roles parent ON parent.oid=m.roleid WHERE parent.rolname IN ('mscqr_rls_cert_owner', 'mscqr_rls_cert_auth_owner', 'mscqr_rls_cert_app', 'mscqr_rls_cert_read', 'mscqr_rls_cert_preauth', 'mscqr_rls_cert_worker', 'mscqr_rls_cert_scheduled', 'mscqr_rls_cert_operator', 'mscqr_rls_cert_migration'))<>18
       OR EXISTS (SELECT 1 FROM pg_auth_members m JOIN pg_roles parent ON parent.oid=m.roleid JOIN pg_roles member ON member.oid=m.member JOIN pg_roles grantor ON grantor.oid=m.grantor WHERE parent.rolname IN ('mscqr_rls_cert_owner', 'mscqr_rls_cert_auth_owner', 'mscqr_rls_cert_app', 'mscqr_rls_cert_read', 'mscqr_rls_cert_preauth', 'mscqr_rls_cert_worker', 'mscqr_rls_cert_scheduled', 'mscqr_rls_cert_operator', 'mscqr_rls_cert_migration') AND (member.rolname<>'certification-administrator' OR m.inherit_option OR NOT ((grantor.oid=10 AND grantor.rolsuper AND m.admin_option AND NOT m.set_option) OR (grantor.rolname='certification-administrator' AND NOT m.admin_option AND m.set_option))))
       OR EXISTS (SELECT 1 FROM pg_roles parent WHERE parent.rolname IN ('mscqr_rls_cert_owner', 'mscqr_rls_cert_auth_owner', 'mscqr_rls_cert_app', 'mscqr_rls_cert_read', 'mscqr_rls_cert_preauth', 'mscqr_rls_cert_worker', 'mscqr_rls_cert_scheduled', 'mscqr_rls_cert_operator', 'mscqr_rls_cert_migration') AND ((SELECT count(*) FROM pg_auth_members m JOIN pg_roles member ON member.oid=m.member JOIN pg_roles grantor ON grantor.oid=m.grantor WHERE m.roleid=parent.oid AND member.rolname='certification-administrator' AND grantor.oid=10 AND grantor.rolsuper AND m.admin_option AND NOT m.inherit_option AND NOT m.set_option)<>1 OR (SELECT count(*) FROM pg_auth_members m JOIN pg_roles member ON member.oid=m.member JOIN pg_roles grantor ON grantor.oid=m.grantor WHERE m.roleid=parent.oid AND member.rolname='certification-administrator' AND grantor.rolname='certification-administrator' AND NOT m.admin_option AND NOT m.inherit_option AND m.set_option)<>1))
       OR EXISTS (SELECT 1 FROM pg_auth_members m JOIN pg_roles member ON member.oid=m.member WHERE member.rolname IN ('mscqr_rls_cert_owner', 'mscqr_rls_cert_auth_owner', 'mscqr_rls_cert_app', 'mscqr_rls_cert_read', 'mscqr_rls_cert_preauth', 'mscqr_rls_cert_worker', 'mscqr_rls_cert_scheduled', 'mscqr_rls_cert_operator', 'mscqr_rls_cert_migration'))
    THEN RAISE EXCEPTION 'managed role membership topology drifted'; END IF;
  ELSE
...
BEGIN
  ...
  IF (app_rls.current_role() IN ('SUPER_ADMIN','PLATFORM_SUPER_ADMIN') AND app_rls.current_assurance() NOT IN ('mfa-verified','step-up-verified','dual-approved-break-glass'))
     OR ((app_rls.current_role() IN ('LICENSEE_ADMIN','ORG_ADMIN') OR app_rls.current_role() IN ('MANUFACTURER','MANUFACTURER_ADMIN','MANUFACTURER_USER')) AND app_rls.current_assurance() NOT IN ('password-verified','mfa-verified','step-up-verified','dual-approved-break-glass'))
...
DECLARE
  ...
  membership_fingerprint text;
BEGIN
  IF NOT ((current_user='mscqr_rls_cert_auth_owner' AND app_rls.operational_read_session_valid()) AND app_rls.attributed_request() AND app_rls.current_purpose()='batch-operational-read' AND app_rls.current_request_id() ~ '^[A-Za-z0-9._:-]{1,128}$' AND (((app_rls.current_role() IN ('LICENSEE_ADMIN','ORG_ADMIN') OR app_rls.current_role() IN ('MANUFACTURER','MANUFACTURER_ADMIN','MANUFACTURER_USER')) AND app_rls.current_assurance() IN ('password-verified','mfa-verified','step-up-verified','dual-approved-break-glass')) OR (app_rls.current_role() IN ('SUPER_ADMIN','PLATFORM_SUPER_ADMIN') AND app_rls.current_assurance() IN ('mfa-verified','step-up-verified','dual-approved-break-glass'))))
...
BEGIN
  ...
  IF t."userId" IS DISTINCT FROM p_user_id OR t."revokedAt" IS NOT NULL OR t."expiresAt"<=p_finalized_at
     ...
     OR (actor_role IN ('SUPER_ADMIN','PLATFORM_SUPER_ADMIN','ORG_ADMIN')
...
LANGUAGE plpgsql VOLATILE PARALLEL UNSAFE SECURITY DEFINER SET search_path=pg_catalog,public AS $fn$
DECLARE invite_row record; actor_row record; candidate_ids text[]; candidate_user_ids text[]; licensee_name text; challenge_id text; challenge_created_at timestamp;
BEGIN
  IF coalesce(array_length(p_token_hash_candidates,1),0) NOT BETWEEN 1 AND 3
     OR EXISTS (SELECT 1 FROM unnest(p_token_hash_candidates) h WHERE h IS NULL OR h !~ '^([0-9a-f]{12}:)?[0-9a-f]{64}$')
     OR (SELECT count(DISTINCT h) FROM unnest(p_token_hash_candidates) h)<>array_length(p_token_hash_candidates,1)
  ...
  SELECT array_agg(i.id ORDER BY i.id) INTO candidate_ids FROM public."Invite" i
    WHERE i."tokenHash"=ANY(p_token_hash_candidates) AND i."expiresAt">p_checked_at;
  IF coalesce(array_length(candidate_ids,1),0)<>1 THEN RETURN; END IF;
  ...
  IF invite_row."usedAt" IS NULL THEN
    IF actor_row."passwordHash" IS NOT NULL THEN RETURN; END IF;
  ELSE
    IF invite_row."acceptedByUserId" IS DISTINCT FROM actor_row.id OR actor_row."passwordHash" IS NULL THEN RETURN; END IF;
    SELECT c.id,c."createdAt" INTO challenge_id,challenge_created_at FROM public."InviteActivationChallenge" c
...
RETURNS TABLE("challengeId" text,"userId" text,"inviteId" text,"email" text)
LANGUAGE plpgsql VOLATILE PARALLEL UNSAFE SECURITY DEFINER SET search_path=pg_catalog,public AS $fn$
...
BEGIN
  ...
  SELECT c.id,c."userId",c."inviteId",c.email INTO challenge_row FROM public."InviteActivationChallenge" c
    WHERE c.id=p_challenge_id AND c.purpose='INVITE_ACTIVATION' AND c."consumedAt" IS NULL AND c."supersededAt" IS NULL;
  IF FOUND THEN RETURN QUERY SELECT challenge_row.id,challenge_row."userId",challenge_row."inviteId",challenge_row.email; END IF;
END
$fn$;
...
BEGIN
  IF p_challenge_id IS NULL OR p_challenge_id !~ '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$'
     ...
     OR p_code_verifier IS NULL OR p_code_verifier !~ '^[0-9a-f]{12}:[0-9a-f]{64}$'
     OR p_requested_at IS NULL OR abs(extract(epoch FROM (p_requested_at-(clock_timestamp() AT TIME ZONE 'UTC'))))>300
     OR p_expires_at IS NULL OR p_expires_at<>p_requested_at+interval '10 minutes' THEN RETURN; END IF;
  ...
  SELECT c.id,c."userId",c."inviteId",c.email,c."createdAt",c."consumedAt",c."supersededAt",c.purpose
    INTO challenge_row FROM public."InviteActivationChallenge" c WHERE c.id=p_challenge_id;
  IF NOT FOUND THEN RETURN; END IF;
  PERFORM set_config('app.b01_preauth_user_id',challenge_row."userId",true),
    set_config('app.b01_preauth_invite_id',challenge_row."inviteId",true);
  ...
  IF actor_row.id IS NULL OR invite_row.id IS NULL OR challenge_row.purpose<>'INVITE_ACTIVATION'
     ...
     OR invite_row."usedAt" IS NULL OR invite_row."expiresAt"<=p_requested_at
...
GRANT EXECUTE ON FUNCTION app_auth.consume_invitation_token(text[],text,text,timestamp without time zone,text,text,text,text,text,timestamp without time zone) TO "mscqr_rls_cert_preauth";
...
GRANT EXECUTE ON FUNCTION app_auth.lookup_invite_activation_challenge(text) TO "mscqr_rls_cert_preauth";
...
GRANT EXECUTE ON FUNCTION app_auth.resend_invite_activation(text,text,text,timestamp without time zone,timestamp without time zone) TO "mscqr_rls_cert_preauth";
GRANT EXECUTE ON FUNCTION app_auth.verify_invite_activation(text,text[],timestamp without time zone) TO "mscqr_rls_cert_preauth";
RESET ROLE;
DO $$ BEGIN
  IF NOT pg_has_role(session_user,'mscqr_rls_cert_auth_owner','SET') THEN RAISE EXCEPTION 'administrative executor lacks SET authority for mscqr_rls_cert_auth_owner'; END IF;
END $$;
...
BEGIN
  ...
  actor_state:=jsonb_build_object(
    ...
    'linkedLicensees',links,'mfaRequired',actor.role IN ('SUPER_ADMIN','PLATFORM_SUPER_ADMIN','ORG_ADMIN'),
...
BEGIN
  ...
  IF p_blocked_login THEN
    IF wants_challenge OR actor.role NOT IN ('SUPER_ADMIN','PLATFORM_SUPER_ADMIN','LICENSEE_ADMIN','MANUFACTURER','MANUFACTURER_ADMIN','MANUFACTURER_USER') THEN
      RAISE EXCEPTION 'AUTH_LOGIN_RISK_DENIED' USING ERRCODE='42501';
    END IF;
