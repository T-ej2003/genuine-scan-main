...
DO $$ DECLARE existing_count integer; rec record; BEGIN
  ...
  IF EXISTS (SELECT 1 FROM pg_roles r JOIN (VALUES ('mscqr_rls_cert_owner', false),
    ...
    ('mscqr_rls_cert_migration', true)) spec(role_name,expected_login) ON spec.role_name=r.rolname WHERE r.rolcanlogin IS DISTINCT FROM spec.expected_login OR r.rolinherit OR r.rolsuper OR r.rolcreatedb OR r.rolcreaterole OR r.rolreplication OR r.rolbypassrls OR obj_description(r.oid,'pg_authid')<>'mscqr-full-rls-clean-room:certification:a9c282f82a7b41d2587510d64a308fcfa04eee13b21652f3a7ed59218726e114') THEN RAISE EXCEPTION 'role cleanup refuses an unmarked or drifted role'; END IF;
