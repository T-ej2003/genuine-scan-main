...
DO $$ DECLARE existing_count integer; rec record; BEGIN
  ...
  IF EXISTS (SELECT 1 FROM pg_roles r JOIN (VALUES ('mscqr_rls_cert_owner', false),
    ...
    ('mscqr_rls_cert_migration', true)) spec(role_name,expected_login) ON spec.role_name=r.rolname WHERE r.rolcanlogin IS DISTINCT FROM spec.expected_login OR r.rolinherit OR r.rolsuper OR r.rolcreatedb OR r.rolcreaterole OR r.rolreplication OR r.rolbypassrls OR obj_description(r.oid,'pg_authid')<>'mscqr-full-rls-clean-room:certification:be3e24ca8d773e88ad38fe7df04d15e1186b11943eabbe7b82c32a3c3f939136') THEN RAISE EXCEPTION 'role cleanup refuses an unmarked or drifted role'; END IF;
