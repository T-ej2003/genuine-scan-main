...
DO $$ DECLARE existing_count integer; rec record; BEGIN
  ...
  IF EXISTS (SELECT 1 FROM pg_roles r JOIN (VALUES ('mscqr_rls_cert_owner', false),
    ...
    ('mscqr_rls_cert_migration', true)) spec(role_name,expected_login) ON spec.role_name=r.rolname WHERE r.rolcanlogin IS DISTINCT FROM spec.expected_login OR r.rolinherit OR r.rolsuper OR r.rolcreatedb OR r.rolcreaterole OR r.rolreplication OR r.rolbypassrls OR obj_description(r.oid,'pg_authid')<>'mscqr-full-rls-clean-room:certification:fb04ec1571b1b3ec7a3a1b6d24d951f1ec2c0b38168629fc4ca7aa45fd539b74') THEN RAISE EXCEPTION 'role cleanup refuses an unmarked or drifted role'; END IF;
