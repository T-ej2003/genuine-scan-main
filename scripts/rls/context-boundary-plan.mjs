...
export const buildContextBoundaryPlan = () => {
  const { workflows, commandSemantics, tables, workerBoundaries, operatorBoundaries, preAuthFunctions } = manifests();
  ...
  for (const workflow of workflows.workflows) {
    ...
    const category = categoryFor(workflow, workflowRules, tableMap);
    ...
    const signature = [category, source, workflow.authorizationBoundaryType, allCommands(workflow).join("+"), unique(workflow.commandActorClasses || []).join("+"), unique(workflow.requiredAssurance || []).join("+")].join("|");
    const key = implementedFamily || (boundaryId ? `family-contract-${slug(boundaryId)}` : `family-${slug(category).slice(0, 32)}-${slug(path.basename(source, path.extname(source))).slice(0, 32)}-${digest(signature)}`);
    if (!groups.has(key)) groups.set(key, { category, workflows: [], dispositions: [] });
  ...
  const families = [...groups.entries()].map(([id, group]) => {
    ...
    return {
      ...
      executionSurfaces: unique(familyWorkflows.map((workflow) => workflow.executionSurface)),
      ...
      actorClasses: unique(familyWorkflows.flatMap((workflow) => workflow.commandActorClasses || [])),
      runtimeIdentity: runtimeIdentities.length === 1 ? runtimeIdentities[0] : runtimeIdentities.length ? "mixed" : "none",
      requiredAssurance: unique(familyWorkflows.flatMap((workflow) => workflow.requiredAssurance || [])),
      canonicalContextKeys: automationEligibility === "contract-only" ? [] : canonicalContextKeys,
      readWriteCommandRuleIds: ruleIds,
      tenantScopeModel: unique(familyRules.map((rule) => rule.scopeRule)),
      ...
      riskLevel: unique(familyWorkflows.map((workflow) => riskFor(group.category, workflow, workflow.commandRuleIds.map((ruleId) => rules.get(ruleId)).filter(Boolean)))).sort((a, b) => ["low", "medium", "high", "critical"].indexOf(b) - ["low", "medium", "high", "critical"].indexOf(a))[0],
  ...
  const manifest = {
    schemaVersion: 1,
    generatedFrom: ["documents/security/rls-program/workflows.json", "documents/security/rls-program/command-semantics.json", "documents/security/rls-program/tables.json", "documents/security/rls-program/runtime-identities.json", "documents/security/rls-program/pre-auth-functions.json", "documents/security/rls-program/worker-boundaries.json", "documents/security/rls-program/operator-boundaries.json", "documents/security/rls-program/policy-dependency-graph.json", "documents/security/rls-program/ARCHITECTURE.md", "backend/prisma/schema.prisma"],
    workflowCount: workflows.workflows.length,
    familyCount: families.length,
    baselineImplementedWorkflowIds,
    approvedBoundaryCounts: { preAuthFunctions: preAuthFunctions.functions.length, workerBoundaries: workerBoundaries.boundaries.length, operatorBoundaries: operatorBoundaries.boundaries.length },
    familyOrder: categoryOrder,
    families,
  };
  validateContextBoundaryPlan(manifest, workflows, commandSemantics, tables);
  fs.writeFileSync(contextBoundaryFamiliesPath, `${JSON.stringify(manifest, null, 2)}\n`);
  writeReport(manifest);
  return manifest;
};

export const validateContextBoundaryPlan = (manifest, workflowManifest, commandManifest, tableManifest) => {
