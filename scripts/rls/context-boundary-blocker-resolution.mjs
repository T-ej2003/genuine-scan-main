...
const ids = {
  ...
  manufacturerIds: "workflow-internal-backend-src-services-manufacturer-scope-service-ts-list-manufacturer-linked-licensee-ids",
...
const workflowReviews = new Map([
  ...
  [ids.manufacturerIds, { blockers: [blockers.authBootstrap], decisions: ["decision-context-manufacturer-bootstrap"], split: split(familyIds.manufacturer, "manufacturer-id-scope-hydration", "Separate minimal linked-licensee ID hydration from richer link/detail projections.", ["hydrateTenantIfNeeded supplies its transaction client after JWT verification; fallback application callers may not."], ["manufacturer"], "Manufacturer actor ID is authoritative; linked licensee IDs are the result and cannot be prerequisite tenant context.", "internal", "ManufacturerLicenseeLink actor-bound ID projection", "SELECT licenseeId only; no licensee wildcard.") }],
...
const reviewedGroups = [
  ...
  [familyIds.dashboard, [ids.dashboard], "blocked"],
  [familyIds.compliance, [ids.compliance], "blocked"],
  [familyIds.feedback, [ids.feedback], "blocked"],
  [familyIds.replacement, [ids.replacement], "blocked"],
