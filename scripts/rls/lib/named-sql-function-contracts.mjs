...
const preAuthSecurity = Object.freeze({
  ...
  ownerPrivileges: [
    ...
    ["Invite", "SELECT", ["id","orgId","licenseeId","email","role","manufacturerId","tokenHash","expiresAt","usedAt","acceptedByUserId"]],
    ["Invite", "UPDATE", ["usedAt","acceptedByUserId"]],
    ["InviteActivationChallenge", "SELECT", ["id","userId","inviteId","email","purpose","codeVerifier","expiresAt","attemptCount","maxAttempts","createdAt","consumedAt","supersededAt"]],
    ["InviteActivationChallenge", "INSERT", ["id","userId","inviteId","email","purpose","codeVerifier","expiresAt","attemptCount","maxAttempts","createdAt"]],
    ["InviteActivationChallenge", "UPDATE", ["attemptCount","consumedAt","supersededAt"]],
  ...
  ownerPolicies: [
    ...
    ["User", "UPDATE", `(${preAuthOwner} AND ((${preAuthOperation}='password-failure' AND lower(email)=current_setting('app.b01_preauth_email',true)) OR (${preAuthOperation} IN ('reset-consume','invite-consume','email-consume','activation-verify','activation-resend') AND ${preAuthUserId}<>'' AND id=${preAuthUserId})))`],
    ["Invite", "SELECT", `(${preAuthOwner} AND ((${preAuthOperation} IN ('invite-lookup','invite-consume') AND "tokenHash"=ANY(${preAuthHashes})) OR (${preAuthOperation} IN ('activation-verify','activation-resend') AND id=current_setting('app.b01_preauth_invite_id',true))))`],
    ["Invite", "UPDATE", `(${preAuthOwner} AND ${preAuthOperation}='invite-consume' AND id=${preAuthTokenId} AND "tokenHash"=ANY(${preAuthHashes}))`],
    ["InviteActivationChallenge", "SELECT", `(${preAuthOwner} AND ((${preAuthOperation}='invite-lookup' AND "inviteId"=current_setting('app.b01_preauth_invite_id',true)) OR (${preAuthOperation} IN ('activation-lookup','activation-verify','activation-resend') AND id=${preAuthTokenId})))`],
    ["InviteActivationChallenge", "INSERT", `(${preAuthOwner} AND ${preAuthOperation} IN ('invite-consume','activation-resend') AND id=current_setting('app.b01_preauth_new_challenge_id',true) AND "userId"=${preAuthUserId} AND "inviteId"=current_setting('app.b01_preauth_invite_id',true) AND email=current_setting('app.b01_preauth_email',true) AND purpose='INVITE_ACTIVATION')`],
    ["InviteActivationChallenge", "UPDATE", `(${preAuthOwner} AND ${preAuthOperation} IN ('activation-verify','activation-resend') AND id=${preAuthTokenId} AND "userId"=${preAuthUserId} AND "inviteId"=current_setting('app.b01_preauth_invite_id',true))`],
    ...
    ["Licensee", "SELECT", `(${preAuthOwner} AND ${preAuthOperation} IN ('invite-lookup','invite-consume','activation-verify','activation-resend') AND id=current_setting('app.b01_preauth_licensee_id',true) AND "orgId"=current_setting('app.b01_preauth_org_id',true))`],
    ["Organization", "SELECT", `(${preAuthOwner} AND ${preAuthOperation} IN ('invite-lookup','invite-consume','activation-verify','activation-resend') AND id=current_setting('app.b01_preauth_org_id',true))`],
    ["AuditLogOutbox", "INSERT", `(${preAuthOwner} AND ${preAuthUserId}<>'' AND payload->>'userId'=${preAuthUserId} AND payload->>'action' IN ('AUTH_LOGIN_FAIL','AUTH_LOGIN_LOCKED','AUTH_PASSWORD_RESET_REQUESTED','AUTH_PASSWORD_RESET_COMPLETED','AUTH_EMAIL_VERIFIED','AUTH_EMAIL_CHANGE_CONFIRMED','AUTH_INVITE_ACCEPTED','AUTH_INVITE_PASSWORD_SET','AUTH_INVITE_ACTIVATED'))`],
  ],
});
...
const authenticationClosureSecurity = Object.freeze({
  ...
  ownerPolicies: [
    ...
    ["RefreshToken", "INSERT", `((${authClosureLoginBinding} AND current_setting('app.auth_closure_operation',true)='login-session-create') OR (${authClosureSessionBinding} AND current_setting('app.auth_closure_operation',true)='authenticated-session-create')) AND id=current_setting('app.auth_closure_token_id',true) AND "userId"=current_setting('app.auth_closure_user_id',true) AND "tokenHash"=current_setting('app.auth_closure_token_hash',true) AND coalesce("orgId",'')=current_setting('app.auth_closure_organization_id',true)`],
    ...
    ["MfaLoginChallenge", "SELECT", `${authClosureSessionBinding} AND current_setting('app.auth_closure_operation',true) IN ('mfa-challenge-read','mfa-challenge-fail','mfa-challenge-complete') AND "userId"=current_setting('app.auth_closure_user_id',true)`],
    ...
    ["AuthMfaChallenge", "SELECT", `${authClosureSessionBinding} AND current_setting('app.auth_closure_operation',true) IN ('mfa-challenge-read','mfa-challenge-fail','mfa-challenge-complete') AND "userId"=current_setting('app.auth_closure_user_id',true)`],
    ...
    ["AuditLogOutbox", "INSERT", `(${authClosureLoginBinding} AND ((current_setting('app.auth_closure_operation',true) IN ('login-mfa-challenge','login-session-create') AND payload->>'action' IN ('AUTH_MFA_CHALLENGE_ISSUED','AUTH_LOGIN_SUCCESS','AUTH_LOGIN_SUCCESS_RECENT_ADMIN_MFA')) OR (current_setting('app.auth_closure_operation',true)='login-risk-block-audit' AND payload->>'action'='AUTH_LOGIN_BLOCKED_RISK' AND payload->>'entityType'='User' AND payload->>'entityId'=current_setting('app.auth_closure_user_id',true) AND payload->'details'->>'riskScore'=current_setting('app.auth_closure_risk_score',true) AND payload->'details'->>'riskLevel'=current_setting('app.auth_closure_risk_level',true) AND "jobType"='AUDIT_LOG_RECOVERY' AND "requestId"=current_setting('app.auth_closure_request_id',true) AND "requestId"~'^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$' AND "payloadDigest"=encode(sha256(convert_to(payload::text,'UTF8')),'hex') AND "idempotencyKey"=encode(sha256(convert_to('AUDIT_LOG_RECOVERY:'||"requestId"||':'||"payloadDigest",'UTF8')),'hex') AND "organizationId" IS NOT DISTINCT FROM nullif(current_setting('app.auth_closure_organization_id',true),'') AND "licenseeId" IS NOT DISTINCT FROM nullif(current_setting('app.auth_closure_licensee_id',true),'') AND "initiatingUserId"=current_setting('app.auth_closure_user_id',true) AND "initiatingActorRoleSnapshot" IN ('SUPER_ADMIN','PLATFORM_SUPER_ADMIN','LICENSEE_ADMIN','MANUFACTURER','MANUFACTURER_ADMIN','MANUFACTURER_USER'))) AND payload->>'userId'=current_setting('app.auth_closure_user_id',true)) OR (${authClosureSessionBinding} AND (((current_setting('app.auth_closure_operation',true) IN ('profile-update','password-change','mfa-enrollment-complete','mfa-disable','mfa-challenge-create','mfa-challenge-fail','mfa-challenge-complete','mfa-webauthn-registration-complete','mfa-webauthn-delete') AND payload->>'action' IN ('AUTH_PROFILE_UPDATED','AUTH_PASSWORD_CHANGED','AUTH_MFA_ENROLLED','AUTH_MFA_REPLACED','AUTH_MFA_DISABLED','AUTH_MFA_CHALLENGE_ISSUED','AUTH_MFA_CHALLENGE_EXPIRED','AUTH_MFA_FAILURE','AUTH_MFA_TOO_MANY_ATTEMPTS','AUTH_MFA_BACKUP_CODE_USED','AUTH_MFA_SUCCESS','AUTH_MFA_LOGIN_COMPLETE','AUTH_WEBAUTHN_ENROLLED','AUTH_WEBAUTHN_CREDENTIAL_REMOVED')) OR (current_setting('app.auth_closure_operation',true)='authenticated-session-create' AND payload->>'action' IN ('AUTH_LOGIN_SUCCESS','AUTH_LOGIN_SUCCESS_RECENT_ADMIN_MFA'))) OR (current_setting('app.auth_closure_operation',true)='manufacturer-scope-read' AND payload->>'action' IN ('MANUFACTURER_BOOTSTRAP_READ','MANUFACTURER_SCOPE_SWITCH'))) AND payload->>'userId'=current_setting('app.auth_closure_user_id',true))`],
  ],
});
...
const b03AttentionRead = `${b03AuthenticatedBinding} AND current_setting('app.b03_operation',true)='attention-read'`;
const b03AttentionRole = `current_setting('app.b03_actor_role',true)`;
const b03AttentionLicensee = `current_setting('app.b03_licensee_id',true)`;
const b03AttentionActor = `current_setting('app.b03_actor_id',true)`;
const b03AttentionTenantScope = `((${b03AttentionRole} IN ('SUPER_ADMIN','PLATFORM_SUPER_ADMIN') AND (${b03AttentionLicensee}='' OR "licenseeId"=${b03AttentionLicensee})) OR (${b03AttentionRole}='LICENSEE_ADMIN' AND "licenseeId"=current_setting('app.b03_actor_licensee_id',true)) OR (${b03AttentionRole}='MANUFACTURER_ADMIN' AND "licenseeId"=${b03AttentionLicensee}))`;
const b03AuthenticatedSecurity = Object.freeze({
  ...
  ownerPrivileges:[
    ...
    ["Incident","SELECT",["id","qrCodeId","qrCodeValue","scanEventId","licenseeId","severity","status","slaDueAt","createdAt"]],
    ["QRCode","SELECT",["id","batchId"]],
    ["Batch","SELECT",["id","licenseeId","manufacturerId"]],
    ["QrScanLog","SELECT",["id","licenseeId","batchId"]],
    ["PolicyAlert","SELECT",["id","licenseeId","alertType","severity","message","manufacturerId","acknowledgedAt","createdAt"]],
    ["SupportTicket","SELECT",["id","incidentId","referenceCode","licenseeId","customerEmail","subject","status","priority","assignedToUserId","slaDueAt","firstResponseAt","resolvedAt","createdAt","updatedAt"]],
    ["SupportTicket","UPDATE",["status","assignedToUserId","resolvedAt","updatedAt"]],
    ["SupportTicketMessage","SELECT",["id","ticketId","actorType","actorUserId","message","isInternal","createdAt"]],
    ["SupportTicketMessage","INSERT",["id","ticketId","actorType","actorUserId","message","isInternal","createdAt"]],
    ["IncidentHandoff","SELECT",["id","incidentId","currentStage","slaDueAt"]],
    ["AuditLog","SELECT",["id","userId","licenseeId","action","entityType","entityId","createdAt"]],
  ...
  ownerPolicies:[
    ...
    ["User","SELECT",`${b03AuthenticatedBinding} AND (id=current_setting('app.b03_actor_id',true) OR current_setting('app.b03_operation',true)='superadmin-read' OR current_setting('app.b03_operation',true) LIKE 'support-ticket-%' OR (current_setting('app.b03_operation',true) IN ('notification-write','actor-read') AND (current_setting('app.b03_actor_role',true) IN ('SUPER_ADMIN','PLATFORM_SUPER_ADMIN') OR "licenseeId"=current_setting('app.b03_licensee_id',true) OR id=current_setting('app.b03_target_user_id',true))))`],
    ...
    ["Incident","SELECT",`(${b03AuthenticatedBinding} AND (id=current_setting('app.b03_incident_id',true) OR "licenseeId"=current_setting('app.b03_licensee_id',true) OR current_setting('app.b03_operation',true) LIKE 'support-ticket-%')) OR (${b03AttentionRead} AND ${b03AttentionTenantScope})`],
    ["QRCode","SELECT",`(${b03AuthenticatedBinding} AND current_setting('app.b03_operation',true)='incident-scope-read') OR (${b03AttentionRead} AND (${b03AttentionLicensee}='' OR "licenseeId"=${b03AttentionLicensee}))`],
    ["Batch","SELECT",`(${b03AuthenticatedBinding} AND current_setting('app.b03_operation',true)='incident-scope-read') OR (${b03AttentionRead} AND ${b03AttentionTenantScope} AND (${b03AttentionRole}<>'MANUFACTURER_ADMIN' OR "manufacturerId"=${b03AttentionActor}))`],
    ["QrScanLog","SELECT",`${b03AttentionRead} AND ${b03AttentionTenantScope}`],
    ["PolicyAlert","SELECT",`${b03AttentionRead} AND ${b03AttentionTenantScope} AND (${b03AttentionRole}<>'MANUFACTURER_ADMIN' OR "manufacturerId"=${b03AttentionActor})`],
    ["SupportTicket","SELECT",`((${b03AttentionRead} OR (${b03AuthenticatedBinding} AND current_setting('app.b03_operation',true) LIKE 'support-ticket-%')) AND ${b03AttentionRole} IN ('SUPER_ADMIN','PLATFORM_SUPER_ADMIN') AND (coalesce(${b03AttentionLicensee},'')='' OR "licenseeId"=${b03AttentionLicensee}) AND (coalesce(current_setting('app.b03_support_ticket_id',true),'')='' OR id=current_setting('app.b03_support_ticket_id',true)))`],
    ["SupportTicket","UPDATE",`${b03AuthenticatedBinding} AND current_setting('app.b03_operation',true)='support-ticket-update' AND id=current_setting('app.b03_support_ticket_id',true) AND ${b03AttentionRole} IN ('SUPER_ADMIN','PLATFORM_SUPER_ADMIN')`],
    ["SupportTicketMessage","SELECT",`${b03AuthenticatedBinding} AND current_setting('app.b03_operation',true)='support-ticket-detail' AND "ticketId"=current_setting('app.b03_support_ticket_id',true)`],
    ["SupportTicketMessage","INSERT",`${b03AuthenticatedBinding} AND current_setting('app.b03_operation',true)='support-ticket-message' AND "ticketId"=current_setting('app.b03_support_ticket_id',true) AND "actorUserId"=current_setting('app.b03_actor_id',true)`],
    ["IncidentHandoff","SELECT",`${b03AuthenticatedBinding} AND current_setting('app.b03_operation',true) LIKE 'support-ticket-%'`],
    ["AuditLog","SELECT",`${b03AttentionRead} AND ((${b03AttentionRole} IN ('SUPER_ADMIN','PLATFORM_SUPER_ADMIN') AND (${b03AttentionLicensee}='' OR "licenseeId"=${b03AttentionLicensee})) OR (${b03AttentionRole}='LICENSEE_ADMIN' AND "licenseeId"=current_setting('app.b03_actor_licensee_id',true)) OR (${b03AttentionRole}='MANUFACTURER_ADMIN' AND "userId"=${b03AttentionActor}))`],
    ...
    ["AuditLog","INSERT",`${b03AuthenticatedBinding} AND ((current_setting('app.b03_operation',true)='incident-email-complete' AND "userId"=current_setting('app.b03_actor_id',true) AND "licenseeId"=current_setting('app.b03_licensee_id',true)) OR (current_setting('app.b03_operation',true) IN ('support-ticket-update','support-ticket-message') AND "userId"=current_setting('app.b03_actor_id',true) AND "entityType"='SupportTicket' AND "entityId"=current_setting('app.b03_support_ticket_id',true)))`],
...
const administrationSecurity = Object.freeze({
  ...
  ownerPrivileges:[
    ["User","SELECT",["id","email","name","role","orgId","licenseeId","status","isActive","disabledAt","deletedAt","passwordHash","emailVerifiedAt","location","website","createdAt"]],
    ...
    ["InviteActivationChallenge","SELECT",["id","userId","consumedAt","supersededAt"]],
    ["InviteActivationChallenge","UPDATE",["supersededAt"]],
  ],
  ownerPolicies:[
    ...
    ["User","UPDATE",`${adminSession} AND ((${adminOperation} IN ('administration-update-user','administration-delete-user','administration-restore-manufacturer') AND id=${adminTargetUser}) OR (${adminOperation} IN ('auth-invite-create','licensee-admin-invite-resend') AND id=${adminTargetUser} AND status::text='INVITED' AND "emailVerifiedAt" IS NULL))`],
    ...
    ["InviteActivationChallenge","SELECT",`${adminSession} AND ${adminOperation} IN ('auth-invite-create','licensee-admin-invite-resend') AND "userId"=${adminTargetUser}`],
    ["InviteActivationChallenge","UPDATE",`${adminSession} AND ${adminOperation} IN ('auth-invite-create','licensee-admin-invite-resend') AND "userId"=${adminTargetUser}`],
...
const printingOwnerPrivileges = [
  ...
  ["PrinterAttestation","SELECT",["id","printerRegistrationId","heartbeatNonce","attestedAt","expiresAt","signatureValid","trustValid","rejectionReason","metadata","createdAt"]],
...
const operationalCommonCommands = [["RefreshToken","SELECT"],["User","SELECT"],["Licensee","SELECT"],["Organization","SELECT"],["ManufacturerLicenseeLink","SELECT"],["AuditLog","SELECT"],["AuditLog","INSERT"]];
...
export const NAMED_SQL_FUNCTION_CONTRACTS = Object.freeze([
  {
    ...
    canonicalWorkflowIds: ["workflow-startup-backend-src-rls-waves-session-c-operator-procedure-service-ts-bootstrap-configured-super-admin-procedure"],
  ...
  publicContract({id:"session-start",name:"start_verification_session",signature:"text,text,text,timestamp without time zone,text,text",returnType:"TABLE(sessionId text, sessionProofToken text, maskedCode text, customerFacingState text, startedAt timestamp without time zone, expiresAt timestamp without time zone, brandName text)",tableCommands:[["CustomerAuthSession","SELECT"],["CustomerAuthSession","UPDATE"],["VerificationEvidenceSnapshot","SELECT"],["VerificationEvidenceSnapshot","UPDATE"],["VerificationDecision","SELECT"],["Licensee","SELECT"],["CustomerVerificationSession","INSERT"],["AuditLog","INSERT"],["SecurityEventOutbox","INSERT"]],workflow:"workflow-internal-backend-src-services-customer-verification-session-service-ts-create-customer-verification-session",caller:"backend/src/rls-waves/session-b/b02/publicBoundaryRepository.ts:startVerificationSession",outputColumns:["sessionId","sessionProofToken","maskedCode","customerFacingState","startedAt","expiresAt","brandName"]}),
  ...
  operationalContract({id:"batch-reservable-qr-summaries",name:"batch_reservable_qr_summaries",signature:"text,text,text,text,text,text,text,text,text[]",returnType:"TABLE(batch_id text, item_count bigint, start_code text, end_code text)",identityArguments:"p_capability text, p_purpose text, p_request_id text, audit_id text, requested_licensee_id text, route_surface text, focus_batch_id text, expected_scope_fingerprint text, batch_ids text[]",tableCommands:[...operationalCommonCommands,["Batch","SELECT"],["QRCode","SELECT"],["PrintItem","SELECT"],["PrintSession","SELECT"],["PrintJob","SELECT"]],canonicalWorkflowIds:[batchWorkflows.reservable],repositoryCallers:["backend/src/services/printReservationService.ts:listReservableQrCodeSummaries"],outputColumns:["batch_id","item_count","start_code","end_code"]}),
  {
    ...
    tableCommands:[["RefreshToken","SELECT"],["User","SELECT"],["Licensee","SELECT"],["Organization","SELECT"],["ManufacturerLicenseeLink","SELECT"],["Invite","SELECT"],["Batch","SELECT"],["QRCode","SELECT"],["QRRange","SELECT"]],
  ...
  },
  {
    ...
    tableCommands:[["RefreshToken","SELECT"],["User","SELECT"],["Licensee","SELECT"],["Organization","SELECT"],["ManufacturerLicenseeLink","SELECT"]],
  ...
  },
  {
    ...
    tableCommands:[["RefreshToken","SELECT"],["RefreshToken","UPDATE"],["User","SELECT"],["User","INSERT"],["User","UPDATE"],["Organization","SELECT"],["Organization","INSERT"],["Licensee","SELECT"],["ManufacturerLicenseeLink","SELECT"],["ManufacturerLicenseeLink","INSERT"],["Invite","SELECT"],["Invite","INSERT"],["Invite","UPDATE"],["InviteActivationChallenge","SELECT"],["InviteActivationChallenge","UPDATE"],["ActionIdempotencyKey","SELECT"],["ActionIdempotencyKey","INSERT"],["ActionIdempotencyKey","UPDATE"],["AuditLog","INSERT"],["SecurityEventOutbox","INSERT"]],
  ...
  {
    id:"b03-list-support-tickets",schema:"app_rls",name:"b03_list_support_tickets",signature:"text,text,text,text,integer,integer,text",returnType:"jsonb",identityArguments:"p_licensee_id text, p_status text, p_priority text, p_search text, p_limit integer, p_offset integer, p_request_id text",definitionLocation:b03AuthenticatedSource,definitionKind:"checked-in-production-package",definitionStatus:"production-reviewed",security:b03AuthenticatedSecurity,tableCommands:[["RefreshToken","SELECT"],["User","SELECT"],["SupportTicket","SELECT"],["Incident","SELECT"],["IncidentHandoff","SELECT"]],context:"Returns one stable bounded support-ticket page only to a capability-bound live platform administrator with recent MFA.",canonicalWorkflowIds:["workflow-internal-backend-src-rls-waves-session-b-b03-repository-functions-ts-list-support-tickets"],repositoryCallers:["backend/src/rls-waves/session-b/b03/repositoryFunctions.ts:listSupportTickets"],inputAuthority:"authenticated capability and recent MFA; filters only narrow platform scope",outputColumns:["result"],disposableProbes:["b03-authenticated-postgres18"],
  },
  {
    id:"b03-get-support-ticket",schema:"app_rls",name:"b03_get_support_ticket",signature:"text,text",returnType:"jsonb",identityArguments:"p_ticket_id text, p_request_id text",definitionLocation:b03AuthenticatedSource,definitionKind:"checked-in-production-package",definitionStatus:"production-reviewed",security:b03AuthenticatedSecurity,tableCommands:[["RefreshToken","SELECT"],["User","SELECT"],["SupportTicket","SELECT"],["SupportTicketMessage","SELECT"],["Incident","SELECT"],["IncidentHandoff","SELECT"]],context:"Returns one explicit support-ticket and message projection only to a capability-bound live platform administrator with recent MFA.",canonicalWorkflowIds:["workflow-internal-backend-src-rls-waves-session-b-b03-repository-functions-ts-get-support-ticket"],repositoryCallers:["backend/src/rls-waves/session-b/b03/repositoryFunctions.ts:getSupportTicket"],inputAuthority:"authenticated capability and recent MFA; ticket ID is selection-only",outputColumns:["result"],disposableProbes:["b03-authenticated-postgres18"],
  },
  {
    id:"b03-update-support-ticket",schema:"app_rls",name:"b03_update_support_ticket",signature:"text,text,text,boolean,timestamp without time zone,text",returnType:"jsonb",identityArguments:"p_ticket_id text, p_status text, p_assigned_to_user_id text, p_set_assignee boolean, p_changed_at timestamp without time zone, p_request_id text",definitionLocation:b03AuthenticatedSource,definitionKind:"checked-in-production-package",definitionStatus:"production-reviewed",security:b03AuthenticatedSecurity,tableCommands:[["RefreshToken","SELECT"],["User","SELECT"],["SupportTicket","SELECT"],["SupportTicket","UPDATE"],["SupportTicketMessage","SELECT"],["Incident","SELECT"],["IncidentHandoff","SELECT"],["AuditLog","INSERT"]],context:"Locks and updates one support ticket under live platform capability and recent MFA, then appends audit evidence atomically.",canonicalWorkflowIds:["workflow-internal-backend-src-rls-waves-session-b-b03-repository-functions-ts-update-support-ticket"],repositoryCallers:["backend/src/rls-waves/session-b/b03/repositoryFunctions.ts:updateSupportTicket"],inputAuthority:"authenticated capability and recent MFA; target and assignee IDs are selection-only",outputColumns:["result"],disposableProbes:["b03-authenticated-postgres18"],
  },
  {
    id:"b03-add-support-ticket-message",schema:"app_rls",name:"b03_add_support_ticket_message",signature:"text,text,boolean,timestamp without time zone,text",returnType:"jsonb",identityArguments:"p_ticket_id text, p_message text, p_is_internal boolean, p_created_at timestamp without time zone, p_request_id text",definitionLocation:b03AuthenticatedSource,definitionKind:"checked-in-production-package",definitionStatus:"production-reviewed",security:b03AuthenticatedSecurity,tableCommands:[["RefreshToken","SELECT"],["User","SELECT"],["SupportTicket","SELECT"],["SupportTicketMessage","INSERT"],["AuditLog","INSERT"]],context:"Appends one bounded support message and matching audit evidence atomically under live platform capability and recent MFA.",canonicalWorkflowIds:["workflow-internal-backend-src-rls-waves-session-b-b03-repository-functions-ts-add-support-ticket-message"],repositoryCallers:["backend/src/rls-waves/session-b/b03/repositoryFunctions.ts:addSupportTicketMessage"],inputAuthority:"authenticated capability and recent MFA; ticket ID is selection-only",outputColumns:["result"],disposableProbes:["b03-authenticated-postgres18"],
  },
  {
    id:"b03-attention-queue",schema:"app_rls",name:"b03_attention_queue_projection",signature:"text,timestamp without time zone,text",returnType:"jsonb",identityArguments:"p_licensee_id text, p_since timestamp without time zone, p_request_id text",definitionLocation:b03AuthenticatedSource,definitionKind:"checked-in-production-package",definitionStatus:"production-reviewed",security:b03AuthenticatedSecurity,tableCommands:[["RefreshToken","SELECT"],["User","SELECT"],["Licensee","SELECT"],["Organization","SELECT"],["ManufacturerLicenseeLink","SELECT"],["Incident","SELECT"],["QRCode","SELECT"],["Batch","SELECT"],["QrScanLog","SELECT"],["PolicyAlert","SELECT"],["SupportTicket","SELECT"],["AuditLog","SELECT"]],context:"Returns exact dashboard attention counts and latest-row projections under live capability, tenant and manufacturer scope.",canonicalWorkflowIds:["workflow-http-backend-src-services-attention-queue-service-ts-get-attention-queue-snapshot-uncached"],repositoryCallers:["backend/src/rls-waves/session-b/b03/repositoryFunctions.ts:readAttentionQueueProjection"],inputAuthority:"authenticated capability and live database scope; licensee ID only narrows platform or linked authority",outputColumns:["result"],disposableProbes:["b03-authenticated-postgres18"],
  },
  {
  ...
  {
    id: "b01-lookup-invitation", schema: "app_auth", name: "lookup_invitation_token", signature: "text[],timestamp without time zone", returnType: "TABLE(email text, role text, expiresAt timestamp without time zone, licenseeName text, requiresConnector boolean, inviteId text, userId text, challengeId text, challengeCreatedAt timestamp without time zone)",
    ...
    tableCommands: [["Invite","SELECT"],["User","SELECT"],["Licensee","SELECT"],["Organization","SELECT"],["InviteActivationChallenge","SELECT"]], context: "Returns a minimal preview or pending challenge after exact-one bearer, bound invited-user, role-family and live tenant validation.",
    canonicalWorkflowIds: ["workflow-internal-backend-src-services-auth-invite-service-ts-get-invite-preview"], repositoryCallers: ["backend/src/rls-waves/session-b/b01/preAuthRepository.ts:lookupInvitationBoundary"], inputAuthority: "hashed invite bearer; all user and tenant data are derived from its stored relationship", outputColumns: ["email","role","expiresAt","licenseeName","requiresConnector","inviteId","userId","challengeId","challengeCreatedAt"], disposableProbes: ["b01-preauth-security-postgres18"],
  },
  {
    id: "b01-consume-invitation", schema: "app_auth", name: "consume_invitation_token", signature: "text[],text,text,timestamp without time zone,text,text,text,text,text,timestamp without time zone", returnType: "TABLE(inviteId text, id text, email text, name text, role text, licenseeId text, orgId text, status text, challengeId text, challengeExpiresAt timestamp without time zone)",
    identityArguments: "p_token_hash_candidates text[], p_new_password_hash text, p_requested_name text, p_consumed_at timestamp without time zone, p_request_id text, p_ip_hash text, p_user_agent text, p_challenge_id text, p_code_verifier text, p_expires_at timestamp without time zone", definitionLocation: preAuthSource, definitionKind: "checked-in-production-package", definitionStatus: "production-reviewed", security: preAuthSecurity,
    tableCommands: [["Invite","SELECT"],["Invite","UPDATE"],["User","SELECT"],["User","UPDATE"],["Licensee","SELECT"],["Organization","SELECT"],["InviteActivationChallenge","INSERT"],["AuditLogOutbox","INSERT"]], context: "Locks one unused invite and bound pending user, sets the password while retaining INVITED state, consumes the bearer and creates one activation challenge atomically.",
    canonicalWorkflowIds: ["workflow-internal-backend-src-services-auth-invite-service-ts-accept-invite"], repositoryCallers: ["backend/src/rls-waves/session-b/b01/preAuthRepository.ts:consumeInvitationBoundary"], inputAuthority: "hashed invitation bearer; request metadata is attribution only", outputColumns: ["inviteId","id","email","name","role","licenseeId","orgId","status","challengeId","challengeExpiresAt"], disposableProbes: ["b01-preauth-security-postgres18"],
  },
  {
    id: "b01-lookup-invite-activation", schema: "app_auth", name: "lookup_invite_activation_challenge", signature: "text", returnType: "TABLE(challengeId text, userId text, inviteId text, email text)",
    identityArguments: "p_challenge_id text", definitionLocation: preAuthSource, definitionKind: "checked-in-production-package", definitionStatus: "production-reviewed", security: preAuthSecurity,
    tableCommands: [["InviteActivationChallenge","SELECT"]], context: "Resolves a live opaque challenge to server-side binding only for verifier construction.", canonicalWorkflowIds: [], repositoryCallers: ["backend/src/rls-waves/session-b/b01/preAuthRepository.ts:lookupInviteActivationBinding"], inputAuthority: "opaque challenge identifier is lookup-only; stored binding is authoritative", outputColumns: ["challengeId","userId","inviteId","email"], disposableProbes: ["b01-preauth-security-postgres18"],
  },
  {
    id: "b01-verify-invite-activation", schema: "app_auth", name: "verify_invite_activation", signature: "text,text[],timestamp without time zone", returnType: "TABLE(verified boolean, userId text, email text, role text)",
    identityArguments: "p_challenge_id text, p_verifier_candidates text[], p_verified_at timestamp without time zone", definitionLocation: preAuthSource, definitionKind: "checked-in-production-package", definitionStatus: "production-reviewed", security: preAuthSecurity,
    tableCommands: [["InviteActivationChallenge","SELECT"],["InviteActivationChallenge","UPDATE"],["Invite","SELECT"],["User","SELECT"],["User","UPDATE"],["AuditLogOutbox","INSERT"]], context: "Locks the challenge and pending user; failed attempts commit and success consumes the challenge and activates the account atomically.", canonicalWorkflowIds: [], repositoryCallers: ["backend/src/rls-waves/session-b/b01/preAuthRepository.ts:verifyInviteActivationBoundary"], inputAuthority: "challenge identifier and versioned verifier candidates; stored user/invite/email binding is authority", outputColumns: ["verified","userId","email","role"], disposableProbes: ["b01-preauth-security-postgres18"],
  },
  {
    id: "b01-resend-invite-activation", schema: "app_auth", name: "resend_invite_activation", signature: "text,text,text,timestamp without time zone,timestamp without time zone", returnType: "TABLE(challengeId text, email text, userId text, inviteId text)",
    identityArguments: "p_challenge_id text, p_new_challenge_id text, p_code_verifier text, p_requested_at timestamp without time zone, p_expires_at timestamp without time zone", definitionLocation: preAuthSource, definitionKind: "checked-in-production-package", definitionStatus: "production-reviewed", security: preAuthSecurity,
    tableCommands: [["InviteActivationChallenge","SELECT"],["InviteActivationChallenge","UPDATE"],["InviteActivationChallenge","INSERT"],["Invite","SELECT"],["User","SELECT"]], context: "After cooldown, locks the pending account and atomically supersedes the old challenge with a new bounded code.", canonicalWorkflowIds: [], repositoryCallers: ["backend/src/rls-waves/session-b/b01/preAuthRepository.ts:resendInviteActivationBoundary"], inputAuthority: "opaque existing challenge; stored binding fixes destination and account", outputColumns: ["challengeId","email","userId","inviteId"], disposableProbes: ["b01-preauth-security-postgres18"],
  },
  {
  ...
  },
  {
    ...
    identityArguments: "p_user_id text, p_organization_id text, p_token_hash text, p_expires_at timestamp without time zone, p_ip_hash text, p_user_agent text, p_authenticated_at timestamp without time zone, p_mfa_verified_at timestamp without time zone, p_created_at timestamp without time zone", definitionLocation: authenticationClosureSource, definitionKind: "checked-in-production-package", definitionStatus: "production-reviewed", security: { ...authenticationClosureSecurity, runtimeExecuteGrantees: ["preauth","app"] },
    tableCommands: [["User","SELECT"],["Licensee","SELECT"],["Organization","SELECT"],["ManufacturerLicenseeLink","SELECT"],["RefreshToken","INSERT"],["AuditLogOutbox","INSERT"]], context: "Creates one refresh credential and login audit for either the password-verified pre-auth subject or the same live capability-bound actor during session replacement; no GUC or input can redirect user or tenant authority.", canonicalWorkflowIds: ["workflow-internal-backend-src-services-auth-refresh-token-service-ts-create-refresh-token"], repositoryCallers: ["backend/src/rls-waves/session-b/b01/sessionCredentialRepository.ts:createRefreshTokenRecord"], inputAuthority: "trusted password-verification subject or same live authenticated capability actor plus database-derived organization scope", outputColumns: ["id","expiresAt"], disposableProbes: ["b01-authentication-closure-postgres18"],
  },
  {
  ...
  {
    id: "b01-record-login-risk", schema: "app_rls", name: "record_auth_session_risk_signal", signature: "integer,text,text[],text,text,timestamp without time zone,text,text,text,timestamp without time zone,integer,boolean,text", returnType: "TABLE(recorded boolean, challengeCreated boolean)",
    identityArguments: "p_risk_score integer, p_risk_level text, p_reasons text[], p_ip_hash text, p_user_agent_hash text, p_recorded_at timestamp without time zone, p_password_hash text, p_challenge_ticket_hash text, p_challenge_session_hash text, p_challenge_expires_at timestamp without time zone, p_challenge_max_attempts integer, p_blocked_login boolean, p_request_id text", definitionLocation: authenticationClosureSource, definitionKind: "checked-in-production-package", definitionStatus: "production-reviewed", security: { ...authenticationClosureSecurity, runtimeExecuteGrantees: ["preauth"] },
    tableCommands: [["User","SELECT"],["User","UPDATE"],["AuthSessionRiskSignal","INSERT"],["MfaLoginChallenge","INSERT"],["AuditLogOutbox","INSERT"]], context: "Atomically records the bounded risk result, successful-login state reset, optional password rehash, optional actor-bound MFA login challenge and exact platform-admin blocked-risk audit.", canonicalWorkflowIds: ["workflow-internal-backend-src-services-auth-session-risk-service-ts-assess-auth-session-risk"], repositoryCallers: ["backend/src/rls-waves/session-b/b01/authenticatedSecurityRepository.ts:recordAuthSessionRiskSignal"], inputAuthority: "password-verified subject bound by app_auth.lookup_password_user; challenge, blocked-risk audit and request values cannot redirect the actor", outputColumns: ["recorded","challengeCreated"], disposableProbes: ["b01-authentication-closure-postgres18"],
  },
  {
  ...
  },
  {
    ...
    security: { ...b01Security, functionSource: authenticatedSessionSource, rollbackDefinition: authenticatedSessionRollback, runtimeExecuteGrantees: ["preauth","app"] },
    tableCommands: [["RefreshToken", "SELECT"], ["RefreshToken", "UPDATE"], ["User", "SELECT"]],
    context: "Issues a capability only for the locked active refresh-session row after the caller presents its exact bearer-derived refresh hash; authenticated session replacement additionally requires the same live capability-bound actor.",
