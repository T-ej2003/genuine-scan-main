...
const exactManagedMembershipStateSql = `
  IF (SELECT count(*) FROM pg_auth_members m JOIN pg_roles parent ON parent.oid=m.roleid WHERE parent.rolname IN (${managedRoleList}))<>${roleSpecs.length}
     OR EXISTS (SELECT 1 FROM pg_auth_members m JOIN pg_roles parent ON parent.oid=m.roleid JOIN pg_roles member ON member.oid=m.member WHERE parent.rolname IN (${managedRoleList}) AND (member.rolname<>${lit(administrativeExecutorRole)} OR m.admin_option OR m.inherit_option OR NOT m.set_option))
     OR EXISTS (SELECT 1 FROM pg_roles parent WHERE parent.rolname IN (${managedRoleList}) AND (SELECT count(*) FROM pg_auth_members m JOIN pg_roles member ON member.oid=m.member WHERE m.roleid=parent.oid AND member.rolname=${lit(administrativeExecutorRole)} AND NOT m.admin_option AND NOT m.inherit_option AND m.set_option)<>1)
...
DO $$ DECLARE existing_count integer; rec record; BEGIN
  ...
  IF (SELECT count(*) FROM pg_auth_members m JOIN pg_roles parent ON parent.oid=m.roleid WHERE parent.rolname IN (${managedRoleList}))<>${roleSpecs.length}
     OR EXISTS (SELECT 1 FROM pg_auth_members m JOIN pg_roles parent ON parent.oid=m.roleid JOIN pg_roles member ON member.oid=m.member WHERE parent.rolname IN (${managedRoleList}) AND (member.rolname<>current_user OR m.admin_option OR m.inherit_option OR NOT m.set_option))
     OR EXISTS (SELECT 1 FROM pg_roles parent WHERE parent.rolname IN (${managedRoleList}) AND (SELECT count(*) FROM pg_auth_members m JOIN pg_roles member ON member.oid=m.member WHERE m.roleid=parent.oid AND member.rolname=current_user AND NOT m.admin_option AND NOT m.inherit_option AND m.set_option)<>1)
     OR EXISTS (SELECT 1 FROM pg_auth_members m JOIN pg_roles member ON member.oid=m.member WHERE member.rolname IN (${managedRoleList}))
