...
usage() {
  printf '%s\n' \
    'Usage: scripts/deploy-standby.sh [mumbai|capetown|standby|standby_regions] [inventory]' \
    '' \
    'Defaults:' \
    '  limit:     standby' \
...
PLAYBOOK="ops/deploy/deploy.yml"
...
if [ "$LIMIT" = "standby_regions" ] && ! has_inventory_group standby_regions; then
  echo "Inventory does not define [standby_regions]. Add this alias or use: scripts/deploy-standby.sh standby" >&2
...
if [ "$LIMIT" = "standby" ] && ! has_inventory_group standby; then
  echo "Inventory does not define [standby]. Add this group or use: scripts/deploy-standby.sh standby_regions" >&2
