...

{
  ...
  printf '- Instance target: `%s` on HTTP port 80\n' "$instance_id"
  printf '- VPC: `%s`\n' "$vpc_id"
  printf '- Candidate ALB subnets: `%s`\n' "$subnet_ids"
  printf '- Existing instance security groups: `%s`\n' "$security_groups"
  printf '- ALB name: `%s`\n' "$ALB_NAME"
  printf '- Target group: `%s`\n' "$TARGET_GROUP_NAME"
  printf '- ALB security group: `%s`\n' "$ALB_SECURITY_GROUP_NAME"
  printf '- Health check path: `%s`\n\n' "$HEALTH_CHECK_PATH"
