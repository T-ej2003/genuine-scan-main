...
export function buildWebImageEvidence({ publicationIdentity, repositoryEvidence, imageReadback, imageImpactSha256, createdAt, expiresAt } = {}) {
  if (!publicationIdentity || publicationIdentity.sourceSha !== publicationIdentity.workflowDefinitionSha || !HASH.test(publicationIdentity.canonicalArtifactSha256 || "")) throw new Error("Web publication identity is invalid.");
...
export function assertWebImageEvidence(evidence, { signature, verify, now = new Date().toISOString() } = {}) {
  if (evidence?.schemaVersion !== 1 || evidence.operation !== WEB_RELEASE.operation || !SHA.test(evidence.sourceSha || "") || evidence.account !== WEB_RELEASE.account || evidence.region !== WEB_RELEASE.region || evidence.repository !== WEB_RELEASE.repository || evidence.platform !== WEB_RELEASE.platform || evidence.dockerfile !== WEB_RELEASE.dockerfile || evidence.buildContext !== WEB_RELEASE.buildContext || !DIGEST.test(evidence.imageDigest || "") || !IMAGE.test(evidence.imageRef || "") || !HASH.test(evidence.imageImpactSha256 || "") || evidence.evidenceSha256 !== canonicalSha256(evidencePayload(evidence)) || evidence.publicationIdentity?.sourceSha !== evidence.sourceSha || evidence.publicationIdentitySha256 !== canonicalSha256(evidence.publicationIdentity) || evidence.publicationArtifactSha256 !== evidence.publicationIdentity?.canonicalArtifactSha256 || evidence.imageDigest !== evidence.publicationIdentity?.record?.image_digest || evidence.reviewer !== evidence.publicationIdentity?.reviewer) throw new Error("Web image evidence is invalid.");
