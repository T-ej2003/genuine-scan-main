...
export function assertStageAProductionArtifactsJournalRetention(lifecycle, journalPrefixes = ["production-stage-a-production-artifacts-reconciliation/", PRODUCTION_ACTIVATION_LIFECYCLE.initialActivationPolicyReconciliationReservationPrefix, PRODUCTION_ACTIVATION_LIFECYCLE.providerReadonlyPolicyReconciliationPrefix]) {
...
export async function runStageAProductionArtifactsRecovery({ sourceSha, recoverySourceSha = sourceSha, workflowRunId, workflowRunAttempt, rootRun, releaseRun, readStateIdentity, terraformStateLock, resolveAuthorization = resolveStageAProductionArtifactsAuthorizationArtifact, journal, recoveryJournal = journal, rootRecoveryJournal = recoveryJournal, sign, verify, readProtectedSource = readStageBProtectedMainCheckout, proveDescendant = proveProtectedMainDescendant, readGovernedExecutableManifestSha256 } = {}) {
  ...
  const transition = assertStageAProductionArtifactsExecutableTransition(authorization);
  ...
  const legacyReservationRepair = samePolicy(transition.predecessor, stageAProductionArtifactsLegacyReservationRepairTransition().predecessor) && samePolicy(transition.desired, buildStageAProductionArtifactsBucketPolicyWithProviderReadonlyJournalProtection());
  if (!historicalTransition && !reservationTransition && !legacyReservationTransition && !bootstrapTransition && !providerReadonlyTransition && !reverseReservationTransition && !legacyReservationRepair) throw new Error("Stage A production-artifacts recovery transition is unsupported.");
  ...
  const readAttempt = () => {
    try { return attemptReader.readRecoveryAttempt(authorization.authorizationSha256); }
    catch (error) {
      if (!reservationTransition || attemptReader !== recoveryJournal || !/AccessDenied|403/i.test(`${error.message || ""}\n${error.stderr || ""}`)) throw error;
  ...
  if (!attempt) {
    ...
    attemptWriter.writeRecoveryAttempt({ recoveryAuthorizationSha256: authorization.authorizationSha256, bytes: Buffer.from(`${JSON.stringify(attempt)}\n`) });
    createdAttempt = true;
  }
  ...
  try {
    ...
    if (postWriteContinuation) {
      const finalAttempt = decode(attemptWriter.readRecoveryAttempt(authorization.authorizationSha256), "Stage A recovery attempt");
