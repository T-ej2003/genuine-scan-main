...
export async function executeBrokerPolicySuccessor({ authorization, packageEvidence, operatorProof }, { iam, lambda, s3, authenticate, now = Date.now, sleep = delay,
  ...
  await authorize(); const journal = await readJournal();
  if (Object.hasOwn(journal.metadata, brokerPolicySuccessorMetadataKey)) { assertBrokerPolicySuccessorClosureMetadata(journal.metadata, bindings); assert.fail("Broker-policy successor is already closed"); }
