...
function assertBindings({ sourceSha, tfvars, binding, preflight, checkPreflight = true, validatePrerequisiteFiles = true }) {
...
export function runStageBStateReconciliation(argv = process.argv.slice(2), deps = {}) {
  ...
  if (prepare) {
    ...
    assertSource(sourceSha); const originalTfvarsPath = required(argv, "--tfvars"); const originalBindingPath = required(argv, "--binding-report"); const originalBindings = assertBindings({ sourceSha, tfvars: originalTfvarsPath, binding: originalBindingPath, preflight: required(argv, "--release-preflight"), validatePrerequisiteFiles: false });
  ...
  const originalTfvarsPath = required(argv, "--tfvars"); const originalBindingPath = required(argv, "--binding-report"); const originalBindings = assertBindings({ sourceSha, tfvars: originalTfvarsPath, binding: originalBindingPath, preflight: required(argv, "--release-preflight"), validatePrerequisiteFiles: false });
