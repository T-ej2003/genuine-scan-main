...
function assertBindings({ sourceSha, tfvars, binding, preflight }) {
  ...
  if (release.status !== "ready-for-plan" || release.sourceSha !== sourceSha || release.tfvarsSha256 !== checked.tfvarsSha256 || release.bindingReportSha256 !== hash(bindingBytes)) throw new Error("Stage B state reconciliation release preflight binding is invalid.");
...
export function runStageBStateReconciliation(argv = process.argv.slice(2), deps = {}) {
  ...
  if (prepare) {
    ...
    assertSource(sourceSha); const bindings = assertBindings({ sourceSha, tfvars: required(argv, "--tfvars"), binding: required(argv, "--binding-report"), preflight: required(argv, "--release-preflight") });
