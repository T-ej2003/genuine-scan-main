...
export function runStageBStateReconciliation(argv = process.argv.slice(2), deps = {}) {
  ...
  const allowed = prepare ? new Set(["--mode", "--source-sha", "--ticket-id", "--admin-profile", "--credential-source", "--tfvars", "--binding-report", "--release-preflight", "--release-preflight-sha256", "--prerequisite-bundle", "--prerequisite-producer-workflow-run-id", "--prerequisite-producer-workflow-run-attempt", "--prerequisite-bundle-artifact-id", "--prerequisite-bundle-artifact-digest", "--terraform-data-dir", "--saved-plan-out", "--preparation-out"]) : new Set(["--mode", "--source-sha", "--tfvars", "--binding-report", "--release-preflight", "--release-preflight-sha256", "--prerequisite-bundle", "--prerequisite-producer-workflow-run-id", "--prerequisite-producer-workflow-run-attempt", "--terraform-data-dir", "--saved-plan", "--saved-plan-sha256", "--preparation", "--preparation-file-sha256", "--authorization", "--authorization-file-sha256", "--result-out"]);
  ...
  if (prepare) {
    ...
    const runtime = writeStageBRuntimeMaterialization({ originalTfvarsBytes: fs.readFileSync(path.resolve(originalTfvarsPath)), originalBindingBytes: fs.readFileSync(path.resolve(originalBindingPath)), prerequisite });
  ...
  const originalTfvarsPath = required(argv, "--tfvars"); const originalBindingPath = required(argv, "--binding-report"); const originalBindings = assertBindings({ sourceSha, tfvars: originalTfvarsPath, binding: originalBindingPath, preflight: required(argv, "--release-preflight"), preflightSha256: required(argv, "--release-preflight-sha256"), validatePrerequisiteFiles: false });
