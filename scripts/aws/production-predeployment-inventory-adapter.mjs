...
export function createProductionPreDeploymentInventoryAdapter({ run, sourceSha, imageDigest, config } = {}) {
  ...
  return {
    async run({ rotationId } = {}) {
      ...
      const registered = await registerPreDeploymentInventoryTaskDefinition({
        input: { ...taskInput },
      ...
      try {
        ...
        const invocation = parseJson(run, ["lambda", "invoke", "--function-name", STAGE_B.brokerAliasArn, "--invocation-type", "RequestResponse", "--cli-binary-format", "raw-in-base64-out", "--payload", JSON.stringify(brokerRequest), outputPath]);
