...
export function commitRotationComponentState({ mode, sourceSha, rotationId, rotationStateSha256, readinessFile, readinessSha256, client, isProtectedMainAncestor = () => true, writerContext } = {}) {
  ...
  return advanceProductionComponentDeploymentStateWithRetry({ client, current, lane: "SECURITY_INFRASTRUCTURE", changes: { security: { sourceSha, releaseIdentity } }, ...writerContext });
}
...
function main() {
  ...
  assert.deepEqual(Object.keys(values).sort(), ["mode", "readiness", "readiness-sha256", "rotation-id", "rotation-state-sha256", "source-sha"]);
