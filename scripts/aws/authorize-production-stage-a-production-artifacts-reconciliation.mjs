...
import { createStageAProductionArtifactsReconciliationAuthorization, resolveStageAProductionArtifactsAuthorizationArtifact, STAGE_A_PRODUCTION_ARTIFACTS_RECOVERY_OPERATION } from "./production-stage-a-production-artifacts-recovery-governance.mjs";
...
export function authorizeStageAProductionArtifactsReconciliation(argv = process.argv.slice(2), { verifyRecoveryCompletionEvidence, resolveRecoveryAuthorization = resolveStageAProductionArtifactsAuthorizationArtifact, journal } = {}) {
  ...
  const recovery = resolveRecoveryAuthorization({ workflowRunId: required(argv, "--recovery-authorization-workflow-run-id"), workflowRunAttempt: required(argv, "--recovery-authorization-workflow-run-attempt"), sourceSha: required(argv, "--source-sha"), operation: STAGE_A_PRODUCTION_ARTIFACTS_RECOVERY_OPERATION });
  ...
  const authorization = createStageAProductionArtifactsReconciliationAuthorization({ sourceSha: required(argv, "--source-sha"), preState, recoveryAuthorization: recovery.authorization, recoveryCompletion, prepareEvidence, savedPlanSha256: required(argv, "--saved-plan-sha256"), protectedEnvironmentApprovalEvidence: read("--environment-approval"), verificationRef: required(argv, "--verification-ref"), verifyRecoveryCompletionEvidence });
