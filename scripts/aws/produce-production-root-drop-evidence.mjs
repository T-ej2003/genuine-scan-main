...
for (let i = 0; i < process.argv.slice(2).length; i += 2) {
  ...
  if (!["source-sha", "output", "profile", "nonce"].includes(key) || !value || args.has(key)) throw new Error(`Invalid or duplicate argument: --${key}`);
  args.set(key, value);
}
for (const key of ["output", "profile"]) if (!args.get(key)) throw new Error(`--${key} is required; the operator credential context must be explicit.`);
