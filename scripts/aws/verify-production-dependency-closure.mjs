...
const MODE_CAPABILITIES = Object.freeze({
  ...
  [MIXED_DUAL_SLOT_RECOVERY_MODE]: ["mixed-recovery-iam-preflight-identify", "mixed-recovery-iam-preflight-read-role", "mixed-recovery-iam-preflight-simulate", "mixed-recovery-iam-preflight-read-resource-policy", "mixed-recovery-remove-awscurrent"],
...
export function assertChangedAwsCallClosure(scanned, graph) {
  ...
  return CALLS.map((contract) => {
    ...
    const releaseProbe = contract.identity !== "RELEASE_DEPLOYER"
      ...
      || contract.capabilityId === "mixed-recovery-remove-awscurrent" && capability?.probe === "administrator-simulation";
    ...
    const reachableMode = stageAModes
      ...
      : contract.sourceFile.endsWith("preflight-production-mixed-dual-slot-recovery-iam.mjs") || contract.sourceFile.endsWith("recover-production-mixed-dual-slot-topology.mjs") ? [MIXED_DUAL_SLOT_RECOVERY_MODE]
