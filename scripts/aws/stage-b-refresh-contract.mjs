...
export function classifyStageBRefreshResult({ plan, terraformExitCode = 0, terraformOutput = "", bindingReport, state, outputsSource } = {}) {
  try {
    ...
    if (terraformExitCode === 2 && (plan.complete !== true || plan.errored !== false)) return { status: "MALFORMED_RESULT", reason: "Terraform detailed exit code requires a complete, non-errored refresh plan.", checkCount: 0, passedCheckCount: 0, failedCheckCount: 0, malformedCheckCount: 0, failedChecks: [], checks: [], resourceChanges: { nonNoOp: 0, changes: [] }, outputChanges: [] };
