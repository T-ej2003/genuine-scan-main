...
const REVIEWED_VARIABLE_VALIDATION_NAMES = Object.freeze(["backend_client_ip_cloudfront_prefix_list_id", "backend_client_ip_cloudfront_prefix_list_version", "backend_client_ip_trust_mode", "backend_client_ip_trusted_alb_cidrs", "backend_client_ip_trusted_cloudfront_cidrs", "production_rotation_secret_value_from", "retained_candidate_task_definitions", "retained_executor_task_definitions", "stage_b_recovery_alias_target_version"]);
...
export function assertStageBRecoveryProvenance({ refreshReport, refreshReportSha256, observationBindingReport, observationBindingReportSha256, recoveryBindingReport, recoveryBindingReportSha256, recoveryClassificationSha256, recoveryAttestationSha256, recoveryMode = "RECOVERY_ALIAS_ONLY" } = {}) {
  ...
  if (!/^[a-f0-9]{64}$/.test(recoveryBindingReportSha256 || "") || recoveryBindingReportSha256 === observationBindingReportSha256) throw new Error("Stage B recovery planning requires a distinct recovery binding artifact.");
  ...
  if (["PARTIAL_APPLY_RECOVERY", "FRESH_IMAGE_PARTIAL_APPLY_RECOVERY"].includes(recoveryMode) && (recoveryBindingReport.recoveryObservationBindingSha256 !== observationBindingReportSha256 || recoveryBindingReport.recoveryStateLineage !== recoveryBindingReport.stateLineage || recoveryBindingReport.recoveryStateSerial !== recoveryBindingReport.stateSerial)) throw new Error("Stage B partial recovery binding is not bound to the exact observation/state identity.");
