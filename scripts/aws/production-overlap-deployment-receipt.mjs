...
const RECEIPT_BOUNDARY_STEPS = Object.freeze(["Authenticate production environment approval boundary", "Deploy rotation transition backend ECS service", "Upload overlap deployment receipt"]);

export function assertReceiptBoundarySteps(steps) {
  if (!Array.isArray(steps)) throw new Error("Overlap deployment job steps are malformed.");
  const boundary = RECEIPT_BOUNDARY_STEPS.map((name) => {
    const matches = steps.map((step, index) => ({ step, index })).filter(({ step }) => step?.name === name);
    if (matches.length !== 1 || matches[0].step.status !== "completed" || matches[0].step.conclusion !== "success") throw new Error("Overlap deployment job did not complete every authenticated receipt boundary.");
    return matches[0];
  });
  if (!(boundary[0].index < boundary[1].index && boundary[1].index < boundary[2].index)) throw new Error("Overlap deployment receipt boundaries are not strictly ordered.");
  return boundary;
}
...
export function buildProductionOverlapDeploymentReceipt(input = {}) {
  ...
  if (body.updateServiceCount === 0 && input.deployment?.disposition !== "ALREADY_APPLIED") throw new Error("Zero-mutation receipt requires authenticated reconciliation.");
...
export function assertProductionOverlapDeploymentReceipt(value, expected = {}) {
  ...
  if (![value.expectedCurrentTaskDefinitionArn, value.taskDefinitionArn, value.observedTaskDefinitionArn].every((item) => ARN.test(item || "")) || value.observedTaskDefinitionArn !== value.taskDefinitionArn || value.cluster !== "mscqr-prod-euw2-main" || value.service !== "mscqr-backend-servi-euw2" || !/^sha256:[a-f0-9]{64}$/.test(value.imageDigest || "") || value.observedImageDigest !== value.imageDigest || !SHA40.test(value.deploymentSha || "") || value.serviceStable !== true || ![0, 1].includes(value.updateServiceCount) || Number.isNaN(Date.parse(value.deployedAt)) || new Date(value.deployedAt).toISOString() !== value.deployedAt) throw new Error("Overlap deployment receipt did not prove one stable exact deployment or read-only reconciliation.");
