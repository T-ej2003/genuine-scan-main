...
const init = ({ exec, env, data }) => {
  ...
  ensureStageBPrivateFile({ filePath: path.join(data, "terraform.tfstate"), repositoryRoot: root, normalize: true, label: "State reconciliation backend metadata" });
...
export function runReconcilerStateReconciliation(argv = process.argv.slice(2), deps = {}) {
  ...
  const normal = () => { const planPath = path.join(path.dirname(savedPath), `.post-refresh-${crypto.randomUUID()}.tfplan`); try { exec("terraform", [`-chdir=${path.join(root, CONTRACT.terraformRoot)}`, "plan", "-input=false", "-lock=false", "-out", planPath], { cwd: root, env, encoding: "utf8", stdio: ["ignore", "pipe", "pipe"] }); ensureStageBPrivateFile({ filePath: planPath, repositoryRoot: root, normalize: true, label: "State reconciliation post-refresh Terraform plan" }); return render({ exec, env, planPath }); } finally { fs.rmSync(planPath, { force: true }); } };
