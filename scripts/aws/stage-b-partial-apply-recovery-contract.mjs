...
function assertHistoricalEvidence(value) {
  ...
  if (!Number.isInteger(value.preApplySerial) || value.preApplySerial < 0) throw new Error("Historical pre-apply serial is malformed.");
...
export function assertRecoveryAttestation(report, { expected = {}, now = new Date() } = {}) {
  ...
  for (const [field, value] of Object.entries(expected)) if (value !== undefined && current[field] !== value) throw new Error(`Recovery attestation ${field} binding mismatch.`);
  return current;
}
