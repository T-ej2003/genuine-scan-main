...
import { B01_PREREQUISITE, assertB01LivePredecessor, assertB01RunTaskRequestEvidence, assertSemanticallyEmptyB01TaskOverrides,
  attestBridgeDiff, buildB01ReadOnlyDefinition, buildB01RunTaskRequest, canonicalSha256 } from "./production-b01-prerequisite-contract.mjs";
import { authenticateB01RunTaskCloudTrail, buildB01ReadOnlyInput } from "./apply-production-b01-prerequisite.mjs";
...
export function authenticateB01ReadOnlyResult(message, contract) {
  ...
  if (body.classification === "UNKNOWN") return Object.freeze(value);
...
export async function probeProductionB01Prerequisite({ deploymentSourceSha, awsProfile, repositoryRoot = root,
  ...
  const built = buildB01ReadOnlyInput({ deploymentSourceSha, databaseHostname: database.Endpoint?.Address, repositoryRoot });
  ...
  return Object.freeze({ status:"PRODUCTION_B01_READONLY_RECONCILED",taskArn,taskDefinitionArn,requestEvidence,classification:result.classification,liveRlsIdentity:result.liveRlsIdentity||null,
...
if (process.argv[1] && pathToFileURL(path.resolve(process.argv[1])).href===import.meta.url) {
  try { const {values}=parseArgs({options:{"deployment-source-sha":{type:"string"},"aws-profile":{type:"string"}},strict:true});
    const result=await probeProductionB01Prerequisite({deploymentSourceSha:values["deployment-source-sha"],awsProfile:values["aws-profile"]}); process.stdout.write(`${JSON.stringify(result)}\n`);
