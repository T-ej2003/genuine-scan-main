...
export async function establishEcsExecVerifierSession({ adapter, mfaSerial, mfaCode, getMfaCode, sessionName = "mscqr-production-ecs-exec-verifier" } = {}) {
  ...
  if (typeof mfaSerial !== "string" || !mfaSerial || (getMfaCode !== undefined && typeof getMfaCode !== "function")) throw new Error("MFA serial and code are required for verifier session establishment.");
  ...
  try {
    ...
    const assumed = await adapter.assumeRole({ roleArn: ECS_EXEC_OPERATOR_ROLE_ARN, sessionName, mfaSerial, mfaCode: code });
    code = "";
