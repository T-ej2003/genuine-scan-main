...
export function runMixedDualSlotRecoveryAuthorizationCli(argv = process.argv.slice(2), { verifyIamCapabilityAttestation = createPinnedRootAttestationVerifier() } = {}) {
  ...
  const iamAttestation = readJson(required(argv, "--iam-preflight-attestation"), "Mixed recovery IAM attestation");
  if (iamAttestation.sha256 !== required(argv, "--iam-preflight-attestation-file-sha256")) throw new Error("Mixed recovery IAM attestation bytes changed after review.");
  assertMixedDualSlotRecoveryIamAttestation(iamAttestation.value, { preflight: preparation.value.iamCapabilityPreflight, sourceSha, verify: verifyIamCapabilityAttestation });
  ...
  const authorization = createMixedDualSlotRecoveryAuthorization({ preparation: preparation.value, preparationFileSha256: preparation.sha256, iamCapabilityAttestation: iamAttestation.value, iamCapabilityAttestationFileSha256: iamAttestation.sha256, verifyIamCapabilityAttestation, protectedEnvironmentApprovalEvidence: approval.value, reason: required(argv, "--reason"), approverRole: required(argv, "--approver-role"), verificationRef: required(argv, "--verification-ref") });
