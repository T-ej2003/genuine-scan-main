...
export function assertReleaseTrainNormalDispatchContract(workflow = yaml.load(fs.readFileSync(path.join(root, PRODUCTION_RELEASE_TRAIN_WORKFLOW_PATH), "utf8"))) {
  ...
  const run = step?.run || "";
  if (!run.includes("production-release-dispatch-contract.mjs") || !run.includes("dispatch-source-bound-workflow.mjs") || !run.includes("--workflow release-gate.yml") || !run.includes('["release_train_run_id", process.env.GITHUB_RUN_ID]') || !run.includes('["preserve_current_frontend", "true"]') || !run.includes('["normal_image_authorization_json", process.env.NORMAL_IMAGE_AUTHORIZATION_JSON]') || !run.includes('["normal_image_authorization_sha256", process.env.NORMAL_IMAGE_AUTHORIZATION_SHA256]')) throw new Error("Release Train does not validate and forward the exact normal Release Gate contract.");
