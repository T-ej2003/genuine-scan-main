...
export function assertReleaseGateProductionIdentity(workflow = yaml.load(fs.readFileSync(path.join(root, PRODUCTION_RELEASE_WORKFLOW_PATH), "utf8"))) {
  ...
  if (!normalInputs?.normal_image_authorization_json || !normalInputs?.normal_image_authorization_sha256 || !normalInputs?.web_image_authorization_json || normalInputs?.preserve_current_frontend) throw new Error("Normal Release Gate lacks authenticated external image authorization inputs or exposes derived frontend behavior.");
  ...
  if (!serialized.includes("verify-production-web-release-authorization.mjs") || !serialized.includes("run-production-web-activation.mjs") || !workflowSerialized.includes("web_required") || !workflowSerialized.includes("Web authorization is forbidden when canonical impact does not require web publication")) throw new Error("Normal Stage-B activation does not enforce the reviewed conditional frontend boundary.");
...
export function assertReleaseTrainNormalDispatchContract(workflow = yaml.load(fs.readFileSync(path.join(root, PRODUCTION_RELEASE_TRAIN_WORKFLOW_PATH), "utf8"))) {
  ...
  if (!inputs?.normal_image_authorization_json?.required || !inputs?.normal_image_authorization_sha256?.required || !inputs?.web_image_authorization_json || inputs?.preserve_current_frontend) throw new Error("Release Train must accept canonical conditional web authorization without exposing frontend preservation as a caller input.");
  const step = workflow?.jobs?.orchestrate?.steps?.find(({ name }) => name === "Trigger final Release Gate");
  const run = step?.run || "";
  if (!run.includes("production-release-dispatch-contract.mjs") || !run.includes("dispatch-protected-main-release-gate.mjs") || !run.includes('--target-ref "$TARGET_REF" --target-sha "$TARGET_SHA"') || !run.includes('["release_train_run_id", process.env.GITHUB_RUN_ID]') || !run.includes('["normal_image_authorization_json", process.env.NORMAL_IMAGE_AUTHORIZATION_JSON]') || !run.includes('["normal_image_authorization_sha256", process.env.NORMAL_IMAGE_AUTHORIZATION_SHA256]') || !run.includes('["web_image_authorization_json", process.env.WEB_IMAGE_AUTHORIZATION_JSON]')) throw new Error("Release Train does not validate and forward the exact normal Release Gate contract.");
