...
function assertObservedMetadata(observed, expectedReleaseSha) {
  ...
  if (!SHA.test(String(observed.headSha || "")) || observed.headSha !== expectedReleaseSha) throw new Error("Observed Stage B publication head SHA is not the protected release SHA.");
...
export function assertObservedStageBImagePublicationMetadata(observed, { expectedReleaseSha } = {}) {
  assertObservedMetadata(observed, expectedReleaseSha);
  return true;
}
...
export function buildStageBImagePublicationIdentity({ observed, artifactBytes, expectedReleaseSha, observedAt = new Date().toISOString() } = {}) {
  ...
  assertStageBImagePublicationIdentity(report, { expectedReleaseSha });
  return Object.freeze(report);
}

export function assertStageBImagePublicationIdentity(identity, { expectedReleaseSha, canonicalArtifactSha256 } = {}) {
  ...
  assertObservedMetadata(identity, expectedReleaseSha);
...
export function readStageBImagePublicationIdentity(identityPath, { identitySha256, expectedReleaseSha, canonicalArtifactBytes } = {}) {
  ...
  assertStageBImagePublicationIdentity(identity, { expectedReleaseSha, canonicalArtifactSha256: sha256(canonicalArtifactBytes) });
...
export function writeStageBImagePublicationIdentity({ observed, artifactBytes, expectedReleaseSha, outputPath, repositoryRoot } = {}) {
  ...
  const identity = buildStageBImagePublicationIdentity({ observed, artifactBytes, expectedReleaseSha });
