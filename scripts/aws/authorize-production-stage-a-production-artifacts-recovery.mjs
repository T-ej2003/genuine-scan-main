...
import { STAGE_A_PRODUCTION_ARTIFACTS_EXECUTABLE_TRANSITIONS, stageAProductionArtifactsExecutableRecoveryTransition } from "./production-stage-a-control-plane.mjs";
...
export function authorizeStageAProductionArtifactsRecovery(argv = process.argv.slice(2)) {
  const transitionId = required(argv, "--transition");
  if (!STAGE_A_PRODUCTION_ARTIFACTS_EXECUTABLE_TRANSITIONS.includes(transitionId)) throw new Error("Stage A production-artifacts recovery transition is not executable.");
  ...
  const authorization = createStageAProductionArtifactsRecoveryAuthorization({ sourceSha: required(argv, "--source-sha"), preState: { lineage: required(argv, "--state-lineage"), serial: Number(required(argv, "--state-serial")), stateSha256: required(argv, "--state-sha256") }, transition: stageAProductionArtifactsExecutableRecoveryTransition(transitionId), protectedEnvironmentApprovalEvidence: JSON.parse(fs.readFileSync(approvalPath, "utf8")), verificationRef: required(argv, "--verification-ref") });
