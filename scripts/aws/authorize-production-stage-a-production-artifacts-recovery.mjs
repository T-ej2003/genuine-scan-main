...
import { STAGE_A_PRODUCTION_ARTIFACTS_TRANSITION, stageAProductionArtifactsRecoveryTransition } from "./production-stage-a-control-plane.mjs";
...
export function authorizeStageAProductionArtifactsRecovery(argv = process.argv.slice(2)) {
  ...
  if (!Object.values(STAGE_A_PRODUCTION_ARTIFACTS_TRANSITION).includes(transitionId)) throw new Error("Stage A production-artifacts recovery transition is not allowlisted.");
  ...
  const authorization = createStageAProductionArtifactsRecoveryAuthorization({ sourceSha: required(argv, "--source-sha"), preState: { lineage: required(argv, "--state-lineage"), serial: Number(required(argv, "--state-serial")), stateSha256: required(argv, "--state-sha256") }, transition: stageAProductionArtifactsRecoveryTransition(transitionId), protectedEnvironmentApprovalEvidence: JSON.parse(fs.readFileSync(approvalPath, "utf8")), verificationRef: required(argv, "--verification-ref") });
