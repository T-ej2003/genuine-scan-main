...
const constructInheritedVerifierAdapters = ({ config, sourceSha, rotationId, runtimeConfigSha256 }) => createProductionCutoverRuntimeComposition({ releaseRun: createProductionCommandRunner({ credentialSource: PRODUCTION_AWS_CREDENTIAL_SOURCE.INHERITED_ECS_EXEC_VERIFIER_SESSION }) }).constructAdapters({ config, sourceSha, rotationId, runtimeConfigSha256, identityMode: "INHERITED_VERIFIER" });
...
export async function verifyProductionCutoverOverlap({ configFile, configSha256, sourceSha, rotationId, workflowRunId, workflowRunAttempt, githubRun = (command, args) => execFileSync(command, args, { cwd: root, encoding: "utf8", stdio: ["ignore", "pipe", "pipe"] }), protectedMain = readFreshProtectedMainIdentity, constructAdapters = constructInheritedVerifierAdapters } = {}) {
  ...
  const preparedStateSha256 = stateValue.phase === "verified" ? stateValue.verification?.preparedStateSha256 : state.sha256;
  ...
  readAndAssertReadyForOverlapDeployment({ filePath: config.readinessEvidenceFile, evidenceSha256: readiness.sha256, sourceSha, rotationId, rotationStateSha256: preparedStateSha256 });
  ...
  for (const [field, expected] of Object.entries({ rotationId, rotationStateSha256: preparedStateSha256, readinessSha256: readiness.sha256, rotationFixtureSha256: fixture.sha256, expectedCurrentTaskDefinitionArn: config.expectedCurrentTaskDefinitionArn, taskDefinitionArn: receipt.taskDefinitionArn, imageDigest: config.backendImageDigest, deploymentSha: config.rotationDeploymentSha })) if (receipt[field] !== expected) throw new Error(`Verifier continuation ${field} binding is wrong.`);
  const adapters = constructAdapters({ config, sourceSha, rotationId, runtimeConfigSha256: configSha256, identityMode: "INHERITED_VERIFIER" });
  ...
  const persistedProof = stateValue.phase === "verified" ? stateValue.overlapRuntime : null;
  const resumeVerified = Boolean(persistedProof);
  if (resumeVerified && (persistedProof.rotationId !== rotationId || persistedProof.phase !== "overlap" || persistedProof.deploymentSha !== receipt.deploymentSha || persistedProof.targetTaskDefinitionArn !== receipt.taskDefinitionArn || persistedProof.targetImageDigest !== receipt.imageDigest || typeof persistedProof.targetTaskArn !== "string" || persistedProof.serviceHealthy !== true || stateValue.verification?.runtimeInvocationRef !== persistedProof.runtimeInvocationRef)) throw new Error("Persisted verified overlap proof is not bound to the authorized deployment receipt.");
  const result = await runPostOverlapVerification({ deployment: { updateServiceCount: resumeVerified ? 0 : 1, propagateTags: "TASK_DEFINITION", taskDefinitionArn: receipt.taskDefinitionArn, receipt, ...(resumeVerified ? { resumeVerified: true, persistedDeployed: { valid: true, taskArn: persistedProof.targetTaskArn, taskDefinitionArn: persistedProof.targetTaskDefinitionArn, imageDigest: persistedProof.targetImageDigest, taskTag: "MSCQRExecTarget=production-backend" }, persistedExecProof: { valid: true, ...persistedProof } } : {}) }, sourceSha, rotationId, rotationStateSha256: preparedStateSha256, rotationFixtureSha256: fixture.sha256, taskDefinitionArn: receipt.taskDefinitionArn, expectedImageDigest: receipt.imageDigest, verifierSession: identities.verifier.session, postDeploy: adapters.postDeploy, ecsExec: adapters.ecsExec, rotationVerify: adapters.rotationPrepare.verifyOverlap });
