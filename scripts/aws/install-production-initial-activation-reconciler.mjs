...
export function executeInstallation({ sourceSha, preparation, authorization, planBytes, planJson, administratorArn = INSTALLATION.administratorArn, livePredecessor, applySavedPlan, verifyInstalled, readState, resultPath, consumptionDirectory, now = new Date() } = {}) {
  ...
  if (livePredecessor === "EXACT_COMPLETE") {
    ...
    verifyInstalled();
    const result = { kind: "PRODUCTION_INITIAL_ACTIVATION_POLICY_RECONCILER_INSTALLATION_RESULT", schemaVersion: 1, operation: INSTALLATION.operation, sourceSha, authorizationArtifactSha256: authorization.authorizationArtifactSha256, status: "COMPLETE", applyCount: 0, targetPolicyCreatePolicyVersionCount: 0, verifier: "PASS", state: stateIdentity(readState?.()), completedAt: new Date().toISOString() };
  ...
  verifyInstalled();
  const result = { kind: "PRODUCTION_INITIAL_ACTIVATION_POLICY_RECONCILER_INSTALLATION_RESULT", schemaVersion: 1, operation: INSTALLATION.operation, sourceSha, authorizationArtifactSha256: authorization.authorizationArtifactSha256, status: "COMPLETE", applyCount: 1, targetPolicyCreatePolicyVersionCount: 0, verifier: "PASS", state: stateIdentity(readState?.()), completedAt: new Date().toISOString() };
  writeStageBPrivateFilesAtomic({ repositoryRoot: root, files: [{ filePath: output, bytes: Buffer.from(`${JSON.stringify(result, null, 2)}\n`), label: "Installation result" }] });
  return Object.freeze(result);
}
