...
const PREPARATION_FIELDS = new Set(["schemaVersion", "kind", "operation", "repository", "environment", "sourceSha", "terraformRoot", "backend", "administratorArn", "roleArn", "policyArn", "sourceContractHashes", "predecessorState", "livePredecessor", "savedPlanSha256", "savedPlanByteLength", "planSemantics", "maxAwsMutations", "preparedAt", "preparationSha256"]);
export function assertInstallationPreparation(value, { sourceSha, planBytes } = {}) {
  ...
  if (value.schemaVersion !== 1 || value.kind !== "PRODUCTION_INITIAL_ACTIVATION_POLICY_RECONCILER_INSTALLATION_PREPARATION" || value.operation !== INSTALLATION.operation || value.repository !== INSTALLATION.repository || value.environment !== INSTALLATION.environment || value.sourceSha !== sourceSha || value.terraformRoot !== INSTALLATION.terraformRoot || canonicalSha256(body) !== preparationSha256) throw new Error("Installation preparation identity or hash is invalid.");
...
export function assertInstallationAuthorization(value, { sourceSha, preparation } = {}) {
  ...
  if (value.schemaVersion !== 1 || value.kind !== "PRODUCTION_INITIAL_ACTIVATION_POLICY_RECONCILER_INSTALLATION_AUTHORIZATION" || value.operation !== INSTALLATION.operation || value.repository !== INSTALLATION.repository || value.environment !== INSTALLATION.environment || value.sourceSha !== sourceSha || value.preparationSha256 !== preparation.preparationSha256 || value.savedPlanSha256 !== preparation.savedPlanSha256 || canonicalSha256(body) !== authorizationSha256) throw new Error("Installation authorization binding or hash is invalid.");
...
export function assertInstallationResult(value, { sourceSha, authorization } = {}) {
  if (!value || value.kind !== "PRODUCTION_INITIAL_ACTIVATION_POLICY_RECONCILER_INSTALLATION_RESULT" || value.sourceSha !== sourceSha || value.authorizationSha256 !== authorization.authorizationSha256 || value.status !== "COMPLETE" || ![0, 1].includes(value.applyCount) || value.targetPolicyCreatePolicyVersionCount !== 0 || value.verifier !== "PASS") throw new Error("Installation result evidence is invalid.");
  return value;
}
