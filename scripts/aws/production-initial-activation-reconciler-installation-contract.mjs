...
export const INSTALLATION = Object.freeze({
  ...
  maxAwsMutations: Object.freeze({ "iam:CreateRole": 2, "iam:CreatePolicy": 2, "iam:AttachRolePolicy": 2, "iam:UpdateAssumeRolePolicy": 0, "iam:PutRolePolicy": 0, "iam:CreatePolicyVersion": 2 }),
});
...
const MIXED_RECOVERY_ROLE_DESCRIPTION = "GitHub production-environment executor for the exact mixed dual-slot topology recovery.";
...
export function assertInstallationPlan(plan) {
  ...
  for (const entry of changes) {
    ...
    if (entry.mode !== "managed" || ![JSON.stringify(["create"]), JSON.stringify(["no-op"]), JSON.stringify(["update"])].includes(action) || update && entry.address !== "aws_iam_policy.reconciler") throw new Error("Installation plan contains an unreviewed resource action.");
    ...
    if (entry.address === "aws_iam_role.reconciler") {
      ...
      if (after.name !== MIXED_RECOVERY_ROLE_NAME || after.path !== "/" || after.description !== MIXED_RECOVERY_ROLE_DESCRIPTION || after.force_detach_policies !== false || after.max_session_duration !== 3600 || after.permissions_boundary !== null || canonicalJson(after.tags) !== canonicalJson(MIXED_RECOVERY_TAGS) || canonicalJson(after.tags_all) !== canonicalJson(MIXED_RECOVERY_TAGS) || canonicalJson(policyValue(after.assume_role_policy, "Mixed recovery trust policy")) !== canonicalJson(sourceJson(`${INSTALLATION.terraformRoot}/trust-policy.json`)) || !create && after.arn !== INSTALLATION.mixedRecoveryRoleArn) throw new Error("Mixed recovery installation role contract is not exact.");
...
export function createInstallationPreparation({ sourceSha, state, livePredecessor, livePredecessorAddresses, planJson, planBytes, preparedAt = new Date().toISOString() } = {}) {
  ...
  if (!["ABSENT", "EXACT_PARTIAL", "EXACT_UPDATE", "EXACT_EXPANSION", "EXACT_COMPLETE"].includes(livePredecessor)) throw new Error("Installation live predecessor classification is invalid.");
  ...
  if (livePredecessor === "ABSENT" && (semantics.createCount !== INSTALLATION.expectedAddresses.length || semantics.noOpCount !== 0)
    ...
    || livePredecessor === "EXACT_EXPANSION" && (semantics.createCount !== 3 || semantics.updateCount > 1 || semantics.updateCount === 1 && !semantics.changedAddresses.includes("aws_iam_policy.reconciler") || semantics.noOpCount !== 3 - semantics.updateCount)
    ...
    || JSON.stringify(livePredecessorAddresses) !== JSON.stringify(["EXACT_UPDATE", "EXACT_EXPANSION"].includes(livePredecessor) ? (livePredecessor === "EXACT_UPDATE" ? [...INSTALLATION.expectedAddresses].sort() : ["aws_iam_policy.reconciler", "aws_iam_role.reconciler", "aws_iam_role_policy_attachment.reconciler"].sort()) : semantics.noOpAddresses)) throw new Error("Installation plan does not match the authenticated live predecessor.");
...
export function assertInstallationPreparation(value, { sourceSha, planBytes } = {}) {
  ...
  if (!value.planSemantics || !Array.isArray(value.planSemantics.changedAddresses) || !Array.isArray(value.planSemantics.noOpAddresses)
    ...
    || JSON.stringify(value.livePredecessorAddresses) !== JSON.stringify(value.livePredecessor === "EXACT_UPDATE" ? [...INSTALLATION.expectedAddresses].sort() : value.livePredecessor === "EXACT_EXPANSION" ? ["aws_iam_policy.reconciler", "aws_iam_role.reconciler", "aws_iam_role_policy_attachment.reconciler"].sort() : value.planSemantics.noOpAddresses)
    ...
    || value.livePredecessor === "EXACT_EXPANSION" && (value.planSemantics.createCount !== 3 || value.planSemantics.updateCount > 1 || value.planSemantics.updateCount === 1 && !value.planSemantics.changedAddresses.includes("aws_iam_policy.reconciler"))
