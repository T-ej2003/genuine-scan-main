...
import { brokerRecoverySuccessorEvidenceReader } from "./component-broker-recovery-successor-evidence-reader-contract.mjs";
...
const MIXED_RECOVERY_ROLE_NAME = "mscqr-production-mixed-dual-slot-recovery-executor";
const MIXED_RECOVERY_POLICY_NAME = "MSCQRProductionMixedDualSlotRecoveryExecutor";
...
const EXPECTED_OUTPUT_CONFIGURATION = Object.freeze({
  ...
  mixed_recovery_permissions_policy_sha256: { expression: { references: ["path.module"] } },
  mixed_recovery_policy_arn: { expression: { references: ["aws_iam_policy.mixed_recovery.arn", "aws_iam_policy.mixed_recovery"] } },
  mixed_recovery_role_arn: { expression: { references: ["aws_iam_role.mixed_recovery.arn", "aws_iam_role.mixed_recovery"] } },
  mixed_recovery_trust_policy_sha256: { expression: { references: ["path.module"] } },
...
const EXPECTED_RESOURCE_CONFIGURATION = Object.freeze({
  ...
  "aws_iam_role.broker_recovery_successor_evidence_reader": {
    address: "aws_iam_role.broker_recovery_successor_evidence_reader", mode: "managed", type: "aws_iam_role", name: "broker_recovery_successor_evidence_reader", provider_config_key: "aws", schema_version: 0,
    expressions: { assume_role_policy: { references: ["path.module"] }, description: { constant_value: EVIDENCE_READER_ROLE_DESCRIPTION }, max_session_duration: { constant_value: 3600 }, name: { references: ["local.broker_recovery_successor_evidence_reader_role_name"] }, tags: { references: ["local.tags"] } },
  },
  "aws_iam_role_policy_attachment.broker_recovery_successor_evidence_reader": {
    address: "aws_iam_role_policy_attachment.broker_recovery_successor_evidence_reader", mode: "managed", type: "aws_iam_role_policy_attachment", name: "broker_recovery_successor_evidence_reader", provider_config_key: "aws", schema_version: 0,
    expressions: { policy_arn: { references: ["aws_iam_policy.broker_recovery_successor_evidence_reader.arn", "aws_iam_policy.broker_recovery_successor_evidence_reader"] }, role: { references: ["aws_iam_role.broker_recovery_successor_evidence_reader.name", "aws_iam_role.broker_recovery_successor_evidence_reader"] } },
  },
  ...
  "aws_iam_role.mixed_recovery": {
    address: "aws_iam_role.mixed_recovery", mode: "managed", type: "aws_iam_role", name: "mixed_recovery", provider_config_key: "aws", schema_version: 0,
    expressions: { assume_role_policy: { references: ["path.module"] }, description: { constant_value: MIXED_RECOVERY_ROLE_DESCRIPTION }, max_session_duration: { constant_value: 3600 }, name: { references: ["local.mixed_recovery_role_name"] }, tags: { references: ["local.tags"] } },
  },
  "aws_iam_role_policy_attachment.mixed_recovery": {
    address: "aws_iam_role_policy_attachment.mixed_recovery", mode: "managed", type: "aws_iam_role_policy_attachment", name: "mixed_recovery", provider_config_key: "aws", schema_version: 0,
    expressions: { policy_arn: { references: ["aws_iam_policy.mixed_recovery.arn", "aws_iam_policy.mixed_recovery"] }, role: { references: ["aws_iam_role.mixed_recovery.name", "aws_iam_role.mixed_recovery"] } },
  },
...
export function assertInstallationPlan(plan) {
  ...
  const evidenceReaderPolicyCreated = JSON.stringify(changes.find((entry) => entry?.address === "aws_iam_policy.broker_recovery_successor_evidence_reader")?.change?.actions) === JSON.stringify(["create"]);
  ...
  for (const entry of changes) {
    ...
    if (create) createdAddresses.push(entry.address); else if (update) updatedAddresses.push(entry.address); else { noOpAddresses.push(entry.address); noOpCount += 1; }
    ...
    if (entry.address === "aws_iam_role.reconciler") {
      if (after.name !== ROLE_NAME || after.path !== "/" || after.description !== ROLE_DESCRIPTION || after.force_detach_policies !== false || after.max_session_duration !== 3600 || !hasKnownNoPermissionsBoundary(after.permissions_boundary, entry.change.after_unknown?.permissions_boundary) || canonicalJson(after.tags) !== canonicalJson(EXPECTED_TAGS) || canonicalJson(after.tags_all) !== canonicalJson(EXPECTED_TAGS) || canonicalJson(policyValue(after.assume_role_policy, "Installation plan trust policy")) !== canonicalJson(sourceJson(`${INSTALLATION.terraformRoot}/trust-policy.json`)) || !create && after.arn !== INSTALLATION.roleArn) throw new Error("Installation plan role contract is not exact.");
      ...
      if (update) {
        ...
        if (!before || before.arn !== INSTALLATION.policyArn || canonicalJson(policyValue(before.policy, "Installation plan predecessor permissions policy")) !== canonicalJson(installationPermissionsPredecessor()) || canonicalJson({ ...before, policy: after.policy }) !== canonicalJson(after)) throw new Error("Installation plan policy update predecessor is not exact.");
      }
    } else if (entry.address === "aws_iam_role.mixed_recovery") {
      if (after.name !== MIXED_RECOVERY_ROLE_NAME || after.path !== "/" || after.description !== MIXED_RECOVERY_ROLE_DESCRIPTION || after.force_detach_policies !== false || after.max_session_duration !== 3600 || !hasKnownNoPermissionsBoundary(after.permissions_boundary, entry.change.after_unknown?.permissions_boundary) || canonicalJson(after.tags) !== canonicalJson(MIXED_RECOVERY_TAGS) || canonicalJson(after.tags_all) !== canonicalJson(MIXED_RECOVERY_TAGS) || canonicalJson(policyValue(after.assume_role_policy, "Mixed recovery trust policy")) !== canonicalJson(sourceJson(`${INSTALLATION.terraformRoot}/mixed-recovery-trust-policy.json`)) || !create && after.arn !== INSTALLATION.mixedRecoveryRoleArn) throw new Error("Mixed recovery installation role contract is not exact.");
    ...
    } else if (entry.address === "aws_iam_role_policy_attachment.mixed_recovery") {
    ...
    } else if (entry.address === "aws_iam_role_policy_attachment.broker_recovery_successor_evidence_reader") {
      const policyArnKnown = !evidenceReaderPolicyCreated && after.policy_arn === INSTALLATION.brokerRecoverySuccessorEvidenceReaderPolicyArn && entry.change.after_unknown?.policy_arn === undefined;
      const policyArnComputed = evidenceReaderPolicyCreated && !Object.hasOwn(after, "policy_arn") && entry.change.after_unknown?.policy_arn === true;
      if (after.role !== brokerRecoverySuccessorEvidenceReader.roleName || !(policyArnKnown || policyArnComputed)) throw new Error("Evidence reader attachment contract is not exact.");
    }
  }
...
export function assertInstallationAuthorizedPostState(rawBytes, { predecessorState, planSemantics } = {}) {
  const observedIdentity = stateIdentity(rawBytes);
  if (predecessorState?.stateExists && observedIdentity.stateExists && observedIdentity.stateSha256 === predecessorState.stateSha256) {
  ...
  if (!predecessorState?.stateExists || identity.lineage !== predecessorState.lineage || identity.serial <= predecessorState.serial) throw new Error("Installation Terraform post-state lineage did not advance from the authenticated predecessor.");
  if (!planSemantics?.resourceChanges?.some(({ actions }) => canonicalJson(actions) !== canonicalJson(["no-op"]))) throw new Error("Installation authorized post-state requires an exact actionable plan.");
  ...
  for (const change of planSemantics.resourceChanges) {
    ...
    const policyArn = { "aws_iam_policy.reconciler": INSTALLATION.policyArn, "aws_iam_policy.mixed_recovery": INSTALLATION.mixedRecoveryPolicyArn, "aws_iam_policy.bootstrap_operator_policy_authorizer": INSTALLATION.bootstrapOperatorPolicyAuthorizerPolicyArn, "aws_iam_policy.broker_recovery_successor_evidence_reader": INSTALLATION.brokerRecoverySuccessorEvidenceReaderPolicyArn }[change.address];
    if (policyArn && (attributes.arn !== policyArn || attributes.id !== policyArn)) throw new Error("Installation Terraform post-state policy identity is not exact.");
...
export function createInstallationPreparation({ sourceSha, state, livePredecessor, livePredecessorAddresses, planJson, planBytes, preparedAt = new Date().toISOString() } = {}) {
  ...
  if (!["ABSENT", "EXACT_PARTIAL", "EXACT_UPDATE", "EXACT_TRUST_UPDATE", "EXACT_AUTHORIZER_TRUST_UPDATE", "EXACT_AUTHORIZER_POLICY_UPDATE", "EXACT_EXPANSION", "EXACT_COMPLETE"].includes(livePredecessor)) throw new Error("Installation live predecessor classification is invalid.");
...
export function assertInstallationPreparation(value, { sourceSha, planBytes } = {}) {
  ...
  const planPredecessorAddresses = value.planSemantics?.resourceChanges?.filter(({ actions }) => canonicalJson(actions) !== canonicalJson(["create"])).map(({ address }) => address).sort();
  if (!value.planSemantics || !Array.isArray(value.planSemantics.changedAddresses) || !Array.isArray(value.planSemantics.noOpAddresses)
    ...
    || JSON.stringify(value.livePredecessorAddresses) !== JSON.stringify(["EXACT_UPDATE", "EXACT_TRUST_UPDATE", "EXACT_AUTHORIZER_TRUST_UPDATE", "EXACT_AUTHORIZER_POLICY_UPDATE"].includes(value.livePredecessor) ? [...INSTALLATION.expectedAddresses].sort() : value.livePredecessor === "EXACT_EXPANSION" ? planPredecessorAddresses : value.planSemantics.noOpAddresses)
