...
const decodeAwsDocument = (value, label) => {
  if (typeof value !== "string" || !value) throw new Error(`${label} is missing.`);
  try { return JSON.parse(decodeURIComponent(value)); } catch (error) { throw new Error(`${label} is not a valid encoded JSON document: ${error.message}`); }
};
...
export function verifyInitialActivationPolicyReconciler({ run, expectedCallerArn = "arn:aws:iam::368992683803:root" } = {}) {
  ...
  if (policy?.Arn !== INITIAL_ACTIVATION_RECONCILER.policyArn || policy?.PolicyName !== INITIAL_ACTIVATION_RECONCILER.policyName || !/^v[1-9][0-9]*$/.test(policy.DefaultVersionId || "")) throw new Error("Initial-activation reconciler managed-policy identity is not exact.");
  ...
  return Object.freeze({ roleArn: role.Arn, policyArn: policy.Arn, defaultVersionId: policy.DefaultVersionId, trustPolicySha256: sha256(Buffer.from(JSON.stringify(trust))), permissionsPolicySha256: sha256(Buffer.from(JSON.stringify(permissions))), targetPolicyArn: INITIAL_ACTIVATION_RECONCILER.targetPolicyArn, releaseRoleArn: INITIAL_ACTIVATION_RECONCILER.releaseRoleArn, roleDefinedInSource: true, pr448RuntimeMigrated: false });
}
