...
const bootstrapPermissionsPredecessors = () => {
  ...
  const expansionSids = ["ReadExactMixedRecoveryRole", "ReadExactMixedRecoveryPolicy", "CreateExactMixedRecoveryRole", "CreateExactMixedRecoveryPolicy", "AttachExactMixedRecoveryPolicyToRole"];
  const exact = (generation, omittedSids, sha256) => {
    const document = structuredClone(desired);
    document.Statement = document.Statement.filter(({ Sid }) => ![...expansionSids, ...omittedSids].includes(Sid));
    const update = document.Statement.find(({ Sid }) => Sid === "UpdateExactReconcilerPolicyVersion");
    if (update) update.Resource = "arn:aws:iam::368992683803:policy/MSCQRProductionInitialActivationPolicyReconciler";
  ...
  return Object.freeze([
    ...
    exact("GENERATION_3", [], "f7750b66517cc245fa54dc805ea21a33c975399e04ea97a8c6f0c21e16b0c263"),
  ]);
};
...
export function createBootstrapPreparation({ sourceSha, predecessor } = {}) {
  if (!/^[a-f0-9]{40}$/.test(sourceSha || "") || !predecessor || !["ABSENT", "EXACT_PARTIAL", "EXACT_PREDECESSOR_GENERATION_1", "EXACT_PREDECESSOR_GENERATION_2", "EXACT_PREDECESSOR_GENERATION_3", "EXACT_COMPLETE"].includes(predecessor.classification)) throw new Error("Bootstrap preparation predecessor is invalid.");
...
export function runBootstrapCli(argv = process.argv.slice(2), deps = {}) {
  ...
  if (argv.includes("--prepare")) {
    ...
    if (!["ABSENT", "EXACT_PARTIAL", "EXACT_PREDECESSOR_GENERATION_1", "EXACT_PREDECESSOR_GENERATION_2", "EXACT_PREDECESSOR_GENERATION_3", "EXACT_COMPLETE"].includes(predecessor.classification)) throw new Error("Bootstrap preparation predecessor is not eligible.");
