...
export const MIXED_DUAL_SLOT_RECOVERY_POST_STATE = "SEVEN_EXISTING_RESOURCES_WITH_AWS_LEGAL_CURRENT_PREVIOUS_SWAP";
...
const stagedIdentity = ({ retainedPrevious, ...identity }, stagingLabels) => Object.freeze({ ...identity, stagingLabels: Object.freeze(stagingLabels) });
// Every reviewed secret has exactly two versions. AWS can only move AWSCURRENT
// to the authenticated retained version, and then moves AWSPREVIOUS to the
// former AWSCURRENT version automatically.
export const MIXED_DUAL_SLOT_RECOVERY_SUCCESSOR = Object.freeze(Object.fromEntries(MIXED_DUAL_SLOT_RECOVERY_ORDER.map((slot) => [slot, Object.freeze({
  current: stagedIdentity(MIXED_DUAL_SLOT_PREDECESSOR[slot].retainedPrevious, ["AWSCURRENT"]),
  previous: stagedIdentity(MIXED_DUAL_SLOT_PREDECESSOR[slot], ["AWSPREVIOUS"]),
})])));
export const MIXED_DUAL_SLOT_RECOVERY_SUCCESSOR_CANONICAL_ID = mixedDualSlotRecoverySha256(MIXED_DUAL_SLOT_RECOVERY_SUCCESSOR);
...
export const MIXED_DUAL_SLOT_RECOVERY_POST_STATE_CANONICAL_ID = MIXED_DUAL_SLOT_RECOVERY_SUCCESSOR_CANONICAL_ID;
...
export function readMixedDualSlotRecoveryGithubEnvironmentGuard({ run, privilegedRun } = {}) {
  ...
  if (!Array.isArray(protectionRules) || protectionRules.length !== 1 || !protectionRules[0] || typeof protectionRules[0] !== "object" || Array.isArray(protectionRules[0]) || protectionRules[0].type !== "branch_policy") fail("Mixed recovery GitHub environment is not the exact protected-main-only execution boundary.");
  const guard = { environmentId: environment?.id, environmentName: environment?.name, deploymentBranchPolicy: { protectedBranches: environment?.deployment_branch_policy?.protected_branches, customBranchPolicies: environment?.deployment_branch_policy?.custom_branch_policies }, branchPolicyCount: policies[0].total_count, branchPolicyId: branches[0]?.id, branchPolicyName: branches[0]?.name, branchPolicyType: branches[0]?.type, protectionRules: [{ type: "branch_policy" }], customProtectionRuleCount: customProtectionRules?.total_count, environmentSecretCount: secrets?.total_count };
  ...
  if (!Number.isInteger(guard.environmentId) || guard.environmentId < 1 || guard.environmentName !== MIXED_DUAL_SLOT_RECOVERY_GITHUB_ENVIRONMENT || canonical(guard.deploymentBranchPolicy) !== canonical({ protectedBranches: false, customBranchPolicies: true }) || guard.branchPolicyCount !== 1 || branches.length !== 1 || !Number.isInteger(guard.branchPolicyId) || guard.branchPolicyId < 1 || guard.branchPolicyName !== "main" || guard.branchPolicyType !== "branch" || canonical(guard.protectionRules) !== canonical([{ type: "branch_policy" }]) || guard.customProtectionRuleCount !== 0 || !Array.isArray(customProtectionRules?.custom_deployment_protection_rules) || customProtectionRules.custom_deployment_protection_rules.length !== 0 || guard.environmentSecretCount !== 0 || !Array.isArray(secrets?.secrets) || secrets.secrets.length !== 0) fail("Mixed recovery GitHub environment is not the exact protected-main-only execution boundary.");
  return Object.freeze(structuredClone(guard));
}

const preflightFields = ["schemaVersion", "kind", "operation", "sourceSha", "principalArn", "action", "resources", "roleTrustPolicySha256", "rolePermissionsBoundary", "oidcProviderGuard", "githubEnvironmentGuard", "organizationsGuard", "secretEncryptionGuards", "resourcePolicies", "evaluations", "observedAt", "preflightSha256"];
export function assertMixedDualSlotRecoveryIamPreflight(value, { sourceSha, now = new Date(), requireFresh = false } = {}) {
  ...
  if (value.schemaVersion !== 7 || value.kind !== MIXED_DUAL_SLOT_RECOVERY_IAM_PREFLIGHT_KIND || value.operation !== MIXED_DUAL_SLOT_RECOVERY_OPERATION || value.sourceSha !== sourceSha || value.principalArn !== MIXED_DUAL_SLOT_RECOVERY_EXECUTION_ROLE_ARN || value.action !== MIXED_DUAL_SLOT_RECOVERY_IAM_ACTION || !SHA256.test(value.roleTrustPolicySha256 || "") || value.rolePermissionsBoundary !== null || canonical(value.oidcProviderGuard) !== canonical({ providerArn: MIXED_DUAL_SLOT_RECOVERY_OIDC_PROVIDER_ARN, url: "token.actions.githubusercontent.com", audience: "sts.amazonaws.com" }) || value.githubEnvironmentGuard.environmentName !== MIXED_DUAL_SLOT_RECOVERY_GITHUB_ENVIRONMENT || !Number.isInteger(value.githubEnvironmentGuard.environmentId) || value.githubEnvironmentGuard.environmentId < 1 || canonical(value.githubEnvironmentGuard.deploymentBranchPolicy) !== canonical({ protectedBranches: false, customBranchPolicies: true }) || value.githubEnvironmentGuard.branchPolicyCount !== 1 || !Number.isInteger(value.githubEnvironmentGuard.branchPolicyId) || value.githubEnvironmentGuard.branchPolicyId < 1 || value.githubEnvironmentGuard.branchPolicyName !== "main" || value.githubEnvironmentGuard.branchPolicyType !== "branch" || canonical(value.githubEnvironmentGuard.protectionRules) !== canonical([{ type: "branch_policy" }]) || value.githubEnvironmentGuard.customProtectionRuleCount !== 0 || value.githubEnvironmentGuard.environmentSecretCount !== 0 || canonical(value.organizationsGuard) !== canonical({ accountId: "368992683803", status: "NOT_IN_ORGANIZATION", evidence: "AWSOrganizationsNotInUseException" }) || canonical(value.resources) !== canonical(MIXED_DUAL_SLOT_RECOVERY_IAM_RESOURCES) || !Array.isArray(value.secretEncryptionGuards) || value.secretEncryptionGuards.length !== 7 || !Array.isArray(value.resourcePolicies) || value.resourcePolicies.length !== 7 || !Array.isArray(value.evaluations) || value.evaluations.length !== expectedEvaluations.length) fail("Mixed recovery IAM preflight identity is invalid.");
  for (const [index, guard] of value.secretEncryptionGuards.entries()) {
    exactKeys(guard, ["resource", "kmsKeyId", "encryption"], `Mixed recovery secret encryption guard ${index + 1}`);
    if (guard.resource !== MIXED_DUAL_SLOT_RECOVERY_IAM_RESOURCES[index] || guard.kmsKeyId !== null || guard.encryption !== "AWS_MANAGED") fail(`Mixed recovery secret encryption is not AWS managed for resource ${index + 1}.`);
  }
...
export function buildMixedDualSlotRecoveryIamPreflight({ sourceSha, principalArn, action, resources, roleTrustPolicySha256, rolePermissionsBoundary = null, oidcProviderGuard, githubEnvironmentGuard, organizationsGuard, secretEncryptionGuards, resourcePolicies, evaluations, observedAt } = {}) {
  const body = { schemaVersion: 7, kind: MIXED_DUAL_SLOT_RECOVERY_IAM_PREFLIGHT_KIND, operation: MIXED_DUAL_SLOT_RECOVERY_OPERATION, sourceSha, principalArn, action, resources, roleTrustPolicySha256, rolePermissionsBoundary, oidcProviderGuard, githubEnvironmentGuard, organizationsGuard, secretEncryptionGuards, resourcePolicies, evaluations, observedAt };
...
export function assertMixedDualSlotRecoverySuccessor(observed) {
  exactKeys(observed, MIXED_DUAL_SLOT_RECOVERY_ORDER, "Mixed dual-slot recovery successor");
  for (const slot of MIXED_DUAL_SLOT_RECOVERY_ORDER) {
    const actual = exactKeys(observed[slot], ["current", "previous"], `Mixed recovery successor ${slot}`);
    exactKeys(actual.current, ["arn", "versionId", "stagingLabels", "payloadSha256", "schemaKeys", "sourceSha", "rotationId", "slot", "materialFingerprint", "keyVersion"], `Mixed recovery successor ${slot} current`);
    exactKeys(actual.previous, ["arn", "versionId", "stagingLabels", "payloadSha256", "schemaKeys", "sourceSha", "rotationId", "slot", "materialFingerprint", "keyVersion"], `Mixed recovery successor ${slot} previous`);
    if (canonical(actual) !== canonical(MIXED_DUAL_SLOT_RECOVERY_SUCCESSOR[slot])) fail(`Mixed dual-slot recovery successor ${slot} is not exact.`);
  }
  return Object.freeze(structuredClone(observed));
}

const exactMutationPlan = (checked) => MIXED_DUAL_SLOT_RECOVERY_ORDER.map((slot) => Object.freeze({
  slot,
  secretArn: checked[slot].arn,
  removeFromVersionId: checked[slot].versionId,
  moveToVersionId: checked[slot].retainedPrevious.versionId,
  expectedAwspreviousBeforeVersionId: checked[slot].retainedPrevious.versionId,
  expectedAwspreviousAfterVersionId: checked[slot].versionId,
  expectedStagesBefore: Object.freeze({ [checked[slot].versionId]: ["AWSCURRENT"], [checked[slot].retainedPrevious.versionId]: ["AWSPREVIOUS"] }),
  expectedStagesAfter: Object.freeze({ [checked[slot].retainedPrevious.versionId]: ["AWSCURRENT"], [checked[slot].versionId]: ["AWSPREVIOUS"] }),
}));
...
export function buildMixedDualSlotRecoveryPreparation({ sourceSha, predecessor, iamCapabilityPreflight, initialCompletedStageLabelMutations = 0, livePredecessor = MIXED_DUAL_SLOT_RECOVERY_LIVE_PREDECESSOR, preparedAt } = {}) {
  ...
  const mutationPlan = exactMutationPlan(checked);
  ...
  const body = { schemaVersion: 4, kind: MIXED_DUAL_SLOT_RECOVERY_PREPARATION_KIND, operation: MIXED_DUAL_SLOT_RECOVERY_OPERATION, sourceSha, historicalSourceSha: MIXED_DUAL_SLOT_RECOVERY_SOURCE, historicalRotationId: MIXED_DUAL_SLOT_RECOVERY_ROTATION, predecessor: checked, predecessorCanonicalId: MIXED_DUAL_SLOT_PREDECESSOR_CANONICAL_ID, retainedHistoryCanonicalId: MIXED_DUAL_SLOT_RETAINED_HISTORY_CANONICAL_ID, successorCanonicalId: MIXED_DUAL_SLOT_RECOVERY_SUCCESSOR_CANONICAL_ID, iamCapabilityPreflight: iamPreflight, iamCapabilityPreflightSha256: iamPreflight.preflightSha256, initialCompletedStageLabelMutations, maximumRemainingStageLabelMutations: 7 - initialCompletedStageLabelMutations, livePredecessor, livePredecessorCanonicalId: mixedDualSlotRecoverySha256(livePredecessor), postState: MIXED_DUAL_SLOT_RECOVERY_POST_STATE, postStateCanonicalId: MIXED_DUAL_SLOT_RECOVERY_POST_STATE_CANONICAL_ID, mutationPlan, mutationPlanSha256: mixedDualSlotRecoverySha256(mutationPlan), expectedUpdateSecretVersionStageCalls: 7, expectedAwscurrentMoves: 7, expectedAwspreviousMoves: 7, expectedSecretValueWrites: 0, expectedSecretCreates: 0, expectedSecretDeletes: 0, preparedAt };
...
export function assertMixedDualSlotRecoveryPreparation(value, { sourceSha } = {}) {
  exactKeys(value, ["schemaVersion", "kind", "operation", "sourceSha", "historicalSourceSha", "historicalRotationId", "predecessor", "predecessorCanonicalId", "retainedHistoryCanonicalId", "successorCanonicalId", "iamCapabilityPreflight", "iamCapabilityPreflightSha256", "initialCompletedStageLabelMutations", "maximumRemainingStageLabelMutations", "livePredecessor", "livePredecessorCanonicalId", "postState", "postStateCanonicalId", "mutationPlan", "mutationPlanSha256", "expectedUpdateSecretVersionStageCalls", "expectedAwscurrentMoves", "expectedAwspreviousMoves", "expectedSecretValueWrites", "expectedSecretCreates", "expectedSecretDeletes", "preparedAt", "preparationSha256"], "Mixed recovery preparation");
  if (value.schemaVersion !== 4 || value.kind !== MIXED_DUAL_SLOT_RECOVERY_PREPARATION_KIND || value.operation !== MIXED_DUAL_SLOT_RECOVERY_OPERATION || value.sourceSha !== sourceSha || value.historicalSourceSha !== MIXED_DUAL_SLOT_RECOVERY_SOURCE || value.historicalRotationId !== MIXED_DUAL_SLOT_RECOVERY_ROTATION || value.predecessorCanonicalId !== MIXED_DUAL_SLOT_PREDECESSOR_CANONICAL_ID || value.retainedHistoryCanonicalId !== MIXED_DUAL_SLOT_RETAINED_HISTORY_CANONICAL_ID || value.successorCanonicalId !== MIXED_DUAL_SLOT_RECOVERY_SUCCESSOR_CANONICAL_ID || value.iamCapabilityPreflightSha256 !== value.iamCapabilityPreflight?.preflightSha256 || !Number.isInteger(value.initialCompletedStageLabelMutations) || value.initialCompletedStageLabelMutations < 0 || value.initialCompletedStageLabelMutations > 7 || value.maximumRemainingStageLabelMutations !== 7 - value.initialCompletedStageLabelMutations || canonical(value.livePredecessor) !== canonical(MIXED_DUAL_SLOT_RECOVERY_LIVE_PREDECESSOR) || value.livePredecessorCanonicalId !== mixedDualSlotRecoverySha256(MIXED_DUAL_SLOT_RECOVERY_LIVE_PREDECESSOR) || value.postState !== MIXED_DUAL_SLOT_RECOVERY_POST_STATE || value.postStateCanonicalId !== MIXED_DUAL_SLOT_RECOVERY_POST_STATE_CANONICAL_ID || value.expectedUpdateSecretVersionStageCalls !== 7 || value.expectedAwscurrentMoves !== 7 || value.expectedAwspreviousMoves !== 7 || value.expectedSecretValueWrites !== 0 || value.expectedSecretCreates !== 0 || value.expectedSecretDeletes !== 0) fail("Mixed recovery preparation identity is invalid.");
  ...
  if (!Array.isArray(value.mutationPlan) || canonical(value.mutationPlan) !== canonical(exactMutationPlan(MIXED_DUAL_SLOT_PREDECESSOR))) fail("Mixed recovery mutation plan is not exact.");
...
export function createMixedDualSlotRecoveryAuthorization({ preparation, preparationFileSha256, iamCapabilityAttestation, iamCapabilityAttestationFileSha256, verifyIamCapabilityAttestation, protectedEnvironmentApprovalEvidence, reason, approverRole, verificationRef, now = new Date() } = {}) {
  ...
  if (typeof verifyIamCapabilityAttestation !== "function") fail("Mixed recovery authorization requires authenticated IAM capability evidence.");
  assertMixedDualSlotRecoveryIamAttestation(iamCapabilityAttestation, { preflight: checked.iamCapabilityPreflight, sourceSha: checked.sourceSha, now, verify: verifyIamCapabilityAttestation });
  if (!SHA256.test(iamCapabilityAttestationFileSha256 || "")) fail("Mixed recovery IAM attestation file identity is invalid.");
  const body = { schemaVersion: 5, kind: MIXED_DUAL_SLOT_RECOVERY_AUTHORIZATION_KIND, operation: MIXED_DUAL_SLOT_RECOVERY_OPERATION, sourceSha: checked.sourceSha, predecessorCanonicalId: checked.predecessorCanonicalId, retainedHistoryCanonicalId: checked.retainedHistoryCanonicalId, successorCanonicalId: checked.successorCanonicalId, iamCapabilityPreflightSha256: checked.iamCapabilityPreflightSha256, iamCapabilityAttestationSha256: iamCapabilityAttestation.attestationSha256, iamCapabilityAttestationFileSha256, preparationSha256: checked.preparationSha256, preparationFileSha256, initialCompletedStageLabelMutations: checked.initialCompletedStageLabelMutations, maximumRemainingStageLabelMutations: checked.maximumRemainingStageLabelMutations, postStateCanonicalId: checked.postStateCanonicalId, mutationPlanSha256: checked.mutationPlanSha256, historicalRotationId: checked.historicalRotationId, historicalSourceSha: checked.historicalSourceSha, expectedUpdateSecretVersionStageCalls: 7, expectedAwscurrentMoves: 7, expectedAwspreviousMoves: 7, expectedSecretValueWrites: 0, expectedSecretCreates: 0, expectedSecretDeletes: 0, approvedBy, reason: reason.trim(), approverRole: approverRole.trim(), verificationRef: verificationRef.trim(), protectedEnvironmentApprovalEvidence, protectedEnvironmentApprovalEvidenceSha256: protectedEnvironmentApprovalEvidence.evidenceSha256 };
...
export function assertMixedDualSlotRecoveryAuthorization(value, { preparation, preparationFileSha256, sourceSha, now = new Date(), allowExpiredResume = false } = {}) {
  ...
  exactKeys(value, ["schemaVersion", "kind", "operation", "sourceSha", "predecessorCanonicalId", "retainedHistoryCanonicalId", "successorCanonicalId", "iamCapabilityPreflightSha256", "iamCapabilityAttestationSha256", "iamCapabilityAttestationFileSha256", "preparationSha256", "preparationFileSha256", "initialCompletedStageLabelMutations", "maximumRemainingStageLabelMutations", "postStateCanonicalId", "mutationPlanSha256", "historicalRotationId", "historicalSourceSha", "expectedUpdateSecretVersionStageCalls", "expectedAwscurrentMoves", "expectedAwspreviousMoves", "expectedSecretValueWrites", "expectedSecretCreates", "expectedSecretDeletes", "approvedBy", "reason", "approverRole", "verificationRef", "protectedEnvironmentApprovalEvidence", "protectedEnvironmentApprovalEvidenceSha256", "authorizationSha256"], "Mixed recovery authorization");
  if (value.schemaVersion !== 5 || value.kind !== MIXED_DUAL_SLOT_RECOVERY_AUTHORIZATION_KIND || value.operation !== MIXED_DUAL_SLOT_RECOVERY_OPERATION || value.sourceSha !== sourceSha || value.predecessorCanonicalId !== checked.predecessorCanonicalId || value.retainedHistoryCanonicalId !== checked.retainedHistoryCanonicalId || value.successorCanonicalId !== checked.successorCanonicalId || value.iamCapabilityPreflightSha256 !== checked.iamCapabilityPreflightSha256 || !SHA256.test(value.iamCapabilityAttestationSha256 || "") || !SHA256.test(value.iamCapabilityAttestationFileSha256 || "") || value.preparationSha256 !== checked.preparationSha256 || !SHA256.test(value.preparationFileSha256 || "") || preparationFileSha256 && value.preparationFileSha256 !== preparationFileSha256 || value.initialCompletedStageLabelMutations !== checked.initialCompletedStageLabelMutations || value.maximumRemainingStageLabelMutations !== checked.maximumRemainingStageLabelMutations || value.postStateCanonicalId !== checked.postStateCanonicalId || value.mutationPlanSha256 !== checked.mutationPlanSha256 || value.historicalRotationId !== checked.historicalRotationId || value.historicalSourceSha !== checked.historicalSourceSha || value.expectedUpdateSecretVersionStageCalls !== 7 || value.expectedAwscurrentMoves !== 7 || value.expectedAwspreviousMoves !== 7 || value.expectedSecretValueWrites !== 0 || value.expectedSecretCreates !== 0 || value.expectedSecretDeletes !== 0) fail("Mixed recovery authorization identity is invalid.");
