...
export function buildMixedDualSlotRecoveryPreparation({ sourceSha, predecessor, livePredecessor = MIXED_DUAL_SLOT_RECOVERY_LIVE_PREDECESSOR, preparedAt } = {}) {
  ...
  const body = { schemaVersion: 1, kind: MIXED_DUAL_SLOT_RECOVERY_PREPARATION_KIND, operation: MIXED_DUAL_SLOT_RECOVERY_OPERATION, sourceSha, historicalSourceSha: MIXED_DUAL_SLOT_RECOVERY_SOURCE, historicalRotationId: MIXED_DUAL_SLOT_RECOVERY_ROTATION, predecessor: checked, predecessorCanonicalId: MIXED_DUAL_SLOT_PREDECESSOR_CANONICAL_ID, livePredecessor, livePredecessorCanonicalId: mixedDualSlotRecoverySha256(livePredecessor), postState: MIXED_DUAL_SLOT_RECOVERY_POST_STATE, postStateCanonicalId: MIXED_DUAL_SLOT_RECOVERY_POST_STATE_CANONICAL_ID, mutationPlan, mutationPlanSha256: mixedDualSlotRecoverySha256(mutationPlan), expectedStageLabelMutations: 7, expectedSecretValueWrites: 0, expectedSecretCreates: 0, expectedSecretDeletes: 0, preparedAt };
...
export function assertMixedDualSlotRecoveryPreparation(value, { sourceSha } = {}) {
  ...
  if (value.schemaVersion !== 1 || value.kind !== MIXED_DUAL_SLOT_RECOVERY_PREPARATION_KIND || value.operation !== MIXED_DUAL_SLOT_RECOVERY_OPERATION || value.sourceSha !== sourceSha || value.historicalSourceSha !== MIXED_DUAL_SLOT_RECOVERY_SOURCE || value.historicalRotationId !== MIXED_DUAL_SLOT_RECOVERY_ROTATION || value.predecessorCanonicalId !== MIXED_DUAL_SLOT_PREDECESSOR_CANONICAL_ID || canonical(value.livePredecessor) !== canonical(MIXED_DUAL_SLOT_RECOVERY_LIVE_PREDECESSOR) || value.livePredecessorCanonicalId !== mixedDualSlotRecoverySha256(MIXED_DUAL_SLOT_RECOVERY_LIVE_PREDECESSOR) || value.postState !== MIXED_DUAL_SLOT_RECOVERY_POST_STATE || value.postStateCanonicalId !== MIXED_DUAL_SLOT_RECOVERY_POST_STATE_CANONICAL_ID || value.expectedStageLabelMutations !== 7 || value.expectedSecretValueWrites !== 0 || value.expectedSecretCreates !== 0 || value.expectedSecretDeletes !== 0) fail("Mixed recovery preparation identity is invalid.");
