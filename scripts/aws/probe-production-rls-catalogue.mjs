...
export function bindProductionRlsProbeIdentities({ sourceSha, requirements, images, probeImage, databaseHostname }) {
  ...
  const { evidenceSha256, ...imageBody } = images || {}; assert.equal(evidenceSha256, canonicalSha256(imageBody));
  assert.equal(images.kind, "APP_ONLY_AUTHENTICATED_IMAGES"); assert.equal(images.sourceSha, sourceSha);
  assert.equal(images.candidateSourceSha, requirements.candidateSourceSha); assert.equal(probeImage?.sourceSha, sourceSha);
  assert.match(images.candidateDigest || "", /^sha256:[a-f0-9]{64}$/); assert.match(probeImage.digest || "", /^sha256:[a-f0-9]{64}$/);
  return Object.freeze({ sourceSha, candidateSourceSha: requirements.candidateSourceSha,
    probeRuntimeSourceSha: sourceSha, probeImageSourceSha: sourceSha, probeImageDigest: probeImage.digest,
    applicationImageSourceSha: images.candidateSourceSha, applicationImageDigest: images.candidateDigest, databaseHostname });
}
...
export function buildProductionRlsProbeDefinition({ baseDefinition, requirements, identity, databaseSecretArn, securityTransportPublicKey = null }) {
  ...
  const container = template.containerDefinitions[0], probe = buildProductionRlsProbeCommand(requirements, identity, { securityTransportPublicKey });
  container.image = `${APP_ONLY.backendRepository}@${identity.probeImageDigest}`; container.entryPoint = probe.entryPoint; container.command = probe.command; container.environment = probe.environment;
...
export function authenticateProductionRlsProbeResult(message, { sourceSha, candidateSourceSha, probeImageDigest, applicationImageDigest, requirementsSha256, securityTransportExpected = false }) {
  ...
  assert.deepEqual(Object.keys(result).sort(), ["candidateSourceSha", "catalogue", "databaseRole", "evidenceSha256", "kind", "requirementsSha256", "schemaVersion", "sourceSha",
    "probeRuntimeSourceSha", "probeImageSourceSha", "probeImageDigest", "applicationImageSourceSha", "applicationImageDigest", ...(securityTransportExpected ? ["securityTransport"] : [])].sort());
  ...
  assert.equal(body.probeRuntimeSourceSha, sourceSha); assert.equal(body.probeImageSourceSha, sourceSha); assert.equal(body.applicationImageSourceSha, candidateSourceSha);
  assert.match(probeImageDigest || "", /^sha256:[a-f0-9]{64}$/, "Expected probe image digest is required"); assert.equal(body.probeImageDigest, probeImageDigest);
  assert.match(applicationImageDigest || "", /^sha256:[a-f0-9]{64}$/, "Expected application image digest is required"); assert.equal(body.applicationImageDigest, applicationImageDigest);
...
export function authenticateCompleteRlsProbeObservation(messages, { sourceSha, candidateSourceSha, probeImageDigest, applicationImageDigest, requirementsSha256, securityMode }) {
  ...
  const evidence = authenticateProductionRlsProbeResult(terminals[0].message, { sourceSha, candidateSourceSha, probeImageDigest, applicationImageDigest, requirementsSha256, securityTransportExpected: securityMode });
...
export async function runProductionRlsCatalogueProbe({ sourceSha, requirementsReference, awsProfile, securityRebaselineReference = null,
  ...
  const identity = bindProductionRlsProbeIdentities({ sourceSha, requirements, images, probeImage, databaseHostname });
  ...
  if (securityMode) { const binding = { sourceSha, candidateSourceSha: requirements.candidateSourceSha, requirementsSha256: requirements.requirementsSha256 }; const catalogue = decryptSecurityCatalogueTransport(observation.chunks, transportKeys.privateKeyPem, binding); const liveInventory = createLiveSecurityRebaselineInventory({ protectedMainSha: sourceSha, candidateSourceSha: requirements.candidateSourceSha, catalogue, canonical: canonicalSecurity.inventory,
      taskEvidence: { taskArn, taskDefinitionArn, containerName: task.containers[0].name, containerExitCode: task.containers[0].exitCode,
        ...
        applicationImageSourceSha: identity.applicationImageSourceSha, applicationImageDigest: identity.applicationImageDigest,
        requestSha256: canonicalSha256(request), verificationContractSha256: canonicalSha256(definition) } });
    writeStageBPrivateFileExclusive({ filePath: securityRebaselineLiveOut, bytes: Buffer.from(`${JSON.stringify(liveInventory)}\n`), repositoryRoot });
    return { status: "PRODUCTION_SECURITY_REBASELINE_INVENTORIED", sourceSha, candidateSourceSha: requirements.candidateSourceSha,
      probeImageDigest: identity.probeImageDigest, applicationImageDigest: identity.applicationImageDigest, taskDefinitionArn, taskArn, classification: result.classification,
      deltaObjectCount: result.deltaObjects.length, requirementsSha256: requirements.requirementsSha256, inventory: securityRebaselineLogSummary(liveInventory) }; }
  return { status: "PRODUCTION_RLS_CATALOGUE_PROBED", sourceSha, candidateSourceSha: requirements.candidateSourceSha,
    probeImageDigest: identity.probeImageDigest, applicationImageDigest: identity.applicationImageDigest,
    taskDefinitionArn, taskArn, classification: result.classification, deltaObjects: result.deltaObjects, requirementsSha256: requirements.requirementsSha256 };
}
