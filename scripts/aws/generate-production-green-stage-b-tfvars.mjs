...
export function validateStageBStageAInput(input, { toolingSha, toolingTreeSha256 } = {}) {
  ...
  if (!proxy || JSON.stringify(Object.keys(proxy).sort()) !== JSON.stringify(["alb", "cloudFront", "mode", "targetGroup"]) || proxy.mode !== "cloudfront-alb" || !proxy.alb || !proxy.targetGroup || !proxy.cloudFront || proxy.alb.name !== "mscqr-alb-euw2" || !/^arn:aws:elasticloadbalancing:eu-west-2:368992683803:loadbalancer\/app\/mscqr-alb-euw2\/.+$/.test(proxy.alb.arn || "") || typeof proxy.alb.dnsName !== "string" || !Array.isArray(proxy.alb.subnetIds) || proxy.alb.subnetIds.length < 2 || !validCidrs(proxy.alb.cidrs) || proxy.targetGroup.name !== "mscqr-backend-tg-euw2-v2" || !/^arn:aws:elasticloadbalancing:eu-west-2:368992683803:targetgroup\/mscqr-backend-tg-euw2-v2\/.+$/.test(proxy.targetGroup.arn || "") || !proxy.cloudFront.distributionId || JSON.stringify(proxy.cloudFront.aliases) !== JSON.stringify(["mscqr.com", "www.mscqr.com"]) || !/^pl-[a-z0-9]+$/.test(proxy.cloudFront.managedPrefixListId || "") || !Number.isInteger(proxy.cloudFront.managedPrefixListVersion) || !validCidrs(proxy.cloudFront.cidrs)) throw new Error("Stage B backend proxy trust evidence is malformed or not bound to the reviewed CloudFront ALB topology.");
...
function assertStageAPrerequisiteBinding(report) {
  ...
  const expectedProxyTrust = { mode: input.stageBBackendProxyTrust.mode, albCidrs: input.stageBBackendProxyTrust.alb.cidrs.join(","), cloudFrontCidrs: input.stageBBackendProxyTrust.cloudFront.cidrs.join(",") };
...
export function generateStageBTfvars({ imageEvidence, imageEvidenceSignature, imageEvidenceBytes, imageEvidenceSignatureBytes, stateBackup, stageAInput, stageAStateBackup, brokerPackagePath, toolingSha, toolingTreeSha256, imageReleaseSha, workflowRunId, canonicalArtifactSha256, environment = STAGE_B_EXPECTED_ENVIRONMENT, now = new Date().toISOString(), verifySignature = verifyImageEvidenceSignature, checksumsFile = checksumsPath, outputPath, bindingReportPath, allowOverwrite = false, recoveryOnly = false, partialApplyRecovery = false, freshImagePartialApplyRecovery = false, recovery } = {}) {
  ...
  const bindingReport = {
    ...
    backendProxyTrust: { mode: values.backend_client_ip_trust_mode, albCidrs: values.backend_client_ip_trusted_alb_cidrs, cloudFrontCidrs: values.backend_client_ip_trusted_cloudfront_cidrs },
