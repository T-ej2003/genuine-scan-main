...
const stageAKeys = Object.freeze([
  ...
  "stageAReadOnlyCanaryDatabaseSecretArn", "stageBBackendProxyTrust",
]);
...
export function validateStageBStageAInput(input, { toolingSha, toolingTreeSha256 } = {}) {
  ...
  const proxy = input.stageBBackendProxyTrust;
  const validCidrs = (values) => Array.isArray(values) && values.length > 0 && values.every((value) => typeof value === "string" && /^\d{1,3}(?:\.\d{1,3}){3}\/(?:[0-9]|[12][0-9]|3[0-2])$/.test(value) && net.isIP(value.split("/")[0]) === 4) && JSON.stringify(values) === JSON.stringify([...new Set(values)].sort());
  const validBackendBehaviors = Array.isArray(proxy?.cloudFront?.backendBehaviors) && proxy.cloudFront.backendBehaviors.length > 0 && proxy.cloudFront.backendBehaviors.every((behavior) => behavior && JSON.stringify(Object.keys(behavior).sort()) === JSON.stringify(["pathPattern", "targetOriginId", "viewerProtocolPolicy"]) && typeof behavior.pathPattern === "string" && behavior.pathPattern && behavior.targetOriginId === proxy.cloudFront.targetOriginId && behavior.viewerProtocolPolicy === "redirect-to-https");
  if (!proxy || JSON.stringify(Object.keys(proxy).sort()) !== JSON.stringify(["alb", "cloudFront", "mode", "targetGroup"]) || proxy.mode !== "cloudfront-alb" || !proxy.alb || !proxy.targetGroup || !proxy.cloudFront || proxy.alb.vpcId !== input.vpcId || proxy.alb.name !== "mscqr-alb-euw2" || !/^arn:aws:elasticloadbalancing:eu-west-2:368992683803:loadbalancer\/app\/mscqr-alb-euw2\/.+$/.test(proxy.alb.arn || "") || typeof proxy.alb.dnsName !== "string" || !Array.isArray(proxy.alb.subnetIds) || proxy.alb.subnetIds.length < 2 || !validCidrs(proxy.alb.cidrs) || proxy.alb.xffHeaderProcessingMode !== "append" || proxy.alb.xffClientPortEnabled !== false || proxy.targetGroup.name !== "mscqr-backend-tg-euw2-v2" || !/^arn:aws:elasticloadbalancing:eu-west-2:368992683803:targetgroup\/mscqr-backend-tg-euw2-v2\/.+$/.test(proxy.targetGroup.arn || "") || !proxy.cloudFront.distributionId || !/^[a-z0-9]+\.cloudfront\.net$/.test(proxy.cloudFront.domainName || "") || !/^[A-Z0-9]+$/.test(proxy.cloudFront.configEtag || "") || JSON.stringify(proxy.cloudFront.aliases) !== JSON.stringify(["mscqr.com", "www.mscqr.com"]) || !proxy.cloudFront.dns || proxy.cloudFront.dns.hostedZoneId !== "Z0569586VLFIGGVI7HAZ" || !Array.isArray(proxy.cloudFront.dns.records) || JSON.stringify(proxy.cloudFront.dns.records.map((record) => `${record.name}:${record.type}:${record.target}:${record.hostedZoneId}`).sort()) !== JSON.stringify(["mscqr.com:A:" + proxy.cloudFront.domainName + ":Z2FDTNDATAQYW2", "mscqr.com:AAAA:" + proxy.cloudFront.domainName + ":Z2FDTNDATAQYW2", "www.mscqr.com:A:" + proxy.cloudFront.domainName + ":Z2FDTNDATAQYW2", "www.mscqr.com:AAAA:" + proxy.cloudFront.domainName + ":Z2FDTNDATAQYW2"].sort()) || !proxy.cloudFront.targetOriginId || !validBackendBehaviors || proxy.cloudFront.originProtocolPolicy !== "https-only" || proxy.cloudFront.originHttpsPort !== 443 || JSON.stringify(proxy.cloudFront.apiPaths) !== JSON.stringify(["/api/health/ready", "/api/auth/login"]) || !/^pl-[a-z0-9]+$/.test(proxy.cloudFront.managedPrefixListId || "") || !Number.isInteger(proxy.cloudFront.managedPrefixListVersion) || !validCidrs(proxy.cloudFront.cidrs)) throw new Error("Stage B backend proxy trust evidence is malformed or not bound to the reviewed CloudFront ALB topology.");
...
function assertStageAPrerequisiteBinding(report) {
  ...
  const expectedProxyTrust = { mode: input.stageBBackendProxyTrust.mode, albCidrs: input.stageBBackendProxyTrust.alb.cidrs.join(","), cloudFrontCidrs: input.stageBBackendProxyTrust.cloudFront.cidrs.join(","), cloudFrontPrefixListId: input.stageBBackendProxyTrust.cloudFront.managedPrefixListId, cloudFrontPrefixListVersion: String(input.stageBBackendProxyTrust.cloudFront.managedPrefixListVersion) };
  if (JSON.stringify(report.backendProxyTrust) !== JSON.stringify(expectedProxyTrust)) throw new Error("Stage B tfvars binding report backend proxy trust does not match its Stage-A prerequisite.");
...
function validateTfvarsValues(values) {
  ...
  if (values.backend_client_ip_trust_mode !== "cloudfront-alb" || !values.backend_client_ip_trusted_alb_cidrs || !values.backend_client_ip_trusted_cloudfront_cidrs || /[\r\n]/.test(values.backend_client_ip_trusted_alb_cidrs) || /[\r\n]/.test(values.backend_client_ip_trusted_cloudfront_cidrs)) throw new Error("Stage B backend client-IP runtime bindings are malformed.");
}
...
export function generateStageBTfvars({ imageEvidence, imageEvidenceSignature, imageEvidenceBytes, imageEvidenceSignatureBytes, stateBackup, stageAInput, stageAStateBackup, brokerPackagePath, toolingSha, toolingTreeSha256, imageReleaseSha, workflowRunId, canonicalArtifactSha256, environment = STAGE_B_EXPECTED_ENVIRONMENT, now = new Date().toISOString(), verifySignature = verifyImageEvidenceSignature, checksumsFile = checksumsPath, outputPath, bindingReportPath, allowOverwrite = false, recoveryOnly = false, partialApplyRecovery = false, freshImagePartialApplyRecovery = false, recovery } = {}) {
  ...
  if (imageEvidenceBytes !== undefined && !Buffer.isBuffer(imageEvidenceBytes)) throw new Error("Pinned image evidence must be bytes.");
  if (imageEvidenceSignatureBytes !== undefined && !Buffer.isBuffer(imageEvidenceSignatureBytes)) throw new Error("Pinned image-evidence signature must be bytes.");
  const evidenceBytes = imageEvidenceBytes || fs.readFileSync(imageEvidence); const signatureBytes = imageEvidenceSignatureBytes || fs.readFileSync(imageEvidenceSignature);
  let report; let signature;
  try { report = JSON.parse(evidenceBytes.toString("utf8")); signature = JSON.parse(signatureBytes.toString("utf8")); } catch (error) { throw new Error(`Pinned image evidence is not valid JSON: ${error.message}`); }
  ...
  const values = {
    ...
    backend_client_ip_trust_mode: stageAPrerequisiteInput.stageBBackendProxyTrust.mode, backend_client_ip_trusted_alb_cidrs: stageAPrerequisiteInput.stageBBackendProxyTrust.alb.cidrs.join(","), backend_client_ip_trusted_cloudfront_cidrs: stageAPrerequisiteInput.stageBBackendProxyTrust.cloudFront.cidrs.join(","),
    backend_client_ip_cloudfront_prefix_list_id: stageAPrerequisiteInput.stageBBackendProxyTrust.cloudFront.managedPrefixListId,
    backend_client_ip_cloudfront_prefix_list_version: String(stageAPrerequisiteInput.stageBBackendProxyTrust.cloudFront.managedPrefixListVersion),
  ...
  const bindingReport = {
    ...
    backendProxyTrust: { mode: values.backend_client_ip_trust_mode, albCidrs: values.backend_client_ip_trusted_alb_cidrs, cloudFrontCidrs: values.backend_client_ip_trusted_cloudfront_cidrs, cloudFrontPrefixListId: values.backend_client_ip_cloudfront_prefix_list_id, cloudFrontPrefixListVersion: values.backend_client_ip_cloudfront_prefix_list_version },
    backendProxyTopology: stageAPrerequisiteInput.stageBBackendProxyTrust,
