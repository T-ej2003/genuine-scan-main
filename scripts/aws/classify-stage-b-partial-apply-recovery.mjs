...
import { assertRecoveryClassification, classifyRecoveryResidue, STAGE_B_PARTIAL_APPLY_RECOVERY_ALGORITHM, STAGE_B_PARTIAL_APPLY_RECOVERY_KEY_ARN } from "./stage-b-partial-apply-recovery-contract.mjs";
...
export function runCli(argv = process.argv.slice(2), { verifySignature = kmsVerify } = {}) {
  ...
  const sourceSha = required(argv, "--source-sha"); const lineage = required(argv, "--lineage"); const serial = required(argv, "--serial");
