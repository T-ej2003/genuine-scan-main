...
import { assertRotationBackendTaskArn, runReconciledRotationDeployment } from "./production-rotation-reconciliation.mjs";
import { rotationReleaseIdentity } from "./commit-production-component-rotation-state.mjs";
...
if (mode === "rotation-overlap") {
  ...
  const approval = readBoundStageBPrivateJson({ filePath: environmentApprovalFile, expectedSha256: environmentApprovalSha256, label: "Production environment approval evidence" });
  assertProductionEnvironmentApprovalEvidence(approval, { sourceSha, repository: "T-ej2003/genuine-scan-main", environment: "production", workflowRef: process.env.GITHUB_WORKFLOW_REF, eventName: process.env.GITHUB_EVENT_NAME, workflowRunId: process.env.GITHUB_RUN_ID, workflowRunAttempt: process.env.GITHUB_RUN_ATTEMPT, executionActor: process.env.GITHUB_ACTOR, githubActions: process.env.GITHUB_ACTIONS });
  ...
  const receiptEnabled = [rotationFixtureSha256, receiptOutput].every(Boolean);
  if ([rotationFixtureSha256, receiptOutput].some(Boolean) && !receiptEnabled) throw new Error("Overlap deployment receipt inputs must be supplied together.");
  ...
  assert.equal(process.env.CLUSTER_NAME, "mscqr-prod-euw2-main");
  assert.equal(process.env.SERVICE_NAME, "mscqr-backend-servi-euw2");
  const deployOverlap = { run: () => runReconciledRotationDeployment({
    readiness: readiness.evidence, current: createProductionComponentDeploymentStateClient({ run }).read(),
    readers: createAppOnlyEcsReaders(run, { assertDefinitionArn: assertRotationBackendTaskArn }),
    expectedCurrentTaskDefinitionArn: process.env.EXPECTED_CURRENT_TASK_DEFINITION_ARN, taskDefinitionArn,
