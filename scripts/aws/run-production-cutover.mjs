...
process.stdout.write(`${JSON.stringify(mode === "prepare-overlap"
  ? { preparedForOverlapAuthorization: true, sourceSha, rotationId, rotationStateFile: config.rotationStateFile, rotationStateSha256: result.rotationStateSha256, taskDefinitionArn: result.taskDefinitionArn, expectedCurrentTaskDefinitionArn: config.expectedCurrentTaskDefinitionArn, imageDigest: config.backendImageDigest, deploymentSha: config.rotationDeploymentSha, readinessFile: result.readinessFile, readinessSha256: result.readinessSha256, ecsUpdateServiceCount: 0, mutationSequence: result.mutationSequence }
