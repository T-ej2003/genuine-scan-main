...
export async function executeProductionDualSlotRebaseline({ preconditions, sourceSha, rotationId, baselineIdentity, writePlan, readReferenceAudit, readSlot, writeSlot, completionFile, bindingsFile, repositoryRoot = process.cwd(), authorization, afterCompletionPersist, afterBindingsPersist } = {}) {
  ...
  const initialSnapshots = await Promise.all(expectedPlan.map(async (expected) => ({ expected, snapshot: await readSlot(expected.slot, expected.secretArn, expected.clientRequestToken, { sourceSha, rotationId }) })));
  if (outputExists.some(Boolean) && initialSnapshots.some(({ expected, snapshot }) => assertReadSnapshot(snapshot, expected, `Output preflight ${expected.slot}`) !== "COMPLETED")) fail("Existing durable rebaseline outputs cannot accompany an incomplete secret baseline.");
  for (const expected of expectedPlan) { const audit = await readReferenceAudit(); if (audit?.dualSlotReferences !== 0 || audit?.legacyRuntimeAuthoritative !== true || audit?.auditSha256 !== checked.liveReferenceAuditSha256) fail("Live reference audit changed during rebaseline."); const snapshot = await readSlot(expected.slot, expected.secretArn, expected.clientRequestToken, { sourceSha, rotationId }); if (assertReadSnapshot(snapshot, expected, `Pre-write ${expected.slot}`) === "COMPLETED") continue; const result = await writeSlot({ slot: expected.slot, secretArn: expected.secretArn, clientRequestToken: expected.clientRequestToken, payload: expected.payload, payloadSha256: expected.payloadSha256 }); writes += 1; if (result?.versionId !== expected.clientRequestToken || result?.arn !== expected.secretArn) fail(`Rebaseline write identity for ${expected.slot} is invalid.`); const verified = await readSlot(expected.slot, expected.secretArn, expected.clientRequestToken, { sourceSha, rotationId }); if (assertReadSnapshot(verified, expected, `Post-write ${expected.slot}`) !== "COMPLETED") fail(`Rebaseline write ${expected.slot} did not converge.`); }
  const finalAudit = await readReferenceAudit(); if (finalAudit?.dualSlotReferences !== 0 || finalAudit?.legacyRuntimeAuthoritative !== true || finalAudit?.auditSha256 !== checked.liveReferenceAuditSha256) fail("Live reference audit changed before baseline completion.");
  const finalSnapshots = []; for (const expected of expectedPlan) finalSnapshots.push({ slot: expected.slot, ...(await readSlot(expected.slot, expected.secretArn, expected.clientRequestToken, { sourceSha, rotationId })) }); const completion = buildBaselineCompletion({ preconditions: checked, sourceSha, rotationId, baselineIdentity, writePlan, finalSnapshots, authorizationBinding: authorization.authorizationSha256 });
