...
const PHASES = Object.freeze([
  ...
  ["verifier-role-assumption", "scripts/aws/establish-production-ecs-exec-verifier-session.mjs"],
...
const INITIAL_ACTIVATION_POLICY_RECONCILIATION_CAPABILITIES = Object.freeze([
  ["initial-activation-policy-reconciliation-root-identify", "sts:GetCallerIdentity", ["*"], false],
  ["initial-activation-policy-reconciliation-root-read-policy", "iam:GetPolicy", [INITIAL_ACTIVATION_POLICY_RECONCILIATION.policyArn], false],
  ["initial-activation-policy-reconciliation-root-read-policy-version", "iam:GetPolicyVersion", [INITIAL_ACTIVATION_POLICY_RECONCILIATION.policyArn], false],
...
const MIXED_DUAL_SLOT_RECOVERY_IAM_PREFLIGHT_CAPABILITIES = Object.freeze([
  ...
  ["mixed-recovery-iam-preflight-describe-secret", "secretsmanager:DescribeSecret", MIXED_DUAL_SLOT_RECOVERY_IAM_RESOURCES],
...
const MIXED_DUAL_SLOT_RECOVERY_EXECUTION_CAPABILITIES = Object.freeze([
  ...
  ["mixed-recovery-move-awscurrent", "secretsmanager:UpdateSecretVersionStage", MIXED_DUAL_SLOT_RECOVERY_IAM_RESOURCES, "scripts/aws/recover-production-mixed-dual-slot-topology.mjs"],
]);
...
const STAGE_A_PRODUCTION_ARTIFACTS_CAPABILITIES = Object.freeze([
  ...
  ["stage-a-artifacts-reconciliation-root-journal-read", "stage-a-production-artifacts-state-reconciliation", "ROOT_OPERATOR", "s3:GetObject", [PRODUCTION_ACTIVATION_LIFECYCLE.stageAProductionArtifactsReconciliationArn], false, "scripts/aws/run-production-stage-a-production-artifacts-reconciliation.mjs"],
  ...
  ["stage-a-artifacts-reconciliation-release-read-raw-state", "stage-a-production-artifacts-state-reconciliation", "RELEASE_DEPLOYER", "s3:GetObject", [stageATerraformStateArn], false, "scripts/aws/run-production-stage-a-production-artifacts-reconciliation.mjs"],
...
export function classifyStageARecoveryAwsCliAction({ action, source, offset } = {}) {
  if (action === "sts:GetCallerIdentity") return [{ identity: "RELEASE_DEPLOYER" }, { identity: "ROOT_OPERATOR" }];
  if (action === "s3:GetBucketPolicy") return [{ identity: "RELEASE_DEPLOYER" }];
  if (action === "s3:GetObject" && source.startsWith(STAGE_A_RECOVERY_RAW_STATE_READ_COMMAND, offset)) {
    return [{ identity: "RELEASE_DEPLOYER", capabilityId: "stage-a-artifacts-recovery-release-read-raw-state", sourceFunction: "stage-a-artifacts-recovery-release-read-raw-state", phase: "stage-a-production-artifacts-policy-recovery", resources: [stageATerraformStateArn] }];
  }
  return [{ identity: "ROOT_OPERATOR" }];
}
...
export function discoverAwsCliActions() {
  ...
  for (const sourceFile of awsCliSourceFiles) {
    ...
    for (const match of source.matchAll(pattern)) {
      ...
      if (sourceFile === "scripts/aws/production-stage-a-production-artifacts-journal.mjs" && ["s3:GetObject", "s3:ListBucket", "s3:PutObject"].includes(action)) {
        ...
        throw new Error("Initial activation lifecycle policy reconciliation core must not issue AWS commands outside the workflow runner.");
        ...
        const snapshotStart = source.indexOf("export function readInitialActivationLifecyclePolicyLiveState(");
        const runnerStart = source.indexOf("export function runInitialActivationLifecyclePolicyReconciliation(");
        const prepareStart = source.indexOf("  if (prepare) {", runnerStart);
        const executionStart = source.indexOf('  if (argv.includes("--admin-profile"))', prepareStart);
        const sharedRead = action === "sts:GetCallerIdentity"
          ? match.index > runnerStart && match.index < prepareStart
          : action !== "iam:CreatePolicyVersion" && match.index > snapshotStart && match.index < runnerStart;
        if (snapshotStart < 0 || runnerStart < 0 || prepareStart < 0 || executionStart < 0 || (!sharedRead && !(action === "iam:CreatePolicyVersion" && match.index > executionStart))) throw new Error("Initial activation reconciliation call is outside its reviewed execution context.");
  ...
  const reconciliationSource = fs.readFileSync(path.join(root, "scripts/aws/run-production-stage-a-production-artifacts-reconciliation.mjs"), "utf8");
  if (reconciliationSource.includes("Stage A reconciliation root journal reader")) calls.push(
    { sourceFile: "scripts/aws/run-production-stage-a-production-artifacts-reconciliation.mjs", sourceFunction: "stage-a-artifacts-reconciliation-root-identify", phase: "stage-a-production-artifacts-state-reconciliation", identity: "ROOT_OPERATOR", action: "sts:GetCallerIdentity", resources: ["*"], capabilityId: "stage-a-artifacts-reconciliation-root-identify" },
    { sourceFile: "scripts/aws/run-production-stage-a-production-artifacts-reconciliation.mjs", sourceFunction: "stage-a-artifacts-reconciliation-root-journal-read", phase: "stage-a-production-artifacts-state-reconciliation", identity: "ROOT_OPERATOR", action: "s3:GetObject", resources: [PRODUCTION_ACTIVATION_LIFECYCLE.stageAProductionArtifactsReconciliationArn], capabilityId: "stage-a-artifacts-reconciliation-root-journal-read" },
  );
  if (reconciliationSource.includes('readRawBackendStateIdentity: () => readRawTerraformStateIdentity(releaseRun)')) calls.push({ sourceFile: "scripts/aws/run-production-stage-a-production-artifacts-reconciliation.mjs", sourceFunction: "stage-a-artifacts-reconciliation-release-read-raw-state", phase: "stage-a-production-artifacts-state-reconciliation", identity: "RELEASE_DEPLOYER", action: "s3:GetObject", resources: [stageATerraformStateArn], capabilityId: "stage-a-artifacts-reconciliation-release-read-raw-state" });
  ...
  if (!/await send\(new UpdateSecretVersionStageCommand\(\{ SecretId: entry\.secretArn, VersionStage: "AWSCURRENT", MoveToVersionId: entry\.moveToVersionId, RemoveFromVersionId: entry\.removeFromVersionId \}\)\)/.test(mixedRecoverySource)) throw new Error("Mixed recovery AWS-legal move call is outside its reviewed execution context.");
  calls.push({ sourceFile: mixedRecoverySourceFile, sourceFunction: "executeMixedDualSlotRecovery", phase: "mixed-dual-slot-recovery-execution", identity: "MIXED_RECOVERY_EXECUTOR", action: "secretsmanager:UpdateSecretVersionStage", resources: [...MIXED_DUAL_SLOT_RECOVERY_IAM_RESOURCES], capabilityId: "mixed-recovery-move-awscurrent" });
...
export function buildStageBDeploymentCapabilityGraph() {
  ...
  const initialActivationPreparation = initialActivationPolicyReconciliation.filter(({ mutation }) => !mutation).map((capability) => {
  ...
  const mixedRecoveryExecution = MIXED_DUAL_SLOT_RECOVERY_EXECUTION_CAPABILITIES.map(([id, action, resources, sourceFile]) => ({
    id, phase: "mixed-dual-slot-recovery-execution", identity: "MIXED_RECOVERY_EXECUTOR", executor: "aws-sdk", sourceFile, sourceFunction: id === "mixed-recovery-move-awscurrent" ? "executeMixedDualSlotRecovery" : id, action, resources: [...resources],
  ...
  const mixedRecoveryIamAttestationSigning = {
    ...
    policy: { sourceFile: "scripts/aws/production-root-attestation-key.mjs", sid: "AccountAdministration", livePolicyArn: ROOT_ATTESTATION_KEY_ALIAS_ARN, expectedVersion: "protected-main-source", expectedPolicySha256: null }, required: true, mutation: true,
  };
  ...
  const capabilities = [...fixed, ...normalActivation, ...initialActivationPolicyReconciliation, ...initialActivationPreparation, ...providerReadonlyReconciliation, ...providerReadonlyPreparation, ...bootstrapOperatorPolicyReconciliation, ...mixedRecoveryIamPreflight, mixedRecoveryIamAttestationSigning, ...mixedRecoveryExecution, ...stageAProductionArtifacts, ROOT_DROP_SIGNING, ...rootAttestationRelease, ...recovery, ...forwardRecovery, ...stateReconciliation, ...stateReconciliationProviderReads, ...publisher, ...manifestCapabilities, ...checkerCapabilities, ...operatorCapabilities, ...runtimeAdmin, ...runtime].sort((a, b) => a.id.localeCompare(b.id));
  return {
...
export function assertStageBDeploymentCapabilityGraph(graph = readJson(CAPABILITY_GRAPH_PATH)) {
  ...
  const expectedStateReconciliationCapabilities = STAGE_B_STATE_RECONCILIATION_CAPABILITIES.length + readJson(manifestPath).required.filter(({ phase }) => phase === "refresh").length;
  if (stateReconciliation.length !== expectedStateReconciliationCapabilities || stateReconciliation.some(({ identity, executor, sourceFile, action, mutation, classification }) => identity !== "RELEASE_DEPLOYER" || executor !== "terraform" || sourceFile !== "scripts/aws/reconcile-production-green-stage-b-state.mjs" || ["ecs:RegisterTaskDefinition", "ecs:DeregisterTaskDefinition", "ecs:UpdateService", "iam:PutRolePolicy", "iam:AttachRolePolicy"].includes(action) || (classification === "TERRAFORM_STATE_ONLY_MUTATION" ? mutation !== true : classification === "TERRAFORM_PROVIDER_READ" ? mutation !== false : false))) throw new Error("Stage B refresh-only state reconciliation capability boundary is not exact.");
