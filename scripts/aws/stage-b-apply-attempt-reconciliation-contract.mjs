...
const assertIndependentReconciliationApproval = (evidence, { sourceSha, approvedBy } = {}) => {
  ...
  if (actualReviewer.toLowerCase() === evidence.executionActor.toLowerCase() || actualReviewer.toLowerCase() !== String(approvedBy).toLowerCase()) fail("Stage B reconciliation requires a distinct actual protected-environment reviewer.");
...
export function createStageBApplyAttemptReconciliationAuthorization({ protectedEnvironmentApprovalEvidence, reconciliationArtifact, reconciliationArtifactSha256, successorSourceSha, approvedBy, approverRole, verificationRef, now = new Date() } = {}) {
  ...
  if (protectedEnvironmentApprovalEvidence.workflowRef !== STAGE_B_APPLY_ATTEMPT_RECONCILIATION_WORKFLOW_REF || protectedEnvironmentApprovalEvidence.schemaVersion !== 3 || protectedEnvironmentApprovalEvidence.actualApproval?.userLogin?.toLowerCase() !== String(approvedBy).toLowerCase()) fail("Stage B reconciliation authorization requires actual independent approval from the dedicated workflow.");
...
export function assertStageBApplyAttemptReconciliationEligibility({ reservation, transitions = [], reconciliationArtifact, reconciliationArtifactSha256, authorization, authorizationSha256, successorSourceSha, expectedState = {}, now = new Date() } = {}) {
  ...
  assertStageBApplyAttemptTransition(predecessorTransitions.at(-1) || reservation, claim);
  const expectedSuccessorAttemptId = stageBApplyAttemptSuccessorIdentity({ predecessorAttemptId: reconciliationArtifact.predecessor.reservationIdentity, sourceSha: successorSourceSha, planSha256: reconciliationArtifact.predecessor.planSha256, savedPlanSha256: reconciliationArtifact.predecessor.savedPlanSha256, stateLineage: reconciliationArtifact.predecessor.stateLineage, stateSerial: reconciliationArtifact.predecessor.stateSerial, stateSha256: reconciliationArtifact.predecessor.stateSha256, workspace: reconciliationArtifact.predecessor.workspace, backendIdentitySha256: reconciliationArtifact.predecessor.backendIdentitySha256 });
  if (claim.status !== "ABORTED_BEFORE_APPLY" || claim.reconciliationBinding?.successorAttemptId !== expectedSuccessorAttemptId || authorization.successorAttemptId !== expectedSuccessorAttemptId) fail("Stage B reconciliation claim is not the exact authenticated next predecessor transition.");
