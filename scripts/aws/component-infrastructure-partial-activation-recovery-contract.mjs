...
const checkpointStates = new Set(["RECOVERY_EXECUTING", "RESOURCE_ADOPTED", "STATE_VERIFIED", "RECOVERY_CLOSED"]);
...
export function assertPartialActivationRecoveryPreparation(value) {
  assert.deepEqual(Object.keys(value || {}).sort(), ["backend", "historicalActivation", "iamInstallation", "liveTable", "lock", "recoveryTransitionId", "schemaVersion", "sourceSha", "stateIdentity"].sort());
  ...
  assert.deepEqual(Object.keys(value.historicalActivation || {}).sort(), ["authorizationArtifactSha256", "authorizationRunId", "planSha256", "preparationSha256", "sourceSha", "transitionId"]);
  assert.match(value.historicalActivation.sourceSha || "", /^[a-f0-9]{40}$/);
  assert.match(value.historicalActivation.authorizationRunId || "", /^[1-9][0-9]*$/);
  assert.match(value.historicalActivation.authorizationArtifactSha256 || "", artifactSha);
  for (const field of ["planSha256", "preparationSha256"]) assert.match(value.historicalActivation[field] || "", sha);
  assert.match(value.historicalActivation.transitionId || "", uuid);
