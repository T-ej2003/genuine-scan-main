...
if (mode === "production" || tfvarsPath || bindingReportPath) {
  ...
  if (mode === "production" && (recoveryPlan || partialApplyRecovery)) {
    ...
    assertStageBRecoveryProvenance({ refreshReport: JSON.parse(fs.readFileSync(process.env.STAGE_B_REFRESH_REPORT_PATH)), refreshReportSha256: process.env.STAGE_B_REFRESH_REPORT_SHA256, observationBindingReport: refreshBindingReport, observationBindingReportSha256: process.env.STAGE_B_REFRESH_BINDING_REPORT_SHA256, recoveryBindingReport: bindingReport, recoveryClassificationSha256: process.env.STAGE_B_RECOVERY_CLASSIFICATION_SHA256, recoveryAttestationSha256: process.env.STAGE_B_RECOVERY_ATTESTATION_SHA256, recoveryMode: partialApplyRecovery ? "PARTIAL_APPLY_RECOVERY" : "RECOVERY_ALIAS_ONLY" });
