...
export function assertNormalReleaseAuthorizationTransport({ sourceSha, authorizationBytes, expectedSha256 } = {}) {
  ...
  if (authorization?.schemaVersion !== 2 || authorization.valid !== true || authorization.sourceSha !== sourceSha || authorization.authorizationSha256 !== authorization.evidenceSha256) throw new Error("Normal release image authorization is stale or not a canonical source-bound envelope.");
