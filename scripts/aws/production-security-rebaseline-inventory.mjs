...
export const SECURITY_REBASELINE_COLLECTOR_VERSION = "production-security-catalogue-v12";
...
export const SECURITY_REBASELINE_COVERAGE = Object.freeze([
  ...
  ["replication and user bindings", "pg_publication/pg_publication_rel/pg_publication_namespace/pg_subscription and pg_subscription_rel via app_rls.production_security_subscription_inventory (subconninfo represented only by a domain-separated digest)/pg_replication_slots/pg_db_role_setting/pg_seclabel/pg_operator/pg_cast/pg_foreign_data_wrapper/pg_foreign_server/pg_user_mapping/pg_foreign_table/pg_language/pg_largeobject_metadata (owner+ACL aggregate only; no OIDs or contents)/pg_tablespace (non-system owner+ACL only)", "securityBindings", "bindings"],
  ["routines", "pg_proc/pg_aggregate", "securityRoutines", "routines/routineGrants"],
  ["relations", "pg_class/pg_attribute/pg_constraint/pg_inherits/pg_index/pg_partitioned_table", "securityTables", "tables/tableGrants/columnGrants"],
  ...
  ["default ACL", "pg_default_acl", "defaults", "defaultPrivileges"], ["parameter ACL", "pg_parameter_acl", "parameterPrivileges", "parameterPrivileges"], ["types", "pg_type/pg_attribute/pg_range", "types", "types/typeGrants"],
...
const FIELDS = Object.freeze({
  ...
  tables: ["kind","persistence","rls","forced","populated","owner","access_method","of_type","view_definition","view_security_options","partition_key","partition_bound","parents","replica_identity","behavior_indexes","columns","constraints"], triggers: ["enabled","function","definition"],
  ...
  databases: ["owner","allow_connections","connection_limit"],
  roleMetadata: ["comment_present","comment_sha256","settings","unsupported_settings"], types: ["kind","owner","category","not_null","base_type","collation","default_value","default_expression","implementation","composite_attributes","range_definition","enum_labels","constraints"],
  sequences: ["owner","persistence","data_type","start_value","increment_by","maximum","minimum","cache_size","cycle"],
...
export function normalizeSecurityRebaselineCatalogue(catalogue, { kind = "LIVE" } = {}) {
  ...
  for (const row of catalogue.securityTables) { exactKeys(row, ["schema","name","kind","persistence","rls","forced","populated","owner","access_method","of_type","view_definition","view_security_options","partition_key","partition_bound","parents","replica_identity","behavior_indexes","columns","constraints","grants","column_grants"], "Table"); const identity=`${row.schema}.${row.name}`, { grants, column_grants: columnGrants, ...fields } = row; delete fields.schema; delete fields.name; add("tables", identity, fields); addGrants("tableGrants", identity, grants); addGrants("columnGrants", identity, columnGrants, "column"); }
  ...
  const databaseFields = new Map(); for (const row of catalogue.databases) { exactKeys(row, ["database","owner","allow_connections","connection_limit","role","grantor","privilege","grantable"], "Database ACL"); const identity = rowIdentity("databases", row), fields = { owner:row.owner,allow_connections:row.allow_connections,connection_limit:row.connection_limit }; addUnexpectedRole(row.role, `database:${identity}`); addUnexpectedRole(row.grantor, `database-grantor:${identity}`); if (databaseFields.has(row.database)) assert.deepEqual(databaseFields.get(row.database), fields); else databaseFields.set(row.database, fields); add("databaseGrants", identity, { present: true }); } for (const [database, fields] of databaseFields) add("databases", database, fields);
  ...
  for (const collection of ["types", "sequences"]) for (const row of catalogue[collection]) { exactKeys(row, collection === "types" ? ["schema","name","kind","owner","category","not_null","base_type","collation","default_value","default_expression","implementation","composite_attributes","range_definition","enum_labels","constraints","grants"] : ["schema","name","owner","persistence","data_type","start_value","increment_by","maximum","minimum","cache_size","cycle","grants"], collection); const identity = rowIdentity(collection, row), { grants, schema, name, ...fields } = row; add(collection, identity, fields); addGrants(collection === "types" ? "typeGrants" : "sequenceGrants", identity, grants); }
