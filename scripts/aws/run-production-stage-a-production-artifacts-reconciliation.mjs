...
const authenticateRecovery = async ({ sourceSha, recoverySourceSha = sourceSha, continuationRebindWorkflowRunId, continuationRebindWorkflowRunAttempt, releaseRun, adapter, journal, verifySignature, recoveryWorkflowRunId, recoveryWorkflowRunAttempt, resolveAuthorization, readProtectedSource, proveDescendant, readGovernedExecutableManifestSha256, allowAdvancedState = false }) => {
  ...
  if (source.head !== recoverySourceSha) {
    ...
    continuationRebindAuthorization = assertStageAProductionArtifactsContinuationRebindAuthorization(rebind.authorization, { historicalRecoveryAuthorization: recovery.authorization, recoveryCompletion: completionEvidence, sourceSha: source.head, readGovernedExecutableManifestSha256 });
  }
  ...
  return Object.freeze({ ...source, recoverySourceSha, recovery, preState, currentState, completionEvidence, continuationRebindAuthorization, verifyRecoveryCompletion });
};
...
export async function runStageAProductionArtifactsReconciliation({ sourceSha, recoverySourceSha = sourceSha, continuationRebindWorkflowRunId, continuationRebindWorkflowRunAttempt, recoveryWorkflowRunId, recoveryWorkflowRunAttempt, reconciliationWorkflowRunId, reconciliationWorkflowRunAttempt, releaseRun, rootRun, adapter, terraformStateLock, resolveAuthorization = resolveStageAProductionArtifactsAuthorizationArtifact, journal, rootJournal, verifySignature, readProtectedSource = readStageBProtectedMainCheckout, proveDescendant = proveProtectedMainDescendant, readGovernedExecutableManifestSha256, preparedEvidence, saved: suppliedSaved, savedPlanPath, prepareEvidencePath } = {}) {
  ...
  assertStageAProductionArtifactsReconciliationGovernanceAuthorization(reconciliation.authorization, { sourceSha: context.head, recoverySourceSha: context.recoverySourceSha, preState: context.preState, recoveryAuthorization: context.recovery.authorization, recoveryCompletion: context.completionEvidence, continuationRebindAuthorization: context.continuationRebindAuthorization, prepareEvidence: evidence, savedPlanSha256: saved.savedPlanSha256, verifyRecoveryCompletionEvidence: verifySignature, proveDescendant, readGovernedExecutableManifestSha256 });
