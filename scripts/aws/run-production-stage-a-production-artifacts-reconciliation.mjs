...
const authenticateRecovery = async ({ sourceSha, releaseRun, adapter, journal, verifySignature, recoveryWorkflowRunId, recoveryWorkflowRunAttempt, resolveAuthorization, readProtectedSource }) => {
  ...
  const recovery = resolveAuthorization({ workflowRunId: recoveryWorkflowRunId, workflowRunAttempt: recoveryWorkflowRunAttempt, sourceSha: source.head, operation: STAGE_A_PRODUCTION_ARTIFACTS_RECOVERY_OPERATION });
  const preState = await adapter.readStateIdentity();
  assertStageAProductionArtifactsRecoveryAuthorization(recovery.authorization, { sourceSha: source.head, preState });
  ...
  return Object.freeze({ ...source, recovery, preState, completionEvidence, verifyRecoveryCompletion });
};

export async function prepareStageAProductionArtifactsReconciliation({ sourceSha, recoveryWorkflowRunId, recoveryWorkflowRunAttempt, releaseRun, adapter, resolveAuthorization = resolveStageAProductionArtifactsAuthorizationArtifact, journal, verifySignature, readProtectedSource = readStageBProtectedMainCheckout } = {}) {
  ...
  const context = await authenticateRecovery({ sourceSha, releaseRun, adapter, journal, verifySignature, recoveryWorkflowRunId, recoveryWorkflowRunAttempt, resolveAuthorization, readProtectedSource });
  return Object.freeze({ ...context, prepared: await prepareStageAProductionArtifactsStateReconciliation({ adapter, sourceSha: context.head, recoveryCompletion: context.completionEvidence.completion, verifyRecoveryCompletion: context.verifyRecoveryCompletion, preState: context.preState, assertSourceIntegrity: () => protectedSource(readProtectedSource, context.head) }) });
}

export async function runStageAProductionArtifactsReconciliation({ sourceSha, recoveryWorkflowRunId, recoveryWorkflowRunAttempt, reconciliationWorkflowRunId, reconciliationWorkflowRunAttempt, releaseRun, adapter, resolveAuthorization = resolveStageAProductionArtifactsAuthorizationArtifact, journal, verifySignature, readProtectedSource = readStageBProtectedMainCheckout, preparedEvidence, saved: suppliedSaved, savedPlanPath, prepareEvidencePath } = {}) {
  ...
  assertStageAProductionArtifactsReconciliationGovernanceAuthorization(reconciliation.authorization, { sourceSha: context.head, preState: context.preState, recoveryAuthorization: context.recovery.authorization, recoveryCompletion: context.completionEvidence, prepareEvidence: evidence, savedPlanSha256: saved.savedPlanSha256, verifyRecoveryCompletionEvidence: verifySignature });
  const coreAuthorization = createCoreAuthorization({ sourceSha: context.head, recoveryCompletion: context.completionEvidence.completion, savedPlanSha256: saved.savedPlanSha256, stateLineage: context.preState.lineage, preStateSerial: context.preState.serial, preStateSha256: context.preState.stateSha256, verifyRecoveryCompletion: context.verifyRecoveryCompletion });
  ...
  const result = await runStageAProductionArtifactsStateReconciliation({ adapter, sourceSha: context.head, recoveryCompletion: context.completionEvidence.completion, verifyRecoveryCompletion: context.verifyRecoveryCompletion, reconciliationAuthorization: coreAuthorization, saved, preparedState: context.preState, assertSourceIntegrity: () => protectedSource(readProtectedSource, context.head), verifyReconciliationAuthorization: (authorization) => {
