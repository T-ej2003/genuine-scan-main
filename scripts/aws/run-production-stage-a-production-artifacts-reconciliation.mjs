...
const authenticateRecovery = async ({ sourceSha, recoverySourceSha = sourceSha, releaseRun, adapter, journal, verifySignature, recoveryWorkflowRunId, recoveryWorkflowRunAttempt, resolveAuthorization, readProtectedSource, proveDescendant, readGovernedExecutableManifestSha256, allowAdvancedState = false }) => {
  ...
  assertStageAProductionArtifactsRecoverySourceCompatibility({ sourceSha: source.head, recoverySourceSha, proveDescendant, historicalGovernedExecutableManifestSha256: recovery.authorization?.governedExecutableManifestSha256, readGovernedExecutableManifestSha256 });
...
export async function prepareStageAProductionArtifactsReconciliation({ sourceSha, recoverySourceSha = sourceSha, recoveryWorkflowRunId, recoveryWorkflowRunAttempt, releaseRun, adapter, resolveAuthorization = resolveStageAProductionArtifactsAuthorizationArtifact, journal, verifySignature, readProtectedSource = readStageBProtectedMainCheckout, proveDescendant = proveProtectedMainDescendant, readGovernedExecutableManifestSha256 } = {}) {
  ...
  const context = await authenticateRecovery({ sourceSha, recoverySourceSha, releaseRun, adapter, journal, verifySignature, recoveryWorkflowRunId, recoveryWorkflowRunAttempt, resolveAuthorization, readProtectedSource, proveDescendant, readGovernedExecutableManifestSha256 });
...
export async function runStageAProductionArtifactsReconciliation({ sourceSha, recoverySourceSha = sourceSha, recoveryWorkflowRunId, recoveryWorkflowRunAttempt, reconciliationWorkflowRunId, reconciliationWorkflowRunAttempt, releaseRun, adapter, terraformStateLock, resolveAuthorization = resolveStageAProductionArtifactsAuthorizationArtifact, journal, verifySignature, readProtectedSource = readStageBProtectedMainCheckout, proveDescendant = proveProtectedMainDescendant, readGovernedExecutableManifestSha256, preparedEvidence, saved: suppliedSaved, savedPlanPath, prepareEvidencePath } = {}) {
