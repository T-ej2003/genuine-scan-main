...
const submission = { ...bundle, authorization, ...(mode === "successor-recovery" ? { recoveryEnvelope: json("recovery-envelope"), imageAuthorization: json("image-authorization") } : {}) };
writeStageBPrivateFileAtomic({ filePath: required("output"), bytes: Buffer.from(`${JSON.stringify(submission, null, 2)}\n`), repositoryRoot: process.cwd(), label: "Dual-slot rebaseline durable evidence bundle" });
