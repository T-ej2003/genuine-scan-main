...
import { assertStageBTerraformBackendConfig, assertStageBTerraformBackendPolicy } from "./aws/stage-b-terraform-backend-contract.mjs";
...
function readInitializedBackendConfig() {
  const metadataPath = path.join(root, terraformRoot, ".terraform", "terraform.tfstate");
  return JSON.parse(fs.readFileSync(metadataPath, "utf8"))?.backend?.config;
}
...
export function runApply({ argv = process.argv.slice(2), env = process.env, deps = { getCaller: currentCaller, apply: (planPath) => spawnSync("terraform", [`-chdir=${terraformRoot}`, "apply", "-input=false", "-no-color", planPath], { cwd: root, env, encoding: "utf8", stdio: "inherit" }) } } = {}) {
  ...
  const defaultDeps = { getCaller: currentCaller, showPlan: showSavedPlan, validatePlan: assertStageBPlan, getBackendConfig: readInitializedBackendConfig, apply: (planPath) => spawnSync("terraform", [`-chdir=${terraformRoot}`, "apply", "-input=false", "-no-color", planPath], { cwd: root, env, encoding: "utf8", stdio: "inherit" }) };
  ...
  assertStageBTerraformBackendConfig(effectiveDeps.getBackendConfig());
