...
test("stale supersession routes reads and writes through the sanitized runner without exposing payload argv", async () => {
  ...
  const { GetSecretValueCommand, PutSecretValueCommand } = requireBackend("@aws-sdk/client-secrets-manager");
  const secretId = arn("mscqr/prod/rotation/jwt_pending");
  await send(new GetSecretValueCommand({ SecretId: secretId }));
  await send(new GetSecretValueCommand({ SecretId: secretId, VersionId: null }));
  await send(new GetSecretValueCommand({ SecretId: secretId, VersionId: "" }));
  await send(new GetSecretValueCommand({ SecretId: secretId, VersionId: "b".repeat(64) }));
  await send(new PutSecretValueCommand({ SecretId: secretId, ClientRequestToken: "a".repeat(64), SecretString: '{"value":"synthetic"}' }));
  assert.deepEqual(calls[0].args, ["secretsmanager", "get-secret-value", "--secret-id", secretId, "--output", "json", "--no-cli-pager"]);
  assert.deepEqual(calls[1].args, calls[0].args);
  assert.deepEqual(calls[2].args, calls[0].args);
  assert.deepEqual(calls[3].args, ["secretsmanager", "get-secret-value", "--secret-id", secretId, "--version-id", "b".repeat(64), "--output", "json", "--no-cli-pager"]);
  assert.equal(calls.slice(0, 4).flatMap(({ args }) => args).includes("undefined"), false);
  assert.equal(calls.slice(0, 4).flatMap(({ args }) => args).includes("null"), false);
  assert.equal(calls.slice(0, 4).flatMap(({ args }) => args).includes(""), false);
  assert.equal(calls[4].args.includes('{"value":"synthetic"}'), false);
  assert.equal(calls[4].args.includes("file:///dev/stdin"), true);
  assert.equal(calls[4].options.input, '{"value":"synthetic"}');
