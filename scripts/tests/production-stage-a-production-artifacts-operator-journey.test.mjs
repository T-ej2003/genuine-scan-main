...
test("real Stage-A recovery and reconciliation runner composition reaches one state-only apply", async () => {
  ...
  const recoveryAuthorization = createRecoveryAuthorization({ sourceSha, preState: state, protectedEnvironmentApprovalEvidence: approval(PRODUCTION_ENVIRONMENT_APPROVAL.stageAProductionArtifactsRecoveryWorkflowRef, "101"), verificationRef: "manual", governedExecutableManifestSha256, transition: reservationTransition });
...
test("historical A-to-B recovery selects root attempt and release completion journals", async () => {
  ...
  const releaseJournal = { readRecoveryAttempt: () => { releaseAttemptWrites += 1; throw new Error("release journal is not permitted for A-to-B attempt"); }, readRecoveryCompletion: () => completionBytes && { bytes: completionBytes }, writeRecoveryAttempt: () => { releaseAttemptWrites += 1; throw new Error("release journal is not permitted for A-to-B attempt"); }, writeRecoveryCompletion: ({ bytes }) => { releaseCompletionWrites += 1; completionBytes = bytes; return { key: "completion" }; } };
  const rootJournal = { readRecoveryAttempt: () => attemptBytes && { bytes: attemptBytes }, readRecoveryCompletion: () => null, writeRecoveryAttempt: ({ bytes }) => { rootAttemptWrites += 1; attemptBytes = bytes; return { key: "attempt" }; }, writeRecoveryCompletion: () => { rootCompletionWrites += 1; throw new Error("root journal is not permitted for A-to-B completion"); } };
  ...
  assert.equal(result.putBucketPolicyCount, 1); assert.equal(policyWrites, 1); assert.equal(rootAttemptWrites, 1); assert.equal(rootCompletionWrites, 0); assert.equal(releaseAttemptWrites, 0); assert.equal(releaseCompletionWrites, 1);
});

test("historical A-to-B descendant execution is rejected after source compatibility", async () => {
  const recoverySourceSha = "a".repeat(40); const descendantSourceSha = "b".repeat(40);
  const authorization = createStageAProductionArtifactsRecoveryAuthorization({ sourceSha: recoverySourceSha, preState: state, protectedEnvironmentApprovalEvidence: approval(PRODUCTION_ENVIRONMENT_APPROVAL.stageAProductionArtifactsRecoveryWorkflowRef, "104", recoverySourceSha), verificationRef: "historical-descendant" });
  const rejectAws = () => { throw new Error("AWS must not be reached"); };
  await assert.rejects(() => runStageAProductionArtifactsRecovery({ sourceSha: descendantSourceSha, recoverySourceSha, workflowRunId: "104", workflowRunAttempt: "1", rootRun: rejectAws, releaseRun: rejectAws, readStateIdentity: async () => state, terraformStateLock, readProtectedSource: () => ({ headSha: descendantSourceSha }), proveDescendant: () => true, readGovernedExecutableManifestSha256: () => governedExecutableManifestSha256, resolveAuthorization: () => ({ authorization }), journal: { readRecoveryCompletion: () => null, writeRecoveryCompletion: () => {} }, recoveryJournal: { readRecoveryAttempt: () => null, writeRecoveryAttempt: () => {} }, rootRecoveryJournal: { readRecoveryCompletion: () => null }, sign: () => "signature", verify: () => true }), /not supported from a descendant source/);
});
...
test("recovery resumes exact desired policy only from its signed immutable attempt", async () => {
  ...
  const recoveryJournal = { readRecoveryAttempt: () => attemptBytes && { bytes: attemptBytes }, readRecoveryCompletion: () => { rootCompletionReads += 1; return null; }, writeRecoveryAttempt: ({ bytes }) => { attemptBytes = bytes; return { key: "attempt" }; }, writeRecoveryCompletion: () => { throw new Error("root journal must not write A-to-B completion"); } };
  ...
  assert.equal(lockAcquires, 2); assert.equal(lockReleases, 2);
...
test("recovery attempt persistence uses the release writer and gates the root policy mutation", async () => {
  const authorization = createRecoveryAuthorization({ sourceSha, preState: state, protectedEnvironmentApprovalEvidence: approval(PRODUCTION_ENVIRONMENT_APPROVAL.stageAProductionArtifactsRecoveryWorkflowRef, "203"), verificationRef: "release-attempt-writer", governedExecutableManifestSha256, transition: reservationTransition });
...
test("reservation-to-A first run authenticates release 403 absence through the exact-key root fallback", async () => {
  ...
  const rootRun = (args) => {
    ...
    if (args[1] === "get-bucket-versioning") return JSON.stringify({ Status: "Enabled" });
    if (args[1] === "get-bucket-lifecycle-configuration") throw new Error("NoSuchLifecycleConfiguration");
    if (args[1] === "put-bucket-policy") { events.push("policy-write"); policyWrites += 1; livePolicy = buildStageAProductionArtifactsBucketPolicyWithInitialActivationReservation(); return ""; }
    throw new Error(`unexpected root ${args[1]}`);
  };
  const releaseJournal = {
    readRecoveryAttempt: (sha) => { events.push(["release-read", sha]); throw denied(); },
    writeRecoveryAttempt: ({ recoveryAuthorizationSha256: sha }) => { events.push(["release-write", sha]); attemptWrites += 1; return { key: "attempt" }; },
    readRecoveryCompletion: () => null,
    writeRecoveryCompletion: () => { events.push("release-completion"); completionWrites += 1; return { key: "completion" }; },
  };
  const rootJournal = {
  ...
  const releaseRead = events.find((event) => event[0] === "release-read"); const rootRead = events.find((event) => event[0] === "root-read"); const releaseWrite = events.find((event) => event[0] === "release-write");
...
test("reservation-to-A release 403 with an unverifiable root fallback fails before any write", async () => {
  ...
  const releaseJournal = { readRecoveryAttempt: () => { throw denied(); }, readRecoveryCompletion: () => null, writeRecoveryAttempt: () => { attemptWrites += 1; } };
  const rootJournal = { readRecoveryAttempt: () => { throw denied(); }, readRecoveryCompletion: () => null, writeRecoveryAttempt: () => { attemptWrites += 1; } };
  await assert.rejects(() => runStageAProductionArtifactsRecovery({ sourceSha, workflowRunId: "205", workflowRunAttempt: "1", rootRun, releaseRun, readStateIdentity: async () => state, terraformStateLock, readProtectedSource: source, resolveAuthorization: () => ({ authorization }), journal: { readRecoveryCompletion: () => null, writeRecoveryCompletion: () => { policyWrites += 1; } }, recoveryJournal: releaseJournal, rootRecoveryJournal: rootJournal, sign: () => Buffer.from("signature").toString("base64"), verify: () => true }), /AccessDenied/);
  assert.equal(attemptWrites, 0); assert.equal(policyWrites, 0);
});

test("A-prime to B installs the ProviderReadOnly journal durability boundary once", async () => {
  const authorization = createRecoveryAuthorization({ sourceSha, preState: state, protectedEnvironmentApprovalEvidence: approval(PRODUCTION_ENVIRONMENT_APPROVAL.stageAProductionArtifactsRecoveryWorkflowRef, "219"), verificationRef: "provider-readonly-journal", governedExecutableManifestSha256, transition: providerReadonlyTransition });
  let livePolicy = buildStageAProductionArtifactsBucketPolicyWithRecoveryListBucketBootstrap(); let policyWrites = 0; let attempt; let completion;
  const releaseRun = (args) => args[1] === "get-caller-identity" ? releaseIdentity : JSON.stringify({ Policy: JSON.stringify(livePolicy) });
  const rootRun = (args) => {
    if (args[1] === "get-caller-identity") return rootIdentity;
    if (args[1] === "get-bucket-versioning") return JSON.stringify({ Status: "Enabled" });
    if (args[1] === "get-bucket-lifecycle-configuration") throw new Error("NoSuchLifecycleConfiguration");
    if (args[1] === "put-bucket-policy") { policyWrites += 1; livePolicy = buildStageAProductionArtifactsBucketPolicyWithProviderReadonlyJournalProtection(); return ""; }
    throw new Error(`unexpected root ${args[1]}`);
  };
  ...
  const input = { sourceSha, workflowRunId: "219", workflowRunAttempt: "1", rootRun, releaseRun, readStateIdentity: async () => state, terraformStateLock, readProtectedSource: source, resolveAuthorization: () => ({ authorization }), journal, recoveryJournal: journal, rootRecoveryJournal: journal, sign: () => Buffer.from("signature").toString("base64"), verify: () => true };
...
test("A to A-prime uses the pre-existing root journal, then A-prime to B uses the scoped release journal", async () => {
  ...
  const rootRun = (args) => {
    ...
    if (args[1] === "get-bucket-versioning") return JSON.stringify({ Status: "Enabled" });
    if (args[1] === "get-bucket-lifecycle-configuration") throw new Error("NoSuchLifecycleConfiguration");
    if (args[1] === "put-bucket-policy") { writes += 1; livePolicy = writes === 1 ? buildStageAProductionArtifactsBucketPolicyWithRecoveryListBucketBootstrap() : buildStageAProductionArtifactsBucketPolicyWithProviderReadonlyJournalProtection(); return ""; }
    throw new Error(`unexpected root ${args[1]}`);
  };
  const releaseRun = (args) => args[1] === "get-caller-identity" ? releaseIdentity : JSON.stringify({ Policy: JSON.stringify(livePolicy) });
  const common = { sourceSha, readStateIdentity: async () => state, terraformStateLock, rootRun, releaseRun, readProtectedSource: source, journal: releaseJournal, recoveryJournal: releaseJournal, rootRecoveryJournal: rootJournal, sign: () => Buffer.from("signature").toString("base64"), verify: () => true };
  for (const [workflowRunId, transition] of [["221", bootstrapTransition], ["222", providerReadonlyTransition]]) {
...
test("post-write continuation locks only after authenticated P2 evidence", async () => {
  const authorization = createRecoveryAuthorization({ sourceSha, preState: state, protectedEnvironmentApprovalEvidence: approval(PRODUCTION_ENVIRONMENT_APPROVAL.stageAProductionArtifactsRecoveryWorkflowRef, "220"), verificationRef: "post-write-no-lock", governedExecutableManifestSha256, transition: reservationTransition });
...
test("post-write continuation holds the existing lock through state CAS and concurrent completion publication", async () => {
  ...
  const collisionJournal = { ...journal, writeRecoveryCompletion: () => { completionWrites += 1; completionBytes = Buffer.from(JSON.stringify(completion)); throw new Error("conditional collision"); } };
  const collision = await runStageAProductionArtifactsRecovery({ ...base, journal: collisionJournal, readStateIdentity: async () => ({ ...state }) });
  assert.equal(collision.alreadyComplete, true); assert.equal(lockAcquires, 1); assert.equal(lockReleases, 1); assert.equal(completionWrites, 1);
...
test("recovery journal retention rejects overlapping current-object expiration and transitions", () => {
  ...
  for (const rule of [
    { Prefix: "production-provider-readonly-policy-reconciliation/" },
    { Prefix: "production-provider-readonly-policy-reconciliation/operation/" },
    { Prefix: "production-" },
  ]) assert.throws(() => assertStageAProductionArtifactsJournalRetention({ Rules: [destructive(rule)] }), /protected immutable record unavailable/);
  ...
  assert.doesNotThrow(() => assertStageAProductionArtifactsJournalRetention({ Rules: [{ Status: "Enabled", Prefix: "", Expiration: { ExpiredObjectDeleteMarker: true } }] }));
...
test("reservation-affecting lifecycle fails before the governed bucket-policy write", async () => {
  const authorization = createRecoveryAuthorization({ sourceSha, preState: state, protectedEnvironmentApprovalEvidence: approval(PRODUCTION_ENVIRONMENT_APPROVAL.stageAProductionArtifactsRecoveryWorkflowRef, "506"), verificationRef: "retention", governedExecutableManifestSha256, transition: reservationTransition });
  let policyWrites = 0;
