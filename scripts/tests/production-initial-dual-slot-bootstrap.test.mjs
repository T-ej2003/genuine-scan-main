...
test("FULL_INITIAL_MIGRATION_SIMULATION keeps envelopes out of ECS and rotates QR key/version atomically", async () => {
  ...
  try {
    ...
    const task = buildOverlapTaskDefinition({ backendImage: "368992683803.dkr.ecr.eu-west-2.amazonaws.com/mscqr-backend@sha256:" + "a".repeat(64), releaseSha: sourceSha, backendLogGroup: "/ecs/rotation", proxyTrust: { mode: "cloudfront-alb", albCidrs: "10.1.0.0/24", cloudFrontCidrs: "198.51.100.0/24", cloudFrontPrefixListId: "pl-0123456789abcdef0", cloudFrontPrefixListVersion: "7" }, postPrepare: true, secretBindings: { ...rotationEcs, ...artifact, ROTATION_INVENTORY_RLS_ROLE: "mscqr_prod_rls_read" } });
