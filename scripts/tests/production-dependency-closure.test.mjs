...
test("complete production dependency closure is exact across modes and failure paths", () => {
  ...
  assert.deepEqual(stageAAdditions, [
    ...
    ["scripts/aws/production-stage-a-production-artifacts-journal.mjs", "s3:ListBucket", "stage-a-artifacts-journal-list-absence"],
    ...
    ["scripts/aws/production-stage-a-production-artifacts-journal.mjs", "s3:ListBucket", "stage-a-artifacts-recovery-root-journal-list-absence"],
  ...
  assert.equal(report.newAwsCalls.length, 44 + stageAAdditions.length + 15 + 14 + 21 + 7 + 1 + 6 + 1 + 5 + 66 + 1); // Historical closure, exact ALB attribute read, and separately identity-mapped app-only calls.
  ...
  assert.deepEqual(report.newAwsCalls.filter(({ capabilityId }) => capabilityId === "mixed-recovery-move-awscurrent").map(({ action, resources, identity, reachableMode }) => [action, resources, identity, reachableMode]), [
    ["secretsmanager:UpdateSecretVersionStage", graph().capabilities.find(({ id }) => id === "mixed-recovery-move-awscurrent").resources, "MIXED_RECOVERY_EXECUTOR", ["MIXED_DUAL_SLOT_RECOVERY"]],
  ]);
  ...
  assert.equal(reconciliationMode.some(({ capabilityId }) => capabilityId === "stage-a-artifacts-journal-list-absence"), true);
  ...
  assert.deepEqual(report.newAwsCalls.filter(({ sourceFile }) => sourceFile.endsWith("production-stage-a-production-artifacts-journal.mjs")).map(({ action, capabilityId, identity }) => [action, capabilityId, identity]), [
    ...
    ["s3:ListBucket", "stage-a-artifacts-journal-list-absence", "RELEASE_DEPLOYER"],
    ...
    ["s3:ListBucket", "stage-a-artifacts-recovery-root-journal-list-absence", "ROOT_OPERATOR"],
    ["s3:PutObject", "stage-a-artifacts-recovery-root-journal-conditional-create", "ROOT_OPERATOR"],
  ]);
  for (const id of ["stage-a-artifacts-journal-read", "stage-a-artifacts-journal-list-absence", "stage-a-artifacts-journal-conditional-create"]) assert.deepEqual(report.newAwsCalls.find(({ capabilityId }) => capabilityId === id)?.reachableMode, ["STAGE_A_PRODUCTION_ARTIFACTS_POLICY_RECOVERY", "STAGE_A_PRODUCTION_ARTIFACTS_STATE_RECONCILIATION"]);
...
test("Stage A production-artifacts mode closure is tuple-exact and omission-proof", () => {
  ...
  for (const id of ["stage-a-artifacts-journal-read", "stage-a-artifacts-journal-list-absence", "stage-a-artifacts-journal-conditional-create", "stage-a-artifacts-recovery-release-lock-acquire", "stage-a-artifacts-recovery-release-lock-release", "stage-a-artifacts-reconciliation-terraform-read-bucket-location", "stage-a-artifacts-reconciliation-terraform-read-state", "release-root-attestation-verify", "release-root-attestation-describe-key", "release-root-attestation-read-key-policy", "release-root-attestation-read-key-tags"]) {
    assert.deepEqual(modesFor(id)?.slice(0, 2), [recovery, reconciliation], id);
  }
  for (const id of ["stage-a-artifacts-recovery-root-journal-read", "stage-a-artifacts-recovery-root-journal-list-absence", "stage-a-artifacts-recovery-root-journal-conditional-create", "stage-a-artifacts-recovery-root-sign"]) assert.deepEqual(modesFor(id), [recovery], id);
