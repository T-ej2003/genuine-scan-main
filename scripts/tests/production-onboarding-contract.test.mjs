...
const overlap = { valid: true, evidenceRef: "onboarding:test", evidenceSha256: "c".repeat(64), sourceSha: "a".repeat(40), imageDigest: `sha256:${"b".repeat(64)}`, taskDefinitionArn: "arn:aws:ecs:eu-west-2:368992683803:task-definition/mscqr-production-rls-green-backend-candidate:1", taskArn: "arn:aws:ecs:eu-west-2:368992683803:task/cluster/task", rotationId: "rotation-test-1234", rotationStateSha256: "d".repeat(64), taskMarker: true, ecsExecProof: true, serviceStable: true, targetTaskDefinitionMatch: true, targetImageDigestMatch: true, health: { serviceHealthy: true, healthReleaseGitSha: "a".repeat(40) }, rotationPhase: "verified", runtime: Object.fromEntries(names.map((name) => [name, true])), acceptance: Object.fromEntries(acceptance.map((name) => [name, true])) };
