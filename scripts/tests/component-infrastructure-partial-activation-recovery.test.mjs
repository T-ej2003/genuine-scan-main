...
test("a retained native import lock is captured only through the incident-bound continuation", async t => {
  ...
  await run(["recover", f.directory, "790"], f.dependencies); assert(captured);
});
...
test("fresh authorization is exact, short-lived and cannot become a historical execution approval", () => {
  const preparation = { schemaVersion: 1, sourceSha, recoveryTransitionId, stateIdentity: "INFRASTRUCTURE_CREATED_STATE_INCOMPLETE", backend: contract, historicalActivation, iamInstallation, liveTable: partialActivationRecoveryTarget, lock, attempt };
