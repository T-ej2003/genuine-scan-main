...
import { assertCanonicalTerraformSerial, assertRecoveryPlanDelta, assertRecoveryAttestation, assertVerifiedStageBRecovery, classifyRecoveryResidue, createRecoveryAttestation, signRecoveryAttestation, verifyRecoveryAttestation } from "../aws/stage-b-partial-apply-recovery-contract.mjs";
...
test("Terraform serial uses one strict safe non-negative integer contract", () => {
  ...
  assert.equal(assertCanonicalTerraformSerial("78"), 78);
  for (const value of ["78foo", "078", " 78 ", "78.0", "7.8e1", "-1", "", null, undefined, NaN, Infinity, 1.5, Number.MAX_SAFE_INTEGER + 1]) assert.throws(() => assertCanonicalTerraformSerial(value));
});
