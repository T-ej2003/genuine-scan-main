...
import { authenticatePublishedComponentAuthorization, componentIamAuthorization as contract } from "../aws/component-iam-authorization.mjs";
...
function fixture(change = () => {}) {
  ...
  const request = approvedInstallationRequest({ ...input, main, run, environment, branches, approvals,
    packageEvidence: { manifest, manifestSha256: digest(manifest), packageSha256: "b".repeat(64) }, now: now - 120000 });
  Object.assign(run, { status: "completed", conclusion: "success" });
  const files = { "component-installation-invocation.json": { StatusCode: 200, ExecutedVersion: "3" }, "component-installation-request.json": request,
  ...
  const artifact = { id: 55, name: "component-installation-authorization-audit", expired: false, size_in_bytes: archive.length,
  ...
  f.authenticate = () => authenticatePublishedComponentAuthorization(input, { now: () => f.now, env: {}, execute: (_cmd, args) => {
