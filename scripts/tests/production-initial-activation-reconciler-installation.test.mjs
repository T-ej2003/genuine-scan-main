...
const stateAttributes = (entry) => {
  const policyArn = { reconciler: INSTALLATION.policyArn, mixed_recovery: MIXED_RECOVERY_EXECUTOR.policyArn, bootstrap_operator_policy_authorizer: BOOTSTRAP_OPERATOR_POLICY_AUTHORIZER.policyArn, broker_recovery_successor_evidence_reader: BROKER_RECOVERY_SUCCESSOR_EVIDENCE_READER.policyArn }[entry.name];
  return entry.type === "aws_iam_policy" ? { ...entry.change.after, arn: policyArn, id: policyArn } : entry.change.after;
};
...
test("exact predecessor executor policy is the only governed update and five versions fail closed", () => {
  assert.equal(discoverInstallationPredecessor({ run: discoveryRun({ document: installationPermissionsPredecessor(), mixedRole: false, mixedPolicy: false, readerRole: false, readerPolicy: false }) }).classification, "EXACT_EXPANSION");
  assert.equal(discoverInstallationPredecessor({ run: discoveryRun({ document: installationPermissionsPredecessor(), mixedRole: false, mixedPolicy: false, readerRole: false, readerPolicy: false, versions: [1, 2, 3, 4, 5].map((id) => ({ VersionId: `v${id}`, IsDefaultVersion: id === 1 })) }) }).classification, "UNEXPECTED");
...
test("executor applies every authenticated partial dedicated-role expansion", () => {
  ...
  for (const present of [["aws_iam_role.mixed_recovery"], ["aws_iam_policy.mixed_recovery"], ["aws_iam_policy.mixed_recovery", "aws_iam_role.mixed_recovery"]]) {
    ...
    const postState = JSON.stringify({ ...JSON.parse(updatePostState), resources: prefixPlan.resource_changes.map((entry) => ({ mode: entry.mode, type: entry.type, name: entry.name, provider: 'provider["registry.terraform.io/hashicorp/aws"]', instances: [{ schema_version: 0, attributes: stateAttributes(entry), sensitive_attributes: [] }] })) });
