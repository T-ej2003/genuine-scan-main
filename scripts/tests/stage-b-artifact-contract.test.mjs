...
test("initial-activation reconciler installation artifacts form a closed private chain", () => {
  const expected = {
    ...
    "initial-activation-reconciler-installation-saved-plan": [".github/workflows/authorize-production-initial-activation-policy-reconciler-installation.yml", "scripts/aws/dispatch-production-initial-activation-reconciler-installation.mjs", "scripts/aws/prepare-production-initial-activation-reconciler-installation.mjs", "scripts/aws/install-production-initial-activation-reconciler.mjs"],
    ...
    "initial-activation-reconciler-installation-preparation": [".github/workflows/authorize-production-initial-activation-policy-reconciler-installation.yml", "scripts/aws/authorize-production-initial-activation-reconciler-installation.mjs", "scripts/aws/dispatch-production-initial-activation-reconciler-installation.mjs", "scripts/aws/install-production-initial-activation-reconciler.mjs"],
...
test("real release-read and backend producers normalize generated permissions", () => {
  ...
  const run = (args, probe) => {
    ...
    if (probe.action === "ecr:DescribeImages" && args.includes("--image-ids")) {
      const repositoryName = args[args.indexOf("--repository-name") + 1]; const imageDigest = args.at(-1).slice("imageDigest=".length);
  ...
  const publicationImages = [
    ["backend", "mscqr-backend", "b"], ["worker", "mscqr-worker", "a"], ["rls-executor", "mscqr-backend", "e"], ["rls-canary", "mscqr-backend", "c"],
  ].map(([service, repository, marker]) => ({ service, repository, digest: `sha256:${marker.repeat(64)}` }));
  const result = runReleaseReadPreflight({ authenticatedImageAuthorization: { images: publicationImages, imageEvidence: { images: publicationImages } }, outputDirectory: directory, run });
