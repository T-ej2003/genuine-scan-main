...
const values = Object.freeze({ MSCQR_VERIFIER_MFA_SERIAL: "arn:aws:iam::368992683803:mfa/operator", MSCQR_VERIFIER_MFA_CODE: "123456", MSCQR_ONBOARDING_EMAIL: "administration@example.invalid", MSCQR_ONBOARDING_PASSWORD: "fixture-admin-value", MSCQR_CANARY_ORDINARY_EMAIL: "canary@example.invalid", MSCQR_CANARY_ORDINARY_PASSWORD: "fixture-canary-value" });
...
test("operator handoff collects exactly six hidden pre-launch inputs and passes them only to its child", async () => {
  const prompts = [];
  let spawned;
  const output = [];
  const result = await runProductionCutoverOperator({
    ...
    prompt: async (request) => { prompts.push(request); return values[request.prompt.includes("verifier MFA serial") ? "MSCQR_VERIFIER_MFA_SERIAL" : request.prompt.includes("verifier MFA code") ? "MSCQR_VERIFIER_MFA_CODE" : request.prompt.includes("administrator email") ? "MSCQR_ONBOARDING_EMAIL" : request.prompt.includes("administrator password") ? "MSCQR_ONBOARDING_PASSWORD" : request.prompt.includes("tenant-canary email") ? "MSCQR_CANARY_ORDINARY_EMAIL" : "MSCQR_CANARY_ORDINARY_PASSWORD"]; },
    spawnChild: childThatExits({ capture: (value) => { spawned = { ...value, environment: { ...value.options.env } }; } }),
  });
  assert.equal(result.exitCode, 0);
  assert.equal(result.signal, null);
  assert.deepEqual(prompts.map(({ prompt }) => prompt), ["Production verifier MFA serial: ", "Production verifier MFA code: ", "Production strict-onboarding administrator email: ", "Production strict-onboarding administrator password: ", "Production strict-onboarding tenant-canary email: ", "Production strict-onboarding tenant-canary password: "]);
  assert.equal(prompts.some(({ prompt }) => /MFA_BOOTSTRAP|onboarding MFA|tenant-canary MFA/.test(prompt)), false);
...
test("operator handoff fails closed before a child exists for empty or malformed input", async () => {
  ...
  await assert.rejects(() => runProductionCutoverOperator({ argv, stdin: { isTTY: true }, stdout: { isTTY: true }, prompt: async (request) => request.prompt.includes("MFA code") ? "bad" : "valid", spawnChild: () => { spawns += 1; } }), /MSCQR_VERIFIER_MFA_CODE entry failed/);
...
test("operator handoff preserves the child exit outcome and clears its child-only environment object", async () => {
  let invocation;
  const result = await runProductionCutoverOperator({ argv, stdin: { isTTY: true }, stdout: { isTTY: true }, prompt: async (request) => values[request.prompt.includes("verifier MFA serial") ? "MSCQR_VERIFIER_MFA_SERIAL" : request.prompt.includes("verifier MFA code") ? "MSCQR_VERIFIER_MFA_CODE" : request.prompt.includes("administrator email") ? "MSCQR_ONBOARDING_EMAIL" : request.prompt.includes("administrator password") ? "MSCQR_ONBOARDING_PASSWORD" : request.prompt.includes("tenant-canary email") ? "MSCQR_CANARY_ORDINARY_EMAIL" : "MSCQR_CANARY_ORDINARY_PASSWORD"], spawnChild: childThatExits({ code: 17, capture: (value) => { invocation = value; } }) });
