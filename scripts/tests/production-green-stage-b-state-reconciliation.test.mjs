...
test("apply failure with exact predecessor readback is a durable zero-mutation result", () => {
  const preparation = prepare(); const authorization = createStageBStateReconciliationAuthorization({ preparation, approval: approval(), now }); let current = { ...state };
  assert.throws(() => executeStageBStateReconciliation({ sourceSha, preparation, authorization, bindings: execBindings(), planBytes: bytes, planJson: plan(), readState: () => current, applyRefreshOnlyPlan: () => { throw new Error("lock acquisition failed"); }, renderRefreshClosurePlan: cleanPlan, renderNormalClosurePlan: cleanPlan, reauthenticateSource: () => {}, now }), (error) => error.reconciliationResult?.status === "state-write-not-committed" && error.reconciliationResult.successorState.serial === 104 && error.reconciliationResult.remainingExpectedStateObservations === null && error.reconciliationResult.sourceToLiveIamSemanticDifferences === null && error.reconciliationResult.newUnexpectedDriftCount === null && error.reconciliationResult.remoteResourceMutationCount === 0 && error.reconciliationResult.terraformStateMutationCount === 0);
});

test("successful apply with failed successor readback retains durable ambiguous evidence", () => {
  const preparation = prepare(); const authorization = createStageBStateReconciliationAuthorization({ preparation, approval: approval(), now }); let current = { ...state }; let reads = 0;
  assert.throws(() => executeStageBStateReconciliation({ sourceSha, preparation, authorization, bindings: execBindings(), planBytes: bytes, planJson: plan(), readState: () => { reads += 1; if (reads === 2) throw new Error("state readback unavailable"); return current; }, applyRefreshOnlyPlan: () => { current = { ...current, serial: 105, stateSha256: "c".repeat(64) }; }, renderRefreshClosurePlan: cleanPlan, renderNormalClosurePlan: cleanPlan, reauthenticateSource: () => {}, now }), (error) => error.reconciliationResult?.status === "state-write-completed-postverify-failed" && error.reconciliationResult.successorState === null && error.reconciliationResult.remainingExpectedStateObservations === null && error.reconciliationResult.sourceToLiveIamSemanticDifferences === null && error.reconciliationResult.newUnexpectedDriftCount === null && error.reconciliationResult.remoteResourceMutationCount === 0 && error.reconciliationResult.terraformStateMutationCount === 1);
});
