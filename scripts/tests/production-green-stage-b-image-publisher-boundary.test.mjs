...
test("repository OIDC stays default and each consumer has an explicit subject migration", () => {
  ...
  assert.match(releaseGate.migration, /dedicated exact-main release-gate mutation boundary/);
  assert.ok(transition.repositoryOidcConsumers.filter(({ workflow }) => !workflow.endsWith("release-gate.yml") && !workflow.endsWith("production-green-stage-b-image-build.yml")).every(({ migration }) => migration.startsWith("none;")));
