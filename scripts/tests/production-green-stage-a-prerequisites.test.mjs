...
test("Stage A backend proxy collector binds mutable ALB XFF attributes and rejects topology drift", () => {
  ...
  const replace = (needle, replacement) => (args) => {
    ...
    if (args[0] === "cloudfront" && args[1] === "get-distribution-config" && ["behavior", "viewer-allow-http", "origin-http", "origin-match-viewer", "origin-port"].includes(needle)) {
  ...
  assert.throws(() => assertStageBBackendProxyTrustCurrent({ expected: trust, vpcId: "vpc-0123456789abcdef0", run: replace("viewer-allow-http") }), /redirect viewers to HTTPS/);
