...
function wrapperFixture({ approvedPlan = plan, shownPlan, savedBytes = savedPlanBytes, approvedBytes } = {}) {
  ...
  const publicationIdentity = buildStageBImagePublicationIdentity({ expectedToolingSha: "b".repeat(40), expectedReleaseSha: releaseSha, artifactBytes: imageArtifactBytes, observed: { workflowRunId: "30760789616", workflowDatabaseId: "401", workflowFile: ".github/workflows/production-green-stage-b-images.yml", workflowName: "Production Green Stage B Images", event: "workflow_dispatch", workflowDefinitionSha: "b".repeat(40), imageReleaseSha: releaseSha, headBranch: "main", conclusion: "success", artifactId: "501", artifactName: "production-green-stage-b-images", artifactExpired: false, artifactArchiveFilename: null }, observedAt: imageObservedAt });
  ...
  const checks = [...STAGE_B_EXPECTED_CHECK_ADDRESSES, ...STAGE_B_EXPECTED_VARIABLE_CHECK_ADDRESSES].map((address) => ({ address, status: "pass", instances: [{ address, status: "pass" }] }));
...
test("exact PARTIAL_APPLY_RECOVERY artifact set reaches the apply seam without ECS mutation authority", () => {
  ...
  const binding = { ...JSON.parse(fs.readFileSync(fixture.tfvarsBindingReportPath)), stateSerial: 96, sourceContractSha256: "s".repeat(64), migrationSetDigest: "m".repeat(64), packageChecksumSha256: "p".repeat(64) };
  ...
  const observationBinding = { ...binding, recoveryOnly: false, tfvarsSha256: "o".repeat(64) }; const observationBytes = Buffer.from(`${JSON.stringify(observationBinding)}\n`); fs.writeFileSync(observationPath, observationBytes, { mode: 0o600 }); fixture.refreshBindingReportPath = observationPath; fixture.refreshBindingReportSha256 = crypto.createHash("sha256").update(observationBytes).digest("hex");
  const currentBinding = { ...binding, recoveryOnly: false }; fs.writeFileSync(fixture.tfvarsBindingReportPath, `${JSON.stringify(currentBinding)}\n`, { mode: 0o600 }); fixture.tfvarsBindingReportSha256 = crypto.createHash("sha256").update(fs.readFileSync(fixture.tfvarsBindingReportPath)).digest("hex");
  const refresh = JSON.parse(fs.readFileSync(fixture.refreshReportPath)); Object.assign(refresh, { status: "RESOURCE_DRIFT", resourceChanges: { nonNoOp: 1, changes: [{ address: "aws_lambda_alias.reviewed", type: "aws_lambda_alias", actions: ["update"] }] }, bindingReportSha256: fixture.refreshBindingReportSha256, tfvarsSha256: observationBinding.tfvarsSha256, stageBStateSerial: 96, stageBStateSha256: binding.stateBackupSha256, stateBackupSha256: binding.stateBackupSha256 });
  ...
  currentBinding.recoveryRefreshReportSha256 = fixture.refreshReportSha256; writePrivate(fixture.tfvarsBindingReportPath, `${JSON.stringify(currentBinding)}\n`); fixture.tfvarsBindingReportSha256 = crypto.createHash("sha256").update(fs.readFileSync(fixture.tfvarsBindingReportPath)).digest("hex");
