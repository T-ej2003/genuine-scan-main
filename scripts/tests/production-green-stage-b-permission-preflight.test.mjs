...
function wrapperFixture({ approvedPlan = plan, shownPlan, savedBytes = savedPlanBytes, approvedBytes } = {}) {
  ...
  const tfvarsBindingReport = {
    ...
    brokerPackageRawSha256: crypto.createHash("sha256").update(brokerBytes).digest("hex"), brokerPackageBase64Sha256: crypto.createHash("sha256").update(brokerBytes).digest("base64"), recoveryMode: "NORMAL", backendProxyTrust: { mode: "cloudfront-alb", albCidrs: "10.1.0.0/24,10.1.1.0/24", cloudFrontCidrs: "198.51.100.0/24" },
