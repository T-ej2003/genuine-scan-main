...
test("Stage A live-evidence policy source contains no mutation permission", () => {
  ...
  assert.deepEqual(statement, { Sid: "ReadStageALivePrerequisites", Effect: "Allow", Action: ["ec2:DescribeSubnets", "ec2:DescribeRouteTables", "ec2:DescribeSecurityGroups", "ecs:DescribeClusters", "rds:DescribeDBInstances", "ec2:DescribeManagedPrefixLists", "ec2:GetManagedPrefixListEntries", "elasticloadbalancing:DescribeLoadBalancers", "elasticloadbalancing:DescribeLoadBalancerAttributes", "elasticloadbalancing:DescribeTargetGroups"], Resource: "*", Condition: { StringEquals: { "aws:RequestedRegion": "eu-west-2" } } });
...
test("production-shaped plan requires and binds the exact account and region variables", () => {
  ...
  assert.equal(report.status, "valid");
  assert.equal(report.requiredEvaluations.length, 270);
...
function wrapperFixture({ approvedPlan = plan, shownPlan, savedBytes = savedPlanBytes, approvedBytes } = {}) {
  ...
  const tfvarsBindingReport = {
    ...
    brokerPackageRawSha256: crypto.createHash("sha256").update(brokerBytes).digest("hex"), brokerPackageBase64Sha256: crypto.createHash("sha256").update(brokerBytes).digest("base64"), recoveryMode: "NORMAL", backendProxyTrust: { mode: "cloudfront-alb", albCidrs: "10.1.0.0/24,10.1.1.0/24", cloudFrontCidrs: "198.51.100.0/24", cloudFrontPrefixListId: "pl-0123456789abcdef0", cloudFrontPrefixListVersion: "7" },
