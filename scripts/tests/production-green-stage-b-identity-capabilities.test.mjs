...
test("generated capability graph is exhaustive, deterministic, and identity-exact", () => {
  ...
  assert.deepEqual(assertStageBDeploymentCapabilityGraph(first), { phases: 56, capabilities: 668, uniqueActions: 157, unmappedCalls: 0, unclassifiedCapabilities: 0, identityBoundaryViolations: 0, sourcePolicyMismatches: 0, manifestMismatches: 0, configurationContradictions: 0 });
  ...
  assert.deepEqual(producerReads.map(({ action }) => action).sort(), ["cloudfront:GetDistributionConfig", "cloudfront:ListDistributions", "ec2:DescribeManagedPrefixLists", "ec2:DescribeRouteTables", "ec2:DescribeSecurityGroups", "ec2:DescribeSubnets", "ec2:GetManagedPrefixListEntries", "ecs:DescribeClusters", "elasticloadbalancing:DescribeLoadBalancerAttributes", "elasticloadbalancing:DescribeLoadBalancers", "elasticloadbalancing:DescribeTargetGroups", "rds:DescribeDBInstances", "route53:ListResourceRecordSets", "s3:GetObject"]);
  ...
  const reconcilerCapabilities = first.capabilities.filter(({ identity, phase }) => identity === "INITIAL_ACTIVATION_RECONCILER" && phase === "initial-activation-lifecycle-policy-reconciliation");
  ...
  const providerReadonlyCapabilities = first.capabilities.filter(({ identity, phase }) => identity === "INITIAL_ACTIVATION_RECONCILER" && phase === "provider-readonly-policy-reconciliation");
  ...
  assert.equal(first.capabilities.filter(({ id, phase, identity, action, resources }) => id === "mixed-recovery-iam-attestation-sign" && phase === "mixed-dual-slot-recovery-iam-preflight" && identity === "ROOT_OPERATOR" && action === "kms:Sign" && resources.includes("arn:aws:kms:eu-west-2:368992683803:alias/mscqr-production-root-attestation")).length, 1);
