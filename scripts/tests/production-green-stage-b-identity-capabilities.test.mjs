...
test("identity matrix assigns IAM simulation only to administrator", () => {
  ...
  assert.equal(matrix.phases.length, 32);
});
...
test("generated capability graph is exhaustive, deterministic, and identity-exact", () => {
  ...
  assert.deepEqual(assertStageBDeploymentCapabilityGraph(first), { phases: 32, capabilities: 299, uniqueActions: 102, unmappedCalls: 0, unclassifiedCapabilities: 0, identityBoundaryViolations: 0, sourcePolicyMismatches: 0, manifestMismatches: 0, configurationContradictions: 0 });
  assert(first.capabilities.every(({ identity }) => first.identities.includes(identity)));
  assert(first.identities.includes("GITHUB_PRODUCTION_MUTATION"));
  assert(first.capabilities.some(({ identity, action }) => identity === "GITHUB_PRODUCTION_MUTATION" && action === "ecs:UpdateService"));
