...
test("generated capability graph is exhaustive, deterministic, and identity-exact", () => {
  ...
  assert.deepEqual(assertStageBDeploymentCapabilityGraph(first), { phases: 34, capabilities: 262, uniqueActions: 125, unmappedCalls: 0, unclassifiedCapabilities: 0, identityBoundaryViolations: 0, sourcePolicyMismatches: 0, manifestMismatches: 0, configurationContradictions: 0 });
  ...
  assert(first.capabilities.some(({ id, action, resources }) => id === "manifest-backend-health-recovery-register-legacy-task-definition" && action === "ecs:RegisterTaskDefinition" && resources.length === 1 && resources[0].endsWith("task-definition/mscqr-backend:*")));
  assert(first.capabilities.some(({ id, action, resources }) => id === "manifest-backend-health-recovery-update-service" && action === "ecs:UpdateService" && resources.length === 1 && resources[0].endsWith("service/mscqr-prod-euw2-main/mscqr-backend-servi-euw2")));
