...
test("generated capability graph is exhaustive, deterministic, and identity-exact", () => {
  ...
  assert.deepEqual(assertStageBDeploymentCapabilityGraph(first), { phases: 56, capabilities: 662, uniqueActions: 154, unmappedCalls: 0, unclassifiedCapabilities: 0, identityBoundaryViolations: 0, sourcePolicyMismatches: 0, manifestMismatches: 0, configurationContradictions: 0 });
  ...
  assert.deepEqual(producerReads.map(({ action }) => action).sort(), ["cloudfront:ListDistributions", "ec2:DescribeManagedPrefixLists", "ec2:DescribeRouteTables", "ec2:DescribeSecurityGroups", "ec2:DescribeSubnets", "ec2:GetManagedPrefixListEntries", "ecs:DescribeClusters", "elasticloadbalancing:DescribeLoadBalancers", "elasticloadbalancing:DescribeTargetGroups", "rds:DescribeDBInstances", "s3:GetObject"]);
