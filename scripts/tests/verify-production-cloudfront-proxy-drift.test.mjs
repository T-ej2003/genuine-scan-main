...
const run = (drift = {}) => (args) => {
  ...
  if (operation === "cloudfront get-distribution-config") return JSON.stringify({ ETag: "E1", DistributionConfig: { Enabled: true, Aliases: { Items: ["mscqr.com", "www.mscqr.com"] }, Origins: { Items: [{ Id: "api", DomainName: "mscqr-alb.example.elb.amazonaws.com", CustomOriginConfig: { OriginProtocolPolicy: drift.originPolicy || "https-only", HTTPSPort: drift.originHttpsPort || 443 } }] }, OriginGroups: { Quantity: 0 }, DefaultCacheBehavior: { TargetOriginId: "api", ViewerProtocolPolicy: drift.viewerPolicy || "redirect-to-https" }, CacheBehaviors: drift.sampledOnlyHttps ? { Items: [{ PathPattern: "/api/health/ready", TargetOriginId: "api", ViewerProtocolPolicy: "redirect-to-https" }, { PathPattern: "/api/auth/login", TargetOriginId: "api", ViewerProtocolPolicy: "redirect-to-https" }, { PathPattern: "/api/*", TargetOriginId: "api", ViewerProtocolPolicy: "allow-all" }] } : drift.wildcardApi ? { Items: [{ PathPattern: "*/admin/*", TargetOriginId: "other", ViewerProtocolPolicy: "allow-all" }] } : { Quantity: 0 } } });
...
test("production proxy drift verifier accepts only an exact live task-definition binding", () => {
  ...
  assert.throws(() => verifyProductionCloudFrontProxyDrift({ run: run({ sampledOnlyHttps: true }) }), /redirect viewers to HTTPS/);
  assert.throws(() => verifyProductionCloudFrontProxyDrift({ run: run({ wildcardApi: true }) }), /redirect viewers to HTTPS/);
