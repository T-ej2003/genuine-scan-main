...
test("workflow and runbook bind the dedicated refresh-only operation", () => {
  ...
  for (const workflow of [authorizer, executor]) { assert.match(workflow, /environment: production-initial-activation-reconciler-bootstrap/); assert.match(workflow, /group: production-deploy/); assert.match(workflow, /cancel-in-progress: false/); }
