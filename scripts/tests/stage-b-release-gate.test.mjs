...
const run = (calls, failures = new Set()) => (_file, args) => {
  ...
  return args.join(" ") === "rev-parse HEAD" ? `${sha}\n` : "";
};
...
test("Stage B accepts only a checked-out release SHA merged into origin/main", () => {
  ...
  assert.deepEqual(calls.map((args) => args.join(" ")), ["fetch --no-tags origin main", `cat-file -e ${sha}^{commit}`, `merge-base --is-ancestor ${sha} origin/main`, "rev-parse HEAD"]);
  ...
  assert.throws(() => assertMergedStageBRelease(sha, { run: run([], new Set([`merge-base --is-ancestor ${sha} origin/main`])) }), /git rejected/);
  assert.throws(() => assertMergedStageBRelease(sha, { run: (_file, args) => args.join(" ") === "rev-parse HEAD" ? "b".repeat(40) : "" }), /does not equal/);
});
