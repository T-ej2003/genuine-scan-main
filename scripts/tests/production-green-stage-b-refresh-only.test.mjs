...
const passingChecks = () => [...STAGE_B_EXPECTED_CHECK_ADDRESSES, ...STAGE_B_EXPECTED_VARIABLE_CHECK_ADDRESSES, ...STAGE_B_EXPECTED_RESOURCE_PRECONDITION_ADDRESSES].map((address) => ({ address, status: "pass", instances: [{ address, status: "pass" }] }));
const noChangePlan = (terraform_version = "1.15.7") => ({ format_version: "1.2", terraform_version, planned_values: { root_module: {} }, configuration: { root_module: {} }, prior_state: {}, errored: false, diagnostics: [], resource_changes: [{ address: "aws_lambda_function.broker", type: "aws_lambda_function", change: { actions: ["no-op"] } }], resource_drift: [], output_changes: {}, checks: passingChecks() });
...
test("resource precondition discovery is source-derived and exact", () => {
  assert.deepEqual(resourcePreconditionAddressesFromSource('resource "aws_lambda_function" "broker" { lifecycle { precondition { condition = true error_message = "ok" } } }'), ["aws_lambda_function.broker"]);
  assert.deepEqual(STAGE_B_EXPECTED_RESOURCE_PRECONDITION_ADDRESSES, ["aws_lambda_function.broker"]);
});
...
test("the protected resource precondition is accepted only in its Terraform 1.15 result shape", () => {
  const resource = { address: { kind: "resource", mode: "managed", type: "aws_lambda_function", name: "broker", to_display: "aws_lambda_function.broker" }, status: "pass", instances: [{ address: { to_display: "aws_lambda_function.broker" }, status: "pass" }] };
  const result = inspectStageBRefreshChecks({ checks: [...passingChecks().filter(({ address }) => address !== "aws_lambda_function.broker"), resource] });
  assert.equal(result.valid, true);
  assert.equal(result.resourcePreconditionCheckCount, 1);
  const failed = { ...resource, status: "fail", instances: [{ ...resource.instances[0], status: "fail", problems: [{ message: "environment too large" }] }] };
  assert.equal(inspectStageBRefreshChecks({ checks: [...passingChecks().filter(({ address }) => address !== "aws_lambda_function.broker"), failed] }).valid, false);
});
test("resource checks outside the protected precondition set fail closed", () => {
  const unknown = { address: { kind: "resource", mode: "managed", type: "aws_ecs_task_definition", name: "worker", to_display: "aws_ecs_task_definition.worker" }, status: "pass", instances: [{ address: { to_display: "aws_ecs_task_definition.worker" }, status: "pass" }] };
  const result = inspectStageBRefreshChecks({ checks: [...passingChecks().filter(({ address }) => address !== "aws_lambda_function.broker"), unknown] });
  assert.equal(result.valid, false);
  assert.ok(result.unknownCheckCount > 0);
});
test("malformed resource check shapes fail closed", () => {
  const malformed = { address: { kind: "resource", mode: "managed", type: "aws_lambda_function", name: "broker", to_display: "aws_lambda_function.other" }, status: "pass", instances: [{ address: { to_display: "aws_lambda_function.broker" }, status: "pass" }] };
  const result = inspectStageBRefreshChecks({ checks: [...passingChecks().filter(({ address }) => address !== "aws_lambda_function.broker"), malformed] });
  assert.equal(result.valid, false);
...
test("valid refresh-only invokes Terraform once and emits non-deployable JSON evidence", () => { const directory = fs.mkdtempSync(path.join(os.tmpdir(), "stage-b-refresh-contract-")); let calls = 0; const result = runRefreshOnly({ argv: args(directory), env: { TF_WORKSPACE: "default" }, deps: { ...validDeps(), runTerraform: (terraformArgs) => { calls += 1; writeTemporaryPlan(terraformArgs); return { status: 0, stdout: "No changes.\n", stderr: "" }; } } }); const report = JSON.parse(fs.readFileSync(result.outputPath)); const expectedChecks = STAGE_B_EXPECTED_CHECK_ADDRESSES.length + STAGE_B_EXPECTED_VARIABLE_CHECK_ADDRESSES.length + STAGE_B_EXPECTED_RESOURCE_PRECONDITION_ADDRESSES.length; assert.equal(calls, 1); assert.equal(result.status, "NO_CHANGES"); assert.equal(report.deployablePlan, false); assert.equal(report.acquisitionStatus, "valid"); assert.equal(report.terraformVersion, "1.15.7"); assert.equal(report.terraformVersionSha256, crypto.createHash("sha256").update(report.terraformVersion).digest("hex")); assert.equal(report.formatVersion, "1.2"); assert.equal(report.planCommandExitCode, 0); assert.equal(report.showCommandExitCode, 0); assert.match(report.refreshPlanSha256, /^[a-f0-9]{64}$/); assert.match(report.refreshPlanJsonSha256, /^[a-f0-9]{64}$/); assert.deepEqual(report.resourceChanges, { nonNoOp: 0, changes: [] }); assert.equal(report.checkCount, expectedChecks); assert.equal(report.infrastructureCheckCount, STAGE_B_EXPECTED_CHECK_ADDRESSES.length); assert.equal(report.variableCheckCount, STAGE_B_EXPECTED_VARIABLE_CHECK_ADDRESSES.length); assert.equal(report.resourcePreconditionCheckCount, STAGE_B_EXPECTED_RESOURCE_PRECONDITION_ADDRESSES.length); assert.equal(report.passedCheckCount, expectedChecks); assert.equal(report.failedCheckCount, 0); assert.equal(report.malformedCheckCount, 0); assert.equal(report.missingCheckCount, 0); assert.equal(report.unknownCheckCount, 0); assert.equal(report.duplicateCheckCount, 0); assert.equal(report.emittedInstanceCount, expectedChecks); assert.equal(report.passedInstanceCount, expectedChecks); assert.equal(report.failedInstanceCount, 0); assert.equal(report.malformedInstanceCount, 0); assert.equal(report.duplicateCheckCount, 0); assert.match(report.instanceInventoryHash, /^[a-f0-9]{64}$/); assert.deepEqual(report.failedChecks, []); });
test("refresh-only publishes reviewed resource drift for recovery while normal evidence rejects it", () => { const directory = fs.mkdtempSync(path.join(os.tmpdir(), "stage-b-refresh-contract-")); const plan = noChangePlan(); plan.resource_changes = [{ address: "aws_lambda_function.broker", type: "aws_lambda_function", change: { actions: ["update"] } }]; const result = runRefreshOnly({ argv: args(directory), env: { TF_WORKSPACE: "default" }, deps: validDeps(plan) }); assert.equal(result.status, "RESOURCE_DRIFT"); const report = JSON.parse(fs.readFileSync(result.outputPath)); assert.equal(report.deployablePlan, false); assert.throws(() => assertStageBRefreshEvidence({ refreshReportPath: result.outputPath, bindingReport: bindingReport() }), /not an approved/); assert.doesNotThrow(() => assertStageBRefreshEvidence({ refreshReportPath: result.outputPath, bindingReport: { ...bindingReport(), stateBackupSha256: stateHash }, allowReviewedResourceDrift: true })); });
...
test("Terraform detailed exit 2 with an omitted or explicit completion marker and an empty semantic change set is no change", () => {
  for (const plan of [noChangePlan(), { ...noChangePlan(), complete: true }]) assert.equal(classifyStageBRefreshResult({ plan, terraformExitCode: 2, bindingReport: bindingReport(), state, outputsSource }).status, "NO_CHANGES");
});
...
test("all source-defined checks and variable validations pass before no-change classification", () => { const result = classifyStageBRefreshResult({ plan: noChangePlan(), bindingReport: bindingReport(), state, outputsSource }); const expectedChecks = STAGE_B_EXPECTED_CHECK_ADDRESSES.length + STAGE_B_EXPECTED_VARIABLE_CHECK_ADDRESSES.length + STAGE_B_EXPECTED_RESOURCE_PRECONDITION_ADDRESSES.length; assert.equal(result.checkCount, expectedChecks); assert.equal(result.infrastructureCheckCount, STAGE_B_EXPECTED_CHECK_ADDRESSES.length); assert.equal(result.variableCheckCount, STAGE_B_EXPECTED_VARIABLE_CHECK_ADDRESSES.length); assert.equal(result.resourcePreconditionCheckCount, STAGE_B_EXPECTED_RESOURCE_PRECONDITION_ADDRESSES.length); assert.equal(result.passedCheckCount, expectedChecks); assert.equal(result.failedCheckCount, 0); assert.equal(result.malformedCheckCount, 0); assert.equal(result.missingCheckCount, 0); assert.equal(result.unknownCheckCount, 0); assert.equal(result.duplicateCheckCount, 0); assert.deepEqual(result.failedChecks, []); });
const structuredCheckAddress = (address, renderedKey) => address.startsWith("aws_") ? { kind: "resource", mode: "managed", type: address.slice(0, address.indexOf(".")), name: address.slice(address.indexOf(".") + 1), [renderedKey]: address } : { kind: address.slice(0, address.indexOf(".")), name: address.slice(address.indexOf(".") + 1), [renderedKey]: address };
test("Terraform JSON display addresses are normalized for checks and instances", () => { const checks = passingChecks().map((check) => ({ ...check, address: structuredCheckAddress(check.address, "to_display"), instances: [{ ...check.instances[0], address: { to_display: check.address } }] })); assert.equal(classifyStageBRefreshResult({ plan: { ...noChangePlan(), checks }, bindingReport: bindingReport(), state, outputsSource }).status, "NO_CHANGES"); });
test("check address parser accepts direct strings and reviewed to_string forms", () => { assert.equal(inspectStageBRefreshChecks({ checks: passingChecks() }).valid, true); const checks = passingChecks().map((check) => ({ ...check, address: structuredCheckAddress(check.address, "to_string"), instances: [{ ...check.instances[0], address: { to_display: check.address } }] })); assert.equal(inspectStageBRefreshChecks({ checks }).valid, true); });
