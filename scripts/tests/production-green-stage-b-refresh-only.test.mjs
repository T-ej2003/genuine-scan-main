...
import { acquireStageBRefreshPlan, parseCli, redactStageBRefreshDiagnostic, runRefreshOnly } from "../refresh-production-green-stage-b.mjs";
import { assertStageBRefreshEvidence, assertStageBRefreshStateBinding, classifyStageBRefreshResult, inspectStageBRefreshChecks, isSupportedStageBTerraformVersion, normalizeStageBRefreshPlan, STAGE_B_EXPECTED_CHECK_ADDRESSES, STAGE_B_EXPECTED_VARIABLE_CHECK_ADDRESSES } from "../aws/stage-b-refresh-contract.mjs";
...
test("valid refresh-only invokes Terraform once and emits non-deployable JSON evidence", () => { const directory = fs.mkdtempSync(path.join(os.tmpdir(), "stage-b-refresh-contract-")); let calls = 0; const result = runRefreshOnly({ argv: args(directory), env: { TF_WORKSPACE: "default" }, deps: { ...validDeps(), runTerraform: (terraformArgs) => { calls += 1; writeTemporaryPlan(terraformArgs); return { status: 0, stdout: "No changes.\n", stderr: "" }; } } }); const report = JSON.parse(fs.readFileSync(result.outputPath)); assert.equal(calls, 1); assert.equal(result.status, "NO_CHANGES"); assert.equal(report.deployablePlan, false); assert.equal(report.acquisitionStatus, "valid"); assert.equal(report.terraformVersion, "1.15.7"); assert.equal(report.terraformVersionSha256, crypto.createHash("sha256").update(report.terraformVersion).digest("hex")); assert.equal(report.formatVersion, "1.2"); assert.equal(report.planCommandExitCode, 0); assert.equal(report.showCommandExitCode, 0); assert.match(report.refreshPlanSha256, /^[a-f0-9]{64}$/); assert.match(report.refreshPlanJsonSha256, /^[a-f0-9]{64}$/); assert.deepEqual(report.resourceChanges, { nonNoOp: 0, changes: [] }); assert.equal(report.checkCount, 10); assert.equal(report.infrastructureCheckCount, 8); assert.equal(report.variableCheckCount, 2); assert.equal(report.passedCheckCount, 10); assert.equal(report.failedCheckCount, 0); assert.equal(report.malformedCheckCount, 0); assert.equal(report.missingCheckCount, 0); assert.equal(report.unknownCheckCount, 0); assert.equal(report.duplicateCheckCount, 0); assert.equal(report.emittedInstanceCount, 10); assert.equal(report.passedInstanceCount, 10); assert.equal(report.failedInstanceCount, 0); assert.equal(report.malformedInstanceCount, 0); assert.equal(report.duplicateInstanceCount, 0); assert.match(report.instanceInventoryHash, /^[a-f0-9]{64}$/); assert.deepEqual(report.failedChecks, []); });
...
test("all source-defined checks and variable validations pass before no-change classification", () => { const result = classifyStageBRefreshResult({ plan: noChangePlan(), bindingReport: bindingReport(), state, outputsSource }); assert.equal(result.checkCount, 10); assert.equal(result.infrastructureCheckCount, 8); assert.equal(result.variableCheckCount, 2); assert.equal(result.passedCheckCount, 10); assert.equal(result.failedCheckCount, 0); assert.equal(result.malformedCheckCount, 0); assert.equal(result.missingCheckCount, 0); assert.equal(result.unknownCheckCount, 0); assert.equal(result.duplicateCheckCount, 0); assert.deepEqual(result.failedChecks, []); });
