...
test("production-default image verification uses the release profile runner", () => {
  const report = reportFixture();
  ...
  try {
    assert.doesNotThrow(() => verifyImageEvidenceSignature({ report, toolingSha, signatureArtifact, now: observedAt, run: releaseRun }));
  } finally {
