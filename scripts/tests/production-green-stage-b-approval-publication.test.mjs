...
test("broker validation proves AWSCURRENT without claiming or launching work", async () => {
  ...
  const config = { clusterArn: STAGE_B.clusterArn, approvalSecretArn: STAGE_B.approvalSecretArn, executorSecurityGroupId: STAGE_B.executorSecurityGroupId, privateSubnetIds: STAGE_B.privateSubnetIds, taskDefinitionArns, templateHashes: stageBTemplateHashes(), approvalExpected: { releaseSha: sourceSha, sourceContractSha256: artifact.sourceContractSha256, migrationSetDigest: artifact.migrationSetDigest, packageChecksumSha256: artifact.packageChecksumSha256, deploymentId: artifact.deploymentId, approvalId: artifact.approvalId, ticketId: artifact.ticketId, images: { backendImageDigest: artifact.backendImageDigest, workerImageDigest: artifact.workerImageDigest, executorImageDigest: artifact.executorImageDigest, canaryImageDigest: artifact.canaryImageDigest }, taskDefinitionArns }, images: { backendImageDigest: artifact.backendImageDigest, workerImageDigest: artifact.workerImageDigest, executorImageDigest: artifact.executorImageDigest, canaryImageDigest: artifact.canaryImageDigest } };
  const event = buildApprovalPublicationValidationRequest({ approvalBytes: Buffer.from(raw), expectedSourceSha: sourceSha });
  const result = await createHandler({ config, readApproval: async () => raw, verifySignature, claimApproval: async () => calls.push("claim"), runTask: async () => calls.push("run"), now: () => now })(event);
...
test("broker validation rejects tampered bytes and wrong source without AWS work", async () => {
  ...
  const handler = createHandler({ config, readApproval: async () => raw, verifySignature, claimApproval: async () => assert.fail("claim must not run"), runTask: async () => assert.fail("run must not run"), now: () => now });
