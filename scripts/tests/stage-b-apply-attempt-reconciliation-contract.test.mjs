...
test("a durable pre-spawn claim accepts fresh authorization renewal without changing successor identity", () => {
  const initial = reservation(); const beforeSpawn = intent(initial); const successor = "e".repeat(40);
  const firstArtifact = createStageBApplyAttemptReconciliationArtifact({ historicalReservation: initial, historicalTransitions: [beforeSpawn], successorSourceSha: successor, generatedAt: now.toISOString(), now }); const firstArtifactSha = stageBApplyAttemptReconciliationSha256(firstArtifact, { now });
  const firstAuthorization = createStageBApplyAttemptReconciliationAuthorization({ protectedEnvironmentApprovalEvidence: approval({ source: successor }), reconciliationArtifact: firstArtifact, reconciliationArtifactSha256: firstArtifactSha, successorSourceSha: successor, approvedBy: "reviewer", approverRole: "independent-production-reviewer", verificationRef: "review-1", now });
  const claim = createStageBApplyAttemptReconciliationClaim({ reservation: initial, transitions: [beforeSpawn], reconciliationArtifact: firstArtifact, reconciliationArtifactSha256: firstArtifactSha, authorization: firstAuthorization, authorizationSha256: firstAuthorization.authorizationSha256, successorSourceSha: successor, now });
  const renewedNow = new Date(now.getTime() + 30 * 60 * 1000); const renewedArtifact = createStageBApplyAttemptReconciliationArtifact({ historicalReservation: initial, historicalTransitions: [beforeSpawn], successorSourceSha: successor, generatedAt: renewedNow.toISOString(), now: renewedNow }); const renewedArtifactSha = stageBApplyAttemptReconciliationSha256(renewedArtifact, { now: renewedNow });
  const renewedAuthorization = createStageBApplyAttemptReconciliationAuthorization({ protectedEnvironmentApprovalEvidence: approval({ source: successor, observedAt: renewedNow.toISOString() }), reconciliationArtifact: renewedArtifact, reconciliationArtifactSha256: renewedArtifactSha, successorSourceSha: successor, approvedBy: "REVIEWER", approverRole: "independent-production-reviewer", verificationRef: "review-renewed", now: renewedNow });
  assert.equal(renewedAuthorization.successorAttemptId, firstAuthorization.successorAttemptId);
  assert.doesNotThrow(() => assertStageBApplyAttemptReconciliationEligibility({ reservation: initial, transitions: [beforeSpawn, claim], reconciliationArtifact: renewedArtifact, reconciliationArtifactSha256: renewedArtifactSha, authorization: renewedAuthorization, authorizationSha256: renewedAuthorization.authorizationSha256, successorSourceSha: successor, now: renewedNow }));
});
