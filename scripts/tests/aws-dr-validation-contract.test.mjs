...
assert.equal(guardrailBranchCommandsAreExact(guardrailBranches("npm run verify:guardrails:source", "set +e\n  npm run verify:guardrails")).dispatch, false);
assert.equal(guardrailBranchCommandsAreExact(guardrailBranches("set +e\n  npm run verify:guardrails:source", "npm run verify:guardrails")).pullRequest, false);
assert.equal(guardrailBranchCommandsAreExact(guardrailBranches("npm run verify:guardrails:source", "npm run verify:guardrails\n  echo success")).dispatch, false);
assert.equal(guardrailBranchCommandsAreExact(guardrailBranches("npm run verify:guardrails:source\n  echo success", "npm run verify:guardrails")).pullRequest, false);
assert.equal(guardrailBranchCommandsAreExact(guardrailBranches("npm run verify:guardrails:source", "false\n  npm run verify:guardrails")).dispatch, false);
...
assert.equal(guardrailBranchCommandsAreExact(guardrailBranches("npm run verify:guardrails:source", "npm run verify:guardrails || :")).dispatch, false);
assert.equal(guardrailBranchCommandsAreExact(guardrailBranches("npm run verify:guardrails:source", "npm run verify:guardrails; true")).dispatch, false);
assert.equal(guardrailBranchCommandsAreExact(guardrailBranches("npm run verify:guardrails:source", "npm run verify:guardrails &")).dispatch, false);
assert.equal(guardrailBranchCommandsAreExact(guardrailBranches("npm run verify:guardrails:source", "! npm run verify:guardrails")).dispatch, false);
