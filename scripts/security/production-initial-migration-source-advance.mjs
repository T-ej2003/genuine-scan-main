...
export function assertProductionStaleSupersessionPredecessor(value, { sourceSha, rotationId, supersessionEvidence } = {}) {
  exact(value, ["schemaVersion", "kind", "sourceSha", "rotationId", "staleSourceSha", "staleRotationId", "supersessionEvidenceIdentitySha256", "runtimeQrVersionLabel", "currentRotationId", "current", "predecessorIdentitySha256"], "Stale-supersession predecessor");
  ...
  if ((sourceSha !== undefined && value.sourceSha !== sourceSha) || (rotationId !== undefined && value.rotationId !== rotationId) || (evidence && (evidence.sourceSha !== value.sourceSha || evidence.staleSourceSha !== value.staleSourceSha || evidence.rotationId !== value.rotationId || evidence.staleRotationId !== value.staleRotationId || evidence.evidenceIdentitySha256 !== value.supersessionEvidenceIdentitySha256))) throw new Error("Stale-supersession predecessor does not match the authenticated transition.");
