...
export function executeProductionRotationInventory({ spawn = spawnSync, env = process.env } = {}) {
  ...
  const result = spawn("psql", ["--no-psqlrc", "--quiet", "--tuples-only", "--no-align", env.DATABASE_URL, "--command", buildRotationInventorySql(role)], { encoding: "utf8", stdio: ["ignore", "pipe", "pipe"], env: { ...env, PSQL_HISTORY: "/dev/null", PGAPPNAME: "mscqr-production-rotation-read-only-inventory" } });
